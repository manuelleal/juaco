"""organismo_v16 (SALA 2, diseno radical) = organismo/organismo_v14.py (feefc88b1fd8d434, TRONCO v14.1 CONGELADO: solo se leyo) + TOKEN:
grafo de nodos-retina (clave = retina exacta) con valor escrito de UN golpe y sobrescrito, nacidos en la primera llegada;
aristas de variante implicitas (Hamming 1) por las que un nodo sin consecuencia propia lee a sus vecinos; la BOCA lee el
grafo si tiene algo que decir y si no v14.1; las dos vias de v14.1 siguen aprendiendo de su propio error, sin tocar.
Con token=0 es organismo_v14 EXACTO y no consume rng. Generado por construye_v16.py. NO editar a mano. NO es candidato:
es el instrumento del diseno registro/investigacion/sala2/DISENO_radical.md (una sola corrida de humo permitida).
"""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.05,memoria_rechazo=20,mu_norm=True,div_signo=True,eta_s=0.15,clip_s=10.0,puerta=3,mask_rel=2,del_s=0.25,del_c=0.25,ema_c=0.05,puerta_pat=5,pat_shuf=0,pat_min=1,token=0,tok_var=1):
    ...
    return dict(token=token,tok_var=tok_var,n_tok=len(_tok),tok={''.join('1' if _i in _k else '0' for _i in range(6)):(round(float(_v[0]),3),_v[1]) for _k,_v in _tok.items()},sobre=sobre,llegadas=llegadas,sin_objetivo=sin_objetivo,memoria_rechazo=memoria_rechazo,err_max=err_max,t_conflicto=t_conflicto,t_techo=t_techo,n_techo=n_techo,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),puerta_pat=puerta_pat,pat_shuf=pat_shuf,pat_min=pat_min,n_cod=len(ncod),
