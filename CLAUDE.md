...
> 24-sep-2026: **TRONCO v14.3** (= v14.2 + reparación N). El examen del criterio v4 PASA en serie y réplica (7 puertas; T-H no se mide); compone en 3T-k hasta K_max 8 (v14.2: 1); en lo demás es v14.2 bit a bit. 24 congelados; el bloque del manifiesto lo pegó el director a mano; tag `v14.3-tronco`. **Regla 1 desde ahora: `cd organismo && python bateria_v143.py 6 && python bateria_generaliza_v143.py organismo_v143 20 --desde 101`.** No mide vida en la pista: el paquete FILTRO + TD (`tronco_v14_3`) sigue en MODESTO ×2, R0 0.63. Rama local `organelos` (sin push): equipo Fable + 3 Opus para que la selección natural ARME órganos (gramática, endosimbiosis) y cruce R0 ≥ 0.90 (cruce); ERR-123..125.
> 25-sep-2026: se integra la rama `organelos`. **F1 ARRANQUE EN FRÍO FUNCIONA ×2** (en ECO w90, el linaje de la familia que pasa sólo lo que tuvo consecuencia se sostiene 1e6 pasos sin vivero ni fundadores repuestos, 20/20 ×2; los controles dan 1/20 y 0/20). **No transfiere a la carrera (NO).** Endosimbiosis MODESTO ×2; ECO v2.1 FUNCIONA ×2 (la selección prende el órgano de enseñar). Líneas cerradas: Prometeo, cruce, anfitrión, H-PLANO w90 (CAE) y F1 en la carrera. El tronco sigue siendo v14.3. ERR-123..148; el siguiente es ERR-149. Detalle en `registro/ESTADO.md` y en el REGISTRO.
...
- **TRONCO v14.3 (24 sep ~15:10; examen del criterio v4 PASA en serie y réplica, semillas 43001–44500; permiso del director del 23-sep 19:30): v14.2 + reparación N (el paso de la vía lenta × M0/(P·P), M0 = 3.0; memoria nueva cero). Donde la masa del estímulo es 3 es v14.2 bit a bit; en 3T-k compone hasta K_max 8. Archivos `organismo/organismo_v143.py`, `organismo_v143g.py`, `bateria_v143.py`, `bateria_generaliza_v143.py` (24 congelados). Regla 1 desde ahora: `cd organismo && python bateria_v143.py 6 && python bateria_generaliza_v143.py organismo_v143 20 --desde 101` (13:49: todo PASA; G1 1.000, G2 0.967, azar 0.437, K 20/20). Identidad 114/114 (`experimentos/tronco_v14_3_examen/identidad_v143ex.py`); tag `v14.3-tronco`. v14.2 queda como regresión histórica.**
