"""Batería de v9 — examen de congelación (CRITERIO v3, sin cambiar un umbral) y, una vez congelado, regresión del tronco.

    python bateria_v8.py [semillas] [--log]        (desde organismo/)
    --log  escribe datos/examen_v9_<fecha>.log y .json (regla 10: log desde el arranque)
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv:
        _log['f'] = open(os.path.join(RAIZ, 'datos', f'examen_v9_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    prereg = os.path.join(RAIZ, 'experimentos', 'v9_memoria_rechazo', 'PREREGISTRO_v9.md')
    log(f"=== Batería v9 — CRITERIO v3, {S} semillas, Pool({N_PARALELO}) ===")
    log(f"sha organismo_v9 {h16(os.path.join(AQUI, 'organismo_v9.py'))}  bateria_v9 {h16(os.path.abspath(__file__))}"
        f"  preregistro {h16(prereg)}")
    V = {}

    with mp.Pool(N_PARALELO) as pool:
        # ---------- 5. identidad ----------
        trabajos = [(c, e, s) for c in ('v8',) for e in ESC_ID for s in range(1, 7)]
        ...
        if not V['5_identidad']:
            log("*** CRITERIO 5 FALLIDO: v9 no es el organismo que dice ser. No se corre el examen.")
            sys.exit(1)
    ...
    log("VEREDICTO bateria_v9: " + " ".join(f"{k}={V[k]}" for k in decisivos))
    if todo and S >= 20:
        log("*** v9 CUMPLE EL CRITERIO v3 con 20 semillas. Por el preregistro: CONGELAR v9 como tronco (si pasa tambien el confirmatorio).")
    elif todo:
        log(f"*** v9 cumple el criterio v3 con {S} semillas. Congelar sólo con S>=20.")
    else:
        log("*** v9 NO cumple el criterio v3. No se congela; v8 sigue siendo el tronco. No recalibrar (regla 3).")

    if _log['f']:
        dj = os.path.join(RAIZ, 'datos', f'examen_v9_{stamp}.json')
        meta = dict(fecha=time.strftime('%Y-%m-%dT%H:%M:%S'), semillas=S, veredictos=V, congelar=bool(todo and S >= 20),
                    ...
                    sha_preregistro=h16(prereg), sha_organismo_v8=h16(os.path.join(AQUI, 'organismo_v8.py')),
