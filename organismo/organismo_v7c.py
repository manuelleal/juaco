"""
Organismo v7 CANDIDATO = v6 + 2L (plasticidad estructural en Kenyon: celda con error de predicción crónico se divide hacia lo distintivo).
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
                    ...
                    if plast:
                        ...
                        for c in idx:
                            if err[c]>theta and (~activa).any():
                                ...
                                Wp[j]=Wp[c]; Wn[j]=Wn[c]; mu[j]=mu[c].copy(); err[c]=err[j]=0; splits+=1
        E-=costo
    ...
    return dict(mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),
