"""Batería de v11 — examen de congelación (CRITERIO v3, sin cambiar un umbral) y, una vez congelado, regresión del tronco.
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv:
        _log['f'] = open(os.path.join(RAIZ, 'datos', f'examen_v11_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    ...
    log(f"sha organismo_v11 {h16(os.path.join(AQUI, 'organismo_v11.py'))}  bateria_v11 {h16(os.path.abspath(__file__))}"
        f"  preregistro {h16(prereg)}")
    V = {}

    with mp.Pool(N_PARALELO) as pool:
        # ---------- 5. identidad ----------
        trabajos = [(c, e, s) for c in ('v9', 'v10') for e in ESC_ID for s in range(1, 7)]
        ...
        if not V['5_identidad']:
            log("*** CRITERIO 5 FALLIDO: v11 no es el organismo que dice ser. No se corre el examen.")
            sys.exit(1)
    ...
    log("VEREDICTO bateria_v11: " + " ".join(f"{k}={V[k]}" for k in decisivos))
    if todo and S >= 20:
        log("*** v11 CUMPLE EL CRITERIO v3 con 20 semillas. Por el preregistro: CONGELAR v11 como tronco (si pasa tambien R y K).")
    elif todo:
        log(f"*** v11 cumple el criterio v3 con {S} semillas. Congelar sólo con S>=20.")
    else:
        log("*** v11 NO cumple el criterio v3. No se congela; v9 sigue siendo el tronco. No recalibrar (regla 3).")

    if _log['f']:
        dj = os.path.join(RAIZ, 'datos', f'examen_v11_{stamp}.json')
