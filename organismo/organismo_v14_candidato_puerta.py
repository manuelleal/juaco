"""TERCER CANDIDATO A v14 (rama v14-candidato; NO es tronco hasta que el director decida): copia de
experimentos/nivel4_puerta_codigo/organismo_v13Bn5c.py = organismo_v13 (cc8b16b492d4d324) + puerta de familiaridad por evidencia
del codigo exacto (n0 = 5) Y >= 1 celda consolidada (PATC), perilla fija ON. Evidencia: registro/PROPUESTA_v14.md.
"""
"""organismo_v13Bn5c = organismo_v13B.py con puerta_pat = 5 y pat_min = 1 POR DEFECTO (PATC, enmienda 1: evidencia Y una celda consolidada). El mismo archivo con UNA constante cambiada, para
que bateria_v13Bc.py lo examine con la perilla encendida sin tocar la bateria congelada.
Generado por construye_puerta_codigo.py. NO editar."""
"""organismo_v13B = organismo/organismo_v13.py (cc8b16b492d4d324, CONGELADO: solo se leyo) + puerta de
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.05,memoria_rechazo=20,mu_norm=True,div_signo=True,eta_s=0.015,clip_s=3.0,puerta=3,puerta_pat=5,pat_shuf=0,pat_min=1):
