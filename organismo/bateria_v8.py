"""Batería de v8 — examen de congelación (CRITERIO v3) y, una vez congelado, regresión del tronco.

    python bateria_v8.py [semillas] [--log]        (desde organismo/)
    --log  escribe datos/examen_v8_<fecha>.log y .json (regla 10: log desde el arranque)

Preregistro: experimentos/congelacion_v8/PREREGISTRO_congelacion_v8.md (sha d2924e69128fe0f6).

  5. Identidad de instrumentos (primero; si falla, no se corre el examen):
       v8 == v7e(lam=0.05) en claves de v7e;  v8(lam=0) == v7 en claves de v7;  7 escenarios x semillas 1..6.
  ...
  4a. [IDENTIDAD, NO EVIDENCIA — ERR-12] splits>0 <=> err_max>0.6 con celdas<90. Si falla: instrumento roto.
  ...
  6. (fuera de esta batería) organismo/bateria.py 20 PASA y manifiesto.py --check.
"""
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv:
        _log['f'] = open(os.path.join(RAIZ, 'datos', f'examen_v8_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    prereg = os.path.join(RAIZ, 'experimentos', 'congelacion_v8', 'PREREGISTRO_congelacion_v8.md')
    log(f"=== Batería v8 — CRITERIO v3, {S} semillas, Pool({N_PARALELO}) ===")
    log(f"sha organismo_v8 {h16(os.path.join(AQUI, 'organismo_v8.py'))}  bateria_v8 {h16(os.path.abspath(__file__))}"
        f"  preregistro {h16(prereg)}")
    V = {}

    with mp.Pool(N_PARALELO) as pool:
        # ---------- 5. identidad ----------
        trabajos = [(c, e, s) for c in ('v7e', 'v7') for e in ESC_ID for s in range(1, 7)]
        ...
        for c, nombre in (('v7e', 'v8 == v7e(lam=0.05)'), ('v7', 'v8(lam=0) == v7')):
        ...
        if not V['5_identidad']:
            log("*** CRITERIO 5 FALLIDO: v8 no es el organismo que dice ser. No se corre el examen.")
            sys.exit(1)
    ...
    disc = [(r['etapa'], r['seed'], r['splits'], r['err_max']) for r in con_plast if (r['splits'] > 0) != (r['err_max'] > 0.6)]
    ...
    log("VEREDICTO bateria_v8: " + " ".join(f"{k}={V[k]}" for k in decisivos))
    if todo and S >= 20:
        log("*** v8 CUMPLE EL CRITERIO v3 con 20 semillas. Por el preregistro: CONGELAR v8 como tronco.")
    elif todo:
        log(f"*** v8 cumple el criterio v3 con {S} semillas. Congelar sólo con S>=20.")
    else:
        log("*** v8 NO cumple el criterio v3. No se congela; v6 sigue siendo el tronco. No recalibrar (regla 3).")

    if _log['f']:
        dj = os.path.join(RAIZ, 'datos', f'examen_v8_{stamp}.json')
        meta = dict(fecha=time.strftime('%Y-%m-%dT%H:%M:%S'), semillas=S, veredictos=V, congelar=bool(todo and S >= 20),
                    sha_organismo_v8=h16(os.path.join(AQUI, 'organismo_v8.py')), sha_bateria_v8=h16(os.path.abspath(__file__)),
                    sha_preregistro=h16(prereg), sha_organismo_v7=h16(os.path.join(AQUI, 'organismo_v7.py')),
                    sha_organismo_v7e=h16(os.path.join(RAIZ, 'experimentos', 'bug01', 'organismo_v7e.py')),
