"""Batería de v10 — examen de congelación (CRITERIO v3, sin cambiar un umbral) y, una vez congelado, regresión del tronco.
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv:
        _log['f'] = open(os.path.join(RAIZ, 'datos', f'examen_v10_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    prereg = os.path.join(RAIZ, 'experimentos', 'v10_direccion_division', 'PREREGISTRO_v10.md')
    log(f"=== Batería v10 — CRITERIO v3, {S} semillas, Pool({N_PARALELO}) ===")
    log(f"sha organismo_v10 {h16(os.path.join(AQUI, 'organismo_v10.py'))}  bateria_v10 {h16(os.path.abspath(__file__))}"
        f"  preregistro {h16(prereg)}")
    V = {}

    with mp.Pool(N_PARALELO) as pool:
        # ---------- 5. identidad ----------
        trabajos = [(c, e, s) for c in ('v9',) for e in ESC_ID for s in range(1, 7)]
        ...
        for c, nombre in (('v9', 'v10(mu_norm=False) == v9'),):
        ...
        if not V['5_identidad']:
            log("*** CRITERIO 5 FALLIDO: v10 no es el organismo que dice ser. No se corre el examen.")
            sys.exit(1)
    ...
    log("VEREDICTO bateria_v10: " + " ".join(f"{k}={V[k]}" for k in decisivos))
    if todo and S >= 20:
        log("*** v10 CUMPLE EL CRITERIO v3 con 20 semillas. Por el preregistro: CONGELAR v10 como tronco (si pasa tambien Q2-Q5).")
    elif todo:
        log(f"*** v10 cumple el criterio v3 con {S} semillas. Congelar sólo con S>=20.")
    else:
        log("*** v10 NO cumple el criterio v3. No se congela; v9 sigue siendo el tronco. No recalibrar (regla 3).")

    if _log['f']:
        dj = os.path.join(RAIZ, 'datos', f'examen_v10_{stamp}.json')
