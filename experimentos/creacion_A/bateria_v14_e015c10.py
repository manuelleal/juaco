"""bateria_v14_e015c10 = organismo/bateria_v14.py (72216f5415de0c86, CONGELADA: solo se leyo) apuntando a
organismo_v14_e015c10 (las dos constantes de A-4). Las SEIS etapas, los CRIT y los umbrales del criterio
v3' quedan INTACTOS. Salida en datos/examen_v14_e015c10_<fecha>.
Generado por experimentos/creacion_A/construye_v14_e015c10.py. NO editar. La bateria original NO se toco."""
...
_D14 = os.path.dirname(os.path.abspath(__file__))   # A-4: experimentos/creacion_A/
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv:
        _log['f'] = open(os.path.join(RAIZ, 'datos', f'examen_v14_e015c10_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    ...
    log(f"sha organismo_v13 {h16(os.path.join(_D14, 'organismo_v14_e015c10.py'))}  bateria_v13 {h16(os.path.abspath(__file__))}"
    ...
    if _log['f']:
        dj = os.path.join(RAIZ, 'datos', f'examen_v14_e015c10_{stamp}.json')
        meta = dict(fecha=time.strftime('%Y-%m-%dT%H:%M:%S'), semillas=S, semilla_inicial=seeds[0], veredictos=V, congelar=bool(todo and S >= 20),
                    sha_organismo_v13=h16(os.path.join(_D14, 'organismo_v14_e015c10.py')), sha_bateria_v13=h16(os.path.abspath(__file__)),
