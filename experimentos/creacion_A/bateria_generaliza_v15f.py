"""bateria_generaliza_v15f = organismo/bateria_generaliza.py (9cf72581ebae7dea, CONGELADA: solo se leyo) con UNA entrada
nueva: organismo_v15f_on -> organismo_v15gf_on, con los MISMOS campos que la entrada del tronco (regla 14 / ERR-38),
verificados campo a campo por el constructor. Umbrales G1/G2/K sin tocar. Generado por construye_v15f.py. NO editar."""
...
RAIZ = os.path.dirname(os.path.dirname(AQUI))   # v15f
...
INSTRUMENTOS = {
    ...
    'organismo_v15f_on': ('organismo_v15gf_on', dict(eta_s=0.15, clip_s=10.0, puerta=3, mask_rel=2, del_s=0.25, del_c=0.25, ema_c=0.05, puerta_pat=5, pat_shuf=0, pat_min=1)),   # v15f: los MISMOS campos que la entrada del tronco (regla 14 / ERR-38), explicitos
}
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv:
        _log['f'] = open(os.path.join(RAIZ, 'datos', f'regresion_generaliza_v15f_{modulo}_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    inst = INSTRUMENTOS[modulo][0]
    log(f"=== REGRESIÓN DE GENERALIZACIÓN (Etapa 3) — {modulo}, {S} semillas ({seeds[0]}..{seeds[-1]}), Pool({N_PARALELO}) ===")
    _dir = {'organismo_v14g': os.path.join(RAIZ, 'organismo'), 'organismo_v15gf_on': AQUI, 'organismo_v11g': GEN, 'organismo_v13g': os.path.join(RAIZ, 'experimentos', 'v13_dos_vias'),
    ...
    if _log['f']:
        dj = os.path.join(RAIZ, 'datos', f'regresion_generaliza_v15f_{modulo}_{stamp}.json')
