"""organismo_g3A = experimentos/enjambre/grupo3/organismo_g3.py (91eb167023cb37b7, del grupo 3; solo se leyo) + dos
perillas del CREADOR A, apagadas por defecto: `mem_apriori` (LECTURA de la memoria EN LA SONDA: errores,
ganadora, cuantas empatan) y `mem_desempate='azar'` (CONTROL DE RIGGING: el indice 0 de _PAR es (0,1)).
Con las dos apagadas es organismo_g3 EXACTO (identidad obligatoria: identidad_g3A.py).
Generado por experimentos/creacion_A/construye_g3A.py. NO editar a mano."""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
                    ...
                    if eta_s:   # v13: actualizacion de la via lenta (su tasa, mismo drenaje)
                        ...
                        if memoria is not None:   # M3: memoria de un golpe por combinacion (BTSP; Milstein 2024; OnlineHD): las 15 celdas escriben en cada mordida, ninguna tasa
                            for _c5m in range(15):
                                ...
                                _MN[_c5m,_dir5m]+=1
                            if mem_desempate=='azar':   # creacion_A: control de rigging (el indice 0 de _PAR es (0,1))
    ...
    return dict(mem_apriori=_memA,mem_desempate=mem_desempate,lab=lab,lenta_eventos=_ev5,mem_ganadora=list(_PAR[_MG]),mem_tabla=[[round(float(x),3) for x in f] for f in _MM],mem_vistas=int((_MN>0).sum()),fase2_en=fase2_en,seleccion=seleccion,sel_estad=sel_estad,sel_theta=sel_theta,sel_rho=sel_rho,sel_cupo=sel_cupo,
