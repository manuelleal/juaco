"""
organismo_v13q5 RAPIDO — gemelo COMPILADO (numba) de experimentos/creacion_A/organismo_v13q5.py (fae9c32b146fdbb4).
NO es un organismo nuevo: misma regla, mismo flujo de azar, mismas salidas (TODAS las claves, incluidas las sondas a
priori, el primer encuentro, la sonda final y la secuencia `lab`). Existe para correr ~40x mas rapido. Solo vale si es
BIT A BIT identico al original: arnes identidad_v13q5_rapido.py (regla 9 de registro/EQUIPO.md: si el arnes no da
100 %, este archivo no se usa para confirmar nada).

Origen y cadena de perillas (cada eslabon se leyo entero antes de compilar):
  organismo_v13q.py (0b59eb03858df3a8, bloque 3b)      gemelo: nivel7_xor_lectura/organismo_v13q_rapido.py (81/81)
    + construye_xor_3d.py  -> organismo_v13q3.py (aaebe073308a40c2): `regla_lenta` ('dos_canales' | 'delta_signo'),
      `constante` (phi' = phi + [1.0]), `lam_lenta` (decaimiento multiplicativo), lecturas 'oraculo01' y
      'oraculo01_ruido', la lectura `lenta()` segun regla, `Ws`, `Ws_apriori` en la sonda y las guardias de knob.
    + construye_v13q4.py   -> organismo_v13q4.py (3cc732dd2b2519cd): `seleccion='wta'` (cascade-correlation local:
      `sel_theta`, `sel_rho`, `sel_cupo`, `sel_calienta`, `sel_estad` 'cond'|'cov') y las claves sel_*.
    + construye_v13q5.py   -> organismo_v13q5.py (fae9c32b146fdbb4): perilla `lab` (la secuencia (t, patron, R,
      residuo) de CADA actualizacion de la via lenta) y las claves lab / lenta_eventos / fase2_en.

Decisiones del gemelo (las mismas de organismo_v13q_rapido.py, mas las nuevas de esta linea):
  - Generator: se crea en Python y se pasa el MISMO objeto rng al bucle compilado (uniform/normal/random/integers dan
    el mismo flujo). La inicializacion (Wl, KW, el re-sorteo de cond()) es linea por linea la del original.
  - Productos con @ (KW@P, Wb@kc, Wl@x, (Wps-Wns)@phi, Ws@phi): numba llama al mismo BLAS. Verificado para
    NF = 3, 4, 6, 7, 21 y 22 (las seis longitudes que produce la cadena lectura x constante).
  - NINGUNA suma de >= 8 elementos con .sum() dentro del bucle (NumPy suma por pares desde n=8): solo sumas de 6,
    conteos enteros y productos por BLAS.
  - argsort con EMPATES: si hay empate en la frontera del top-K se delega en NumPy (objmode); sin empate el conjunto
    top-K es unico. clip / outer / where / minimum / maximum: escritos como bucles elementales (misma aritmetica IEEE
    y el MISMO orden de operaciones que el original, incluida la asociatividad: eta_s*_ds*phi == (eta_s*_ds)*phi,
    pero sel_rho*(_ds*phi) NO se reasocia).
  - argmax de la competencia (`seleccion`): bucle con `>` estricto = np.argmax (primer maximo) sobre
    np.where(candidato_cerrado, |e|, -1.0).
  - Estructuras del original como arrays: `objs` (dict, ORDEN DE INSERCION) -> opos/otip + _borrar que desplaza;
    ...
    `_ev5` y `_abre4` (listas que crecen) -> vectores preasignados + contador, rearmados en Python con el mismo
    `round(...,4)` / `int()` / `float()` del original.
  ...
  - La SONDA de fase2_en tampoco se reimplementa: en t==fase2_en el bucle solo saca una FOTO de (Wp, Wn, Wps, Wns, Ws,
    KW, activa) y las cinco lecturas (W_apriori, W_lenta_apriori, Ws_apriori, familiar_apriori, codigos_f2) se calculan
    despues en Python con las lineas literales del original sobre esa foto. Asi la sonda usa el argsort de NumPy.
  - phi(P) es fija por patron: se precalcula PHIM (npat x NF) en Python con el phi del original (incluida la entrada
    constante). Con lectura='lineal' y constante=False PHIM == PATM.
  - `lab`: la secuencia se graba en el MISMO punto del original (justo despues de calcular `_ds`, antes de cualquier
    regla) y se rearma como (int(t), nombre_del_patron, float(R), float(_ds)).
...
def patrones_regla():
    """v9g: los C(6,3)=20 patrones binarios de 6 px con exactamente 3 px activos. (identico a organismo_v13q5)"""
    pats = {}
...
def _bucle(rng, T, learn, invertir_en, iA, iB, nuevo_idx, nuevo_en, nuevo_valc, f2_on, fase2_en, test_idx,
           ...
           rl_delta, lam_lenta, sel_on, sel_cov, sel_theta, sel_rho, sel_cupo, sel_calienta, cand4, ab4,
           lab, ev_t, ev_k, ev_R, ev_ds):
    ...
    Wps = np.zeros(NF); Wns = np.zeros(NF)                                  # xor: la via lenta vive sobre phi(P)
    Ws = np.zeros(NF)                                                       # 3d: vector con signo (solo si rl_delta)
    e4 = np.zeros(NF); nsel4 = 0                                            # A-1(ii): elegibilidad y contador de mordidas
    ab_t = np.zeros(NF + 1, np.int64); ab_j = np.zeros(NF + 1, np.int64)    # A-1(ii): historia de aperturas
    ab_e = np.zeros(NF + 1); ab_n = np.zeros(NF + 1, np.int64); nab = 0
    nev = 0                                                                 # v13q5: eventos de `lab`
    pos = 0; E = 1.0
    ...
    Wp_f2 = np.zeros(NKMAX); Wn_f2 = np.zeros(NKMAX); Wps_f2 = np.zeros(NF); Wns_f2 = np.zeros(NF)   # v13g/3b/3d: FOTO de la sonda
    Ws_f2 = np.zeros(NF)
    ...
    for t in range(T):
        ...
        if f2_on and t == fase2_en:                                         # v13g/3b/3d: foto para la sonda a priori y entrada de los de test
            for i in range(NKMAX):
                Wp_f2[i] = Wp[i]; Wn_f2[i] = Wn[i]; act_f2[i] = activa[i]
                for j in range(6): KW_f2[i, j] = KW[i, j]
            for i in range(NF):
                Wps_f2[i] = Wps[i]; Wns_f2[i] = Wns[i]; Ws_f2[i] = Ws[i]
        ...
        if io >= 0:
            kk = otip[io]; P = PATM[kk]; ph = PHIM[kk]                       # xor: phi(P_[kk]) precalculado (3d: con la constante ya dentro)
            kc = _code(KW, activa, P); Wb = Wp - Wn; _wf = Wb @ kc
            if rl_delta: _ws = Ws @ ph                                       # 3d: lenta(P) segun regla_lenta
            else:        _ws = (Wps - Wns) @ ph
            nfam = 0
            for i in range(NKMAX):
            ...
            if mordio:
                ...
                if learn:
                    ...
                    if eta_s != 0.0:
                        ...
                        if lab:                                              # v13q5: LECTURA pura (no cambia ningun numero)
                            ev_t[nev] = t; ev_k[nev] = kk; ev_R[nev] = R; ev_ds[nev] = _ds; nev += 1
                        if sel_on:                                           # A-1(ii): elegibilidad, competencia, y la via lenta aprende SOLO lo abierto
                            nsel4 += 1
                            if not sel_cov:                                  # 'cond': media movil del residuo BAJO el rasgo
                                for i in range(NF):
                                    if ph[i] > 0: e4[i] = (1 - sel_rho) * e4[i] + sel_rho * _ds
                            else:                                            # 'cov': correlacion acumulada (cascade-correlation)
                                for i in range(NF):
                                    e4[i] = (1 - sel_rho) * e4[i] + sel_rho * (_ds * ph[i])
                            if nsel4 >= sel_calienta:
                                nab_ac = 0
                                for i in range(NF):
                                    if ab4[i] and cand4[i]: nab_ac += 1
                                if nab_ac < sel_cupo:
                                    hay = False
                                    for i in range(NF):
                                        if cand4[i] and not ab4[i]:
                                            hay = True; break
                                    if hay:                                  # argmax(np.where(cand&~ab, |e|, -1.0)): primer maximo
                                        j4 = 0
                                        vbest = abs(e4[0]) if (cand4[0] and not ab4[0]) else -1.0
                                        for i in range(1, NF):
                                            vi = abs(e4[i]) if (cand4[i] and not ab4[i]) else -1.0
                                            if vi > vbest:
                                                vbest = vi; j4 = i
                                        if abs(e4[j4]) > sel_theta:
                                            ab4[j4] = True
                                            ab_t[nab] = t; ab_j[nab] = j4; ab_e[nab] = e4[j4]; ab_n[nab] = nsel4; nab += 1
                            if rl_delta:                                     # _phm4 = phi * ab4 (los cerrados quedan en 0)
                                for i in range(NF):
                                    _pm = ph[i] if ab4[i] else 0.0
                                    Ws[i] = min(max(Ws[i] * (1 - lam_lenta) + eta_s * _ds * _pm, -clip_s), clip_s)
                            else:
                                if lam != 0.0:
                                    for i in range(NF):
                                        _pm = ph[i] if ab4[i] else 0.0
                                ...
                                if _ds > 0:
                                    for i in range(NF):
                                        _pm = ph[i] if ab4[i] else 0.0
                                        Wps[i] = min(max(Wps[i] + eta_s * _ds * _pm, 0.0), clip_s)
                                else:
                                    for i in range(NF):
                                        _pm = ph[i] if ab4[i] else 0.0
                                        Wns[i] = min(max(Wns[i] + eta_s * aversion * (-_ds) * _pm, 0.0), clip_s)
                        elif rl_delta:                                       # 3d: UN vector con signo, decaimiento multiplicativo, tope simetrico
                            for i in range(NF):
                                Ws[i] = min(max(Ws[i] * (1 - lam_lenta) + eta_s * _ds * ph[i], -clip_s), clip_s)
                        else:                                                # 'dos_canales': la regla original de v13/v13q, sin tocar
                            if lam != 0.0:
                                for i in range(NF):
                                    if ph[i] > 0:
                                        _mcs = min(Wps[i], Wns[i]); Wps[i] = Wps[i] - lam * _mcs; Wns[i] = Wns[i] - lam * _mcs
    ...
    return (Wp, Wn, Wps, Wns, Ws, KW, activa, splits, st_t[:nst], st_k[:nst], mord, vis, sobre, llegadas, sin_objetivo, deaths,
            err_max, t_conflicto, t_techo, n_techo, Wp_f2, Wn_f2, Wps_f2, Wns_f2, Ws_f2, KW_f2, act_f2, f2_ok,
            pr_set, pr_t, pr_W, pr_pb, pr_h, pr_m,
            e4, ab4, ab_t[:nab], ab_j[:nab], ab_e[:nab], ab_n[:nab], nsel4, nev)


def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.05,memoria_rechazo=20,mu_norm=True,div_signo=True,eta_s=0.0,clip_s=3.0,puerta=None,mundo='AB',regla='px0',fase2_en=None,sonda_final=False,lectura='lineal',regla_lenta='dos_canales',constante=False,lam_lenta=0.002,seleccion=None,sel_theta=0.6,sel_rho=0.05,sel_cupo=1,sel_calienta=0,sel_estad='cond',lab=False):
    if log_cada:
        raise ValueError("organismo_v13q5_rapido: log_cada no esta compilado; usa organismo_v13q5 para eso")
    ...
    return dict(lab=lab,lenta_eventos=_ev5,fase2_en=fase2_en,seleccion=seleccion,sel_estad=sel_estad,sel_theta=sel_theta,sel_rho=sel_rho,sel_cupo=sel_cupo,
