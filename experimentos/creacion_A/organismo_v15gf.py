"""organismo_v15gf = organismo/organismo_v14g.py (1f1318480cd34cde, solo se leyo) + perilla `memoria_pares` (v15f).
Con None es organismo_v14g EXACTO. Generado por construye_v15f.py. NO editar."""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
    ...
    for t in range(T):
        ...
        R=0.
        if pos in objs:
            kk=objs[pos]; kc=kenyon(P_[kk]); Wb=Wp-Wn; _wf=float(Wb@kc); _ws=_lenta_v15f(P_[kk])   # v13: las dos vias (v15f: tabla si conoce la combinacion, si no la lineal)
            ...
            if mordio:
                ...
                if learn:
                    ...
                    if eta_s:   # v13: actualizacion de la via lenta (su tasa, mismo drenaje)
                        _lbv=_lin_v15f(P_[kk]); _ds=dlt if puerta is None else R-_lbv   # v15f: la lineal aprende de SU error (apagada: _lbv == _ws)
