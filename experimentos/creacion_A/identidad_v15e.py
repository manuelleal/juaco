"""CREADOR A — arnes de identidad del candidato v15e (tabla de pares REESCRIBIBLE, sin doble cuenta). Un proceso, sin Pool.

I1  `organismo_v15e(memoria_pares=None)` == `organismo/organismo_v14.py` (TRONCO v14.1) en TODAS las claves de v14.1.
    12 escenarios x 2 semillas = 24 comprobaciones.
I2  El `rng` NO se consume con la perilla apagada (T = 120 000, 2 semillas: un numero de mas y todo diverge).
I3  `organismo_v15ge(memoria_pares=None)` == `organismo/organismo_v14g.py` (mundo de regla) con los kwargs EXACTOS de la
    entrada 'organismo_v14' de bateria_generaliza (regla 14 / ERR-38), 3 reglas x 2 semillas = 6.
I4  NO es identidad: la perilla ENCENDIDA corre sin excepcion con `puerta_pat=5` (la colision `_ev` de v15c) y muestra
    la propiedad que v15e reclama: tras una mordida de B la via lenta lee EXACTAMENTE −3 (lineal + residuo), no −4.35.
Uso: python identidad_v15e.py [T]
"""
import sys, os, json
import numpy as np
...
import organismo_v15e as V15E
import organismo_v14g as V14G
import organismo_v15ge as V15GE
...
if __name__ == '__main__':
    ...
    ok = tot = 0
    print(f'--- I1: organismo_v15e(memoria_pares=None) == organismo_v14 (TRONCO v14.1), T={T} ---', flush=True)
    for etq, kw in ESC:
        for s in (1, 2):
            tot += 1
            d = comp(V14.run(s, T=T, **kw), V15E.run(s, T=T, memoria_pares=None, **kw))
            ok += (not d)
    ...
    for s in (1, 2):
        tot += 1
        d = comp(V14.run(s, T=120000), V15E.run(s, T=120000, memoria_pares=None))
        ok += (not d)
        print(f'  T=120000 s{s}: {"IDENTICO" if not d else "DIFIERE " + str(d)}', flush=True)
    print(f'\n--- I3: organismo_v15ge(memoria_pares=None) == organismo_v14g (kwargs exactos del tronco), T={T} ---', flush=True)
    for etq, kw in ESCG:
        for s in (1, 2):
            tot += 1
            d = comp(V14G.run(s, T=T, **kw), V15GE.run(s, T=T, memoria_pares=None, **kw))
            ok += (not d)
            print(f'  {etq:>40} s{s}: {"IDENTICO" if not d else "DIFIERE " + str(d)}', flush=True)
    print(f'\nIDENTIDAD (I1+I2+I3): {ok}/{tot}', flush=True)

    print('\n--- I4 (NO es identidad): la perilla ENCENDIDA corre con puerta_pat=5 y la via lenta lee EXACTO tras una mordida ---', flush=True)
    for s in (1, 2):
        r = V15E.run(s, T=min(T, 20000), memoria_pares='suma')
        print(f'  s{s} T={min(T,20000)}: W {r["W"]}  W_lenta(lineal) {r["W_lenta"]}  W_tabla(residuo) {r["W_tabla"]}  '
              f'ganadora {r["mem_ganadora"]}  familiar {r["mem_fam"]}  mordidas B {r["mord"]["B"]}  splits {r["splits"]}', flush=True)
    print('  Lo que se busca: sin excepcion, y lineal + residuo = ±R exacto en cada patron mordido (la suma no dobla la cuenta).')
