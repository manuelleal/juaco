"""bateria_generaliza_v15d = organismo/bateria_generaliza.py (9cf72581ebae7dea, CONGELADA: solo se leyo) con UNA entrada
nueva: organismo_v15d_on -> organismo_v15gd_on. Umbrales G1/G2/K sin tocar.
...
RAIZ = os.path.dirname(os.path.dirname(AQUI))   # v15d
...
INSTRUMENTOS = {
    ...
    'organismo_v15d_on': ('organismo_v15gd_on', dict(eta_s=0.15, clip_s=10.0, puerta=3, mask_rel=2, del_s=0.25, del_c=0.25, ema_c=0.05, puerta_pat=5, pat_shuf=0, pat_min=1)),   # v15d: lineal + tabla de pares (suma)
}
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv:
        _log['f'] = open(os.path.join(RAIZ, 'datos', f'regresion_generaliza_v15d_{modulo}_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    inst = INSTRUMENTOS[modulo][0]
    log(f"=== REGRESIÓN DE GENERALIZACIÓN (Etapa 3) — {modulo}, {S} semillas ({seeds[0]}..{seeds[-1]}), Pool({N_PARALELO}) ===")
    _dir = {'organismo_v14g': os.path.join(RAIZ, 'organismo'), 'organismo_v15gd_on': AQUI, 'organismo_v11g': GEN, 'organismo_v13g': os.path.join(RAIZ, 'experimentos', 'v13_dos_vias'),
