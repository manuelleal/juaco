"""organismo_vivo_relevo = experimentos/nivel11_mundo_vivo/organismo_vivo_rep2.py (96feb4918dc5d694; solo se leyo; cadena organismo_vivo
20c0961c79de8825 <- organismo_v14 feefc88b1fd8d434) + memoria de pares con RELEVO (v15f) por necesidad, brazo exploratorio `relevo_boca` y la
inversion del mundo vivo `invertir_vivo_en`. Con memoria_pares=None es organismo_vivo_rep2 EXACTO (y organismo_vivo con
reproduccion=0; organismo_v14 con vivo=0, n_nec=1) sin consumir rng. Generado por construye_vivo_relevo.py. NO editar."""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
    ...
    def valor(P):   # v13: el valor que usa la boca. Sin puerta: rapida+lenta (un error). Con puerta: la rapida si el patron le es FAMILIAR, si no la lenta
        _k=kenyon(P); _f=float((Wp[_nm]-Wn[_nm])@_k); _s=_lenta_v15(_nm,P)
    ...
    def _vnec(_n,P,_k):   # VIVO: el valor que la boca usaria para la necesidad _n (mismo ruteo que valor()). SOLO LECTURA
        _f=float((Wp[_n]-Wn[_n])@_k); _s=_lenta_v15(_n,P)
    ...
    for t in range(T):
        ...
        R=0.
        if pos in objs:
            kk=objs[pos]; kc=kenyon(PAT[kk]); Wb=Wp[_nm]-Wn[_nm]; _wf=float(Wb@kc); _ws=_lenta_v15(_nm,PAT[kk])   # v13: las dos vias (VIVO: las de la necesidad activa; RELEVO: la casilla si la conoce, si no la lineal)
            ...
            if memoria_pares is not None and relevo_boca and _tabla_v15(_nm,PAT[kk])[1]: _wt=_ws   # v15g (brazo exploratorio): la casilla vista manda antes que la puerta
            ...
            if mordio:
                ...
                if learn:
                    ...
                    if eta_s:   # v13: actualizacion de la via lenta (su tasa, mismo drenaje)
                        ...
                        if memoria_pares is not None: _escribe_v15(_nm,PAT[kk],R)   # RELEVO: la tabla de la necesidad activa escribe R crudo
                    ...
                    if n_nec>1:   # VIVO: las OTRAS necesidades aprenden de SU componente del MISMO bocado (esto es lo que compra el mundo vivo)
                        for _n in range(n_nec):
                            if _n==_nm: continue
                            _Rn=_Rv[_n]; _wfn=float((Wp[_n]-Wn[_n])@kc); _wsn=_lenta_v15(_n,PAT[kk])
                            ...
                            if eta_s:
                                ...
                                if memoria_pares is not None: _escribe_v15(_n,PAT[kk],_Rn)   # RELEVO: la tabla de la otra necesidad escribe SU componente
    ...
    return dict(memoria_pares=memoria_pares,relevo_boca=relevo_boca,invertir_vivo_en=invertir_vivo_en,mem_ganadora=([list(_PARv[_g]) for _g in _MGv] if memoria_pares is not None else None),W_tabla=([{k:(round(_tabla_v15(_n,PAT[k])[0],3) if _tabla_v15(_n,PAT[k])[1] else None) for k in PAT} for _n in range(n_nec)] if memoria_pares is not None else None),mem_vistas=(int((_MNv>0).sum()) if memoria_pares is not None else None),sobre=sobre,llegadas=llegadas,sin_objetivo=sin_objetivo,memoria_rechazo=memoria_rechazo,err_max=err_max,t_conflicto=t_conflicto,t_techo=t_techo,n_techo=n_techo,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),puerta_pat=puerta_pat,pat_shuf=pat_shuf,pat_min=pat_min,n_cod=len(ncod),
