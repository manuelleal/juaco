"""organismo_v15ge = organismo/organismo_v14g.py (1f1318480cd34cde, solo se leyo) + perilla `memoria_pares` (v15e).
Con None es organismo_v14g EXACTO. Generado por construye_v15e.py. NO editar."""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.05,memoria_rechazo=20,mu_norm=True,div_signo=True,eta_s=0.0,clip_s=3.0,puerta=None,mundo='AB',regla='px0',fase2_en=None,sonda_final=False,mask_rel=0,del_s=0.25,del_c=0.25,ema_c=0.05,puerta_pat=0,pat_shuf=0,pat_min=0,memoria_pares=None,mem_alfa=1.0,mem_rho=0.02):
    ...
    for t in range(T):
        ...
        R=0.
        if pos in objs:
            kk=objs[pos]; kc=kenyon(P_[kk]); Wb=Wp-Wn; _wf=float(Wb@kc); _ws=_lenta_v15e(P_[kk])   # v13: las dos vias (v15e: lineal + residuo de la tabla si la perilla esta encendida)
            ...
            if mordio:
                ...
                if learn:
                    ...
                    if eta_s:   # v13: actualizacion de la via lenta (su tasa, mismo drenaje)
                        _lbv=_lin_v15e(P_[kk]); _ds=dlt if puerta is None else R-_lbv   # v15e: la lineal aprende de SU error (apagada: _lbv == _ws, la misma expresion)
                        ...
                        if memoria_pares is not None:   # v15e: la tabla escribe TRAS el paso de la lineal, con el residuo que deja: la suma queda EXACTA en R
                            _Pv=P_[kk]; _lav=float((Wps-Wns)@_Pv); _tv=R-_lav   # lectura lineal tras su paso; el residuo sigue a la ULTIMA recompensa
                            for _cv in range(15):
