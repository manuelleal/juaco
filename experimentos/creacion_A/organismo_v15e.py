"""organismo_v15e = organismo/organismo_v14.py (feefc88b1fd8d434, TRONCO v14.1 CONGELADO: solo se leyo) + perilla
`memoria_pares` = None | 'suma' (mem_alfa=1.0): la via lenta lee lineal + tabla de pares REESCRIBIBLE; la lineal
aprende de su error, la tabla guarda tras cada mordida el residuo R - lineal (sobrescritura): la suma queda exacta en R
y se desdice en una mordida. Con None es organismo_v14 EXACTO y no se consume ni un numero del rng (identidad_v15e.py).
Generado por construye_v15e.py. NO editar a mano."""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.05,memoria_rechazo=20,mu_norm=True,div_signo=True,eta_s=0.15,clip_s=10.0,puerta=3,mask_rel=2,del_s=0.25,del_c=0.25,ema_c=0.05,puerta_pat=5,pat_shuf=0,pat_min=1,memoria_pares=None,mem_alfa=1.0,mem_rho=0.02):
    ...
    def valor(P):   # v13: el valor que usa la boca. Sin puerta: rapida+lenta (un error). Con puerta: la rapida si el patron le es FAMILIAR, si no la lenta
        _k=kenyon(P); _f=float((Wp-Wn)@_k); _s=_lenta_v15e(P)
    ...
    for t in range(T):
        ...
        R=0.
        if pos in objs:
            kk=objs[pos]; kc=kenyon(PAT[kk]); Wb=Wp-Wn; _wf=float(Wb@kc); _ws=_lenta_v15e(PAT[kk])   # v13: las dos vias (v15e: lineal + residuo de la tabla si la perilla esta encendida)
            ...
            if mordio:
                ...
                if learn:
                    ...
                    if eta_s:   # v13: actualizacion de la via lenta (su tasa, mismo drenaje)
                        _lbv=_lin_v15e(PAT[kk]); _ds=dlt if puerta is None else R-_lbv   # v15e: la lineal aprende de SU error (apagada: _lbv == _ws, la misma expresion)
                        ...
                        if memoria_pares is not None:   # v15e: la tabla escribe TRAS el paso de la lineal, con el residuo que deja: la suma queda EXACTA en R
                            _Pv=PAT[kk]; _lav=float((Wps-Wns)@_Pv); _tv=R-_lav   # lectura lineal tras su paso; el residuo sigue a la ULTIMA recompensa
                            for _cv in range(15):
