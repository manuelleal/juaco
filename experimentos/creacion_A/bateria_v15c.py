"""bateria_v15c = organismo/bateria_v14.py (72216f5415de0c86, CONGELADA: solo se leyo) sobre organismo_v15c con la
perilla APAGADA (criterio 5 del encargo). Las SEIS etapas y los umbrales del criterio v3' INTACTOS.
Generado por construye_v15c.py. NO editar."""
...
RAIZ = os.path.dirname(os.path.dirname(AQUI))   # v15c: experimentos/creacion_A/ -> bundle/
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv:
        _log['f'] = open(os.path.join(RAIZ, 'datos', f'examen_v15c_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    ...
    log(f"sha organismo_v13 {h16(os.path.join(_D14, 'organismo_v15c.py'))}  bateria_v13 {h16(os.path.abspath(__file__))}"
    ...
    if _log['f']:
        dj = os.path.join(RAIZ, 'datos', f'examen_v15c_{stamp}.json')
        meta = dict(fecha=time.strftime('%Y-%m-%dT%H:%M:%S'), semillas=S, semilla_inicial=seeds[0], veredictos=V, congelar=bool(todo and S >= 20),
                    sha_organismo_v13=h16(os.path.join(_D14, 'organismo_v15c.py')), sha_bateria_v13=h16(os.path.abspath(__file__)),
