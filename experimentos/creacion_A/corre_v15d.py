"""Candidato v15d: la memoria de un golpe por pares en la VIA LENTA DEL TRONCO. Ejecuta PREREGISTRO_v15d.md.
REGLA 10: log desde el arranque. REGLA 11: lista los python vivos antes del Pool.

ETAPA 1  identidad interna (`identidad_v15d.py` como SUBPROCESO): I1 v15d(None) == v14.1, I2 el rng no se consume,
         I3 v15gd(None) == v14g, I4 la ganadora coincide con organismo_g3A (la equivalencia exacta NO se reclama).
ETAPA 2  V1 examen del criterio v3' (`bateria_v15d.py 20 --desde 101 --log`, SUBPROCESO; trae su propio Pool).
ETAPA 3  V2a generalizacion (`bateria_generaliza_v15d.py organismo_v15d_on 20 --desde 101 --log`, SUBPROCESO).
...
Uso:  python experimentos/creacion_A/corre_v15d.py [--desde N]
      python experimentos/creacion_A/corre_v15d.py --humo   (UN proceso, sin Pool: identidad corta + 1 semilla)
"""
...
MODOS = ['suma', 'ruta', None]   # v15d: los dos modos honestos y el apagado, pareados
N_PARALELO = 14
...
if __name__ == '__main__':
    ...
    nom = f"v15d_humo_{stamp}" if HUMO else f"v15d_s{SEEDS[0]}-{SEEDS[-1]}_{stamp}"
    _log['f'] = open(os.path.join(RAIZ, 'datos', nom + '.log'), 'w', encoding='utf-8', newline='\n')
    pre = os.path.join(AQUI, 'PREREGISTRO_v15d.md')
    log(f"ARRANQUE v15d ({'HUMO, un proceso, sin Pool' if HUMO else f'semillas {SEEDS[0]}-{SEEDS[-1]}'}).")
    log(f"sha preregistro {h16(pre) if os.path.exists(pre) else '(falta)'}  script {h16(os.path.abspath(__file__))}"
        f"  construye_v15d {h16(os.path.join(AQUI,'construye_v15d.py'))}"
        f"  organismo_v15d {h16(os.path.join(AQUI,'organismo_v15d.py'))}"
        f"  organismo_v15d_on {h16(os.path.join(AQUI,'organismo_v15d_on.py'))}"
        f"  organismo_v15gd {h16(os.path.join(AQUI,'organismo_v15gd.py'))}"
        f"  organismo_v15gd_on {h16(os.path.join(AQUI,'organismo_v15gd_on.py'))}"
        f"  bateria_v15d {h16(os.path.join(AQUI,'bateria_v15d.py'))}"
        f"  bateria_generaliza_v15d {h16(os.path.join(AQUI,'bateria_generaliza_v15d.py'))}"
    ...
    if HUMO:
        ...
        r1 = sub([sys.executable, os.path.join(AQUI, 'identidad_v15d.py'), '20000'], 'identidad')
        ...
        log("   python experimentos/creacion_A/bateria_v15d.py 20 --desde 101 --log")
        log("   python experimentos/creacion_A/bateria_generaliza_v15d.py organismo_v15d_on 20 --desde 101 --log")
    ...
    r1 = sub([sys.executable, os.path.join(AQUI, 'identidad_v15d.py'), '30000'], 'identidad')
    ...
    r2 = sub([sys.executable, os.path.join(AQUI, 'bateria_v15d.py'), '20', '--desde', '101', '--log'], 'V1 examen')
    log("ETAPA 3/5 — V2a: generalizacion con la memoria ENCENDIDA (subproceso, trae su Pool).")
    r3 = sub([sys.executable, os.path.join(AQUI, 'bateria_generaliza_v15d.py'),
              'organismo_v15d_on', '20', '--desde', '101', '--log'], 'V2a generalizacion')
    ...
    p0 = med([r['acc'] for r in G('px0', 'suma')])[0]
    az = med([r['acc'] for r in G('azar', 'suma')])[0]
    c14 = med([r['celdas'] for r in G('xor01', None)])[0]; c15 = med([r['celdas'] for r in G('xor01', 'suma')])[0]
    ...
    log(f"   V4 (lo medible) celdas v14.1 {f3(c14,1)} vs v15d {f3(c15,1)} (±10 %)                   {'OK' if V['V4'] else 'NO'}")
    ...
    meta = dict(fecha=time.strftime('%Y-%m-%dT%H:%M:%S'), semillas=SEEDS, reglas=REGLAS, veredictos=V,
                ...
                sha_v15d=h16(os.path.join(AQUI, 'organismo_v15d.py')),
                sha_v15gd=h16(os.path.join(AQUI, 'organismo_v15gd.py')),
