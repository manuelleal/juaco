"""mundo_largo_A = mundo_largo.py (9f74ff6b5941e5a5) + perillas `beta_m` (metaplasticidad por masa de conflicto) y
`m_stats` (lectura). Generado por experimentos/creacion_A/construye_largo_A.py. NO editar a mano.
Con beta_m=None y m_stats=False es mundo_largo EXACTO (identidad obligatoria)."""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        ...
        if pos in objs:
            ...
            if mordio:
                ...
                if learn:
                    ...
                    _gm=1.0 if beta_m is None else 1.0/(1.0+beta_m*np.minimum(Wp,Wn))   # creacion_A: g_c = 1/(1+beta_m*m_c); con beta_m=None es 1.0 exacto (identidad)
    ...
    return dict(m_stats=_mst,beta_m=beta_m,sobre=sobre,llegadas=llegadas,sin_objetivo=sin_objetivo,memoria_rechazo=memoria_rechazo,err_max=err_max,t_conflicto=t_conflicto,t_techo=t_techo,n_techo=n_techo,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),
