"""bateria_generaliza_A = organismo/bateria_generaliza.py (9d65c18e0237da97, CONGELADA: solo se leyo) con UNA
entrada nueva en INSTRUMENTOS para el brazo A-4 (organismo_v14_e015c10 -> organismo_v14g con eta_s=0.15,
clip_s=10.0). Umbrales G1/G2/K sin tocar. El instrumento del mundo de regla NO se duplica.
Generado por experimentos/creacion_A/construye_v14_e015c10.py. NO editar."""
...
INSTRUMENTOS = {
    ...
    'organismo_v14_e015c10': ('organismo_v14g', dict(eta_s=0.15, clip_s=10.0, puerta=3, mask_rel=2, del_s=0.25, del_c=0.25, ema_c=0.05, puerta_pat=5, pat_shuf=0, pat_min=1)),   # A-4: v14 con las dos constantes de la via lenta
}
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv:
        _log['f'] = open(os.path.join(RAIZ, 'datos', f'regresion_generaliza_A_{modulo}_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    inst = INSTRUMENTOS[modulo][0]
    log(f"=== REGRESIÓN DE GENERALIZACIÓN (Etapa 3) — {modulo}, {S} semillas ({seeds[0]}..{seeds[-1]}), Pool({N_PARALELO}) ===")
    _dir = {'organismo_v14g': os.path.join(RAIZ, 'organismo'), 'organismo_v11g': GEN, 'organismo_v13g': os.path.join(RAIZ, 'experimentos', 'v13_dos_vias'),
