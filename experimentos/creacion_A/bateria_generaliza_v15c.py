"""bateria_generaliza_v15c = organismo/bateria_generaliza.py (9cf72581ebae7dea, CONGELADA: solo se leyo) con UNA entrada
nueva en INSTRUMENTOS: organismo_v15c_on -> organismo_v15gc_on (la memoria de pares ENCENDIDA).
Umbrales G1/G2/K sin tocar. Generado por construye_v15c.py. NO editar."""
...
RAIZ = os.path.dirname(os.path.dirname(AQUI))   # v15c
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv:
        _log['f'] = open(os.path.join(RAIZ, 'datos', f'regresion_generaliza_v15c_{modulo}_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    inst = INSTRUMENTOS[modulo][0]
    log(f"=== REGRESIÓN DE GENERALIZACIÓN (Etapa 3) — {modulo}, {S} semillas ({seeds[0]}..{seeds[-1]}), Pool({N_PARALELO}) ===")
    _dir = {'organismo_v14g': os.path.join(RAIZ, 'organismo'), 'organismo_v15gc_on': AQUI, 'organismo_v11g': GEN, 'organismo_v13g': os.path.join(RAIZ, 'experimentos', 'v13_dos_vias'),
