"""bateria_v15f = organismo/bateria_v14.py (72216f5415de0c86, CONGELADA: solo se leyo) sobre **organismo_v15f_on** (la perilla
ENCENDIDA). Las SEIS etapas y los umbrales del criterio v3' INTACTOS. Unica diferencia de instrumento: el sha de
organismo_v11/v10 del meta se lee de organismo/ (como en bateria_v15e). Generado por construye_v15f.py. NO editar."""
...
RAIZ = os.path.dirname(os.path.dirname(AQUI))   # v15f: experimentos/creacion_A/ -> bundle/
...
def tarea_id(args):
    cual, esc, seed = args
    import organismo_v15f_on as v13   # v15f: la perilla ENCENDIDA (el examen mide AL CANDIDATO)
    if cual == 'v11':
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv:
        _log['f'] = open(os.path.join(RAIZ, 'datos', f'examen_v15f_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    ...
    log(f"sha organismo_v13 {h16(os.path.join(_D14, 'organismo_v15f_on.py'))}  bateria_v13 {h16(os.path.abspath(__file__))}"
    ...
    if _log['f']:
        dj = os.path.join(RAIZ, 'datos', f'examen_v15f_{stamp}.json')
        meta = dict(fecha=time.strftime('%Y-%m-%dT%H:%M:%S'), semillas=S, semilla_inicial=seeds[0], veredictos=V, congelar=bool(todo and S >= 20),
                    sha_organismo_v13=h16(os.path.join(_D14, 'organismo_v15f_on.py')), sha_bateria_v13=h16(os.path.abspath(__file__)),
