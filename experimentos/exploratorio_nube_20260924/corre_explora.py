...
def _brazos():
    ...
    for extra in ('carros_extra', 'carros_predice', 'carros_ver', 'carros_apr_reserva'):
...
def identidad(log):
    ...
    pares = [('W0', 'FAB'), ('W2', 'FAB'), ('WP', 'FAB'), ('WV', 'FAB'), ('WA', 'APR'), ('RESP', 'RES')]
...
def main(argv=None):
    ...
    log(f"EXPLORATORIO — no es dato · {et} · python {sys.version.split()[0]} · corre_explora.py {h16(os.path.abspath(__file__))} · "
        f"carros_reserva.py {h16(os.path.join(AQUI, 'carros_reserva.py'))} · pista.py {h16(P.__file__)} · juez.py {h16(J.__file__)}")
    if a.identidad:
