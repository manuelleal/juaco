"""BLOQUE 4 -- UN CANAL CON REFERENCIA POR SENALAMIENTO, SIMETRICO, entre dos v15f en el mismo mundo de familias.
Ejecuta PREREGISTRO_bloque4_canal.md (su sha va en el meta del JSON).
...
import numpy as np
import corre_familias_b3 as B3R       # bloque 3: v15f en el mundo de familias
B2R = B3R.B2R                          # bloque 2: el mundo, `resuelve`, la letra
...
SHA_B3_ESPERADO = '62a1e53b452b078e'
...
for _d, _sg in (('pos', '+'), ('neg', '-')):
    for _n, _m, _p, _o in PLANTILLA:
        BRAZOS[_n + _sg] = dict(dir=_d, modo=_m, pat=_p, otro=_o)
        ORDEN.append(_n + _sg)
BRAZOS_ACTIVOS = list(ORDEN)

CASOS_ID = {   # subconjunto critico del arnes de identidad_familias_b4.py (92/92)
    'a': ("(a) canal=None == organismo_familias_b3 (mundo=AB)", 'B3', T_ID),
    'g': ("(g) canal=None == organismo_familias_b3 (el mundo del bloque 4)", 'B3F', T_ID),
    'n': ("(n) canal=None + mundo=AB == organismo_v14 (TRONCO)", 'V14', T_ID),
...
def emisor(seed, Ti, nk=30, ktop=3, nkmax=90):
    ...
    r = B4.run(seed, T=Ti, fam_seed=seed, canal={'modo': 'emite'}, **kw)
    ...
    out['cod0'] = dict(r['cod0']); out['seed'] = seed; out['deaths'] = r['deaths']
    return out
...
def tarea(args):
    ...
    if tipo == 'ID':
        ...
        import organismo_familias_b3 as B3, organismo_familias_b4 as B4
        etiq, ref, Tc = CASOS_ID[cual]
        Tc = Tc or Ti
        salta, debe = tuple(NUEVAS_B4), False
        if ref == 'B3':
    ...
    r = B4.run(seed + SEM_R, T=Ti, fam_seed=fsr, canal=canal_de(brazo, msg, fsr), **kw)
    return resumen(brazo, seed, r, msg)


SHAS = lambda: dict(
    preregistro=h16(os.path.join(AQUI, 'PREREGISTRO_bloque4_canal.md')),
    script=h16(os.path.abspath(__file__)),
    constructor=h16(os.path.join(AQUI, 'construye_familias_b4.py')),
    instrumento=h16(os.path.join(AQUI, 'organismo_familias_b4.py')),
    arnes=h16(os.path.join(AQUI, 'identidad_familias_b4.py')),
    origen_b3=h16(os.path.join(AQUI, 'organismo_familias_b3.py')),
    runner_bloque3=h16(os.path.join(AQUI, 'corre_familias_b3.py')),
...
def guarda_origen():
    s, ok = SHAS(), True
    for k, esp in [('origen_organismo_v14', SHA_V14_ESPERADO), ('origen_b3', SHA_B3_ESPERADO),
...
def veredicto(res, msgs, SEEDS, diags):
    ...
    for b in BRAZOS_ACTIVOS:
        sg = b[-1]; d = BRAZOS[b]['dir']; cor = 'CORTADO' + sg
        if b == cor:
            continue
...
def humo(nk, ktop, nkmax, Tb, brazos):
    ...
    CF._log['f'] = open(os.path.join(RAIZ, 'datos', f'familias_b4_humo_{stamp}.log'), 'w',
                        encoding='utf-8', newline='\n')
    CF._log['t0'] = time.time()
    log("HUMO del disenador del bloque 4, UN proceso, sin Pool (regla 3, <= 6 corridas).")
    log("PREREGISTRO_bloque4_canal.md + PROTOCOLO_canal_20260918.md (sala 3).")
    log("Semillas 1-2: NINGUNA de las 541-580 del bloque queda expuesta.")
    for k, v in SHAS().items():
        log(f"    sha {k:26s} {v}")
    log(f"    origenes verificados -> {'OK' if guarda_origen() else 'FALLA'}")
    sem = [1, 2]

    log("1/4 IDENTIDAD (subconjunto; el arnes completo, 92/92, va en identidad_familias_b4.py).")
    ident = []
    ...
    dj = os.path.join(RAIZ, 'datos', f'familias_b4_humo_{stamp}.json')
...
if __name__ == '__main__':
    ...
    if '--brazos' in sys.argv:
        ...
        for b in list(BRAZOS_ACTIVOS):
            if 'CORTADO' + b[-1] not in BRAZOS_ACTIVOS:
                raise SystemExit(f"--brazos: CORTADO{b[-1]} es obligatorio (es el GEMELO de esa direccion).")
    if '--humo' in sys.argv:
    ...
    stamp = time.strftime('%Y%m%d_%H%M%S')
    CF._log['f'] = open(os.path.join(RAIZ, 'datos', f'familias_b4_s{SEEDS[0]}-{SEEDS[-1]}_{stamp}.log'),
                        'w', encoding='utf-8', newline='\n')
    CF._log['t0'] = time.time()
    log(f"ARRANQUE BLOQUE 4 (canal con referencia por senalamiento, SIMETRICO): brazos {BRAZOS_ACTIVOS}, "
        f"semillas {SEEDS[0]}-{SEEDS[-1]}, T={T}. Pool({N_PARALELO}).")
    log("El mundo es el del bloque 2/3 + exc_fija (que NO toca su rng). Lo unico que cambia entre los brazos de "
        "una direccion es el `canal`.")
    for k, v in SHAS().items():
        log(f"    sha {k:26s} {v}")
    ...
    dj = os.path.join(RAIZ, 'datos', f'familias_b4_s{SEEDS[0]}-{SEEDS[-1]}_{stamp}.json')
    with open(dj, 'w', encoding='utf-8') as f:
