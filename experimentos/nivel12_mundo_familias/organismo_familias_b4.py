...
def _familias(seed,D,n_var,F,V,n_exc,fam_val,n_neu,exc_evita=-1,vira=0,exc_fija=None):
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
    ...
    _FA=_familias((seed if fam_seed is None else int(fam_seed)),fam_D,fam_nvar,fam_F,fam_V,n_exc,fam_val,n_neu,exc_evita,vira,exc_fija) if _MF else None   # rng PROPIO: no toca el del organismo
    if _MF: PAT=_FA['PAT']
