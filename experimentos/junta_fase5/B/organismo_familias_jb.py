"""organismo_familias_jb = organismo_familias_b6.py (bloque 6, SUFIJO DE VARIANTE, b10cbd4ddd0c32a3)
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
    ...
    if (not isinstance(mem_marginal,int)) or isinstance(mem_marginal,bool) or mem_marginal not in (0,1,2,3): raise ValueError(f"mem_marginal={mem_marginal!r}: se espera 0, 1, 2 o 3")   # JB: perilla mal escrita no cae en silencio
    _MRv=int(mem_marginal)   # JB: 0 = abstiene (b6 EXACTO); 1 = marginal del bin ponderada por visitas; 2 = marginal sin ponderar; 3 = la subcasilla MAS VISITADA del bin (la moda)
    if (not isinstance(mem_canales,int)) or isinstance(mem_canales,bool) or mem_canales not in (0,1,2): raise ValueError(f"mem_canales={mem_canales!r}: se espera 0, 1 o 2")   # JB: perilla mal escrita no cae en silencio
    if (not isinstance(mem_kvar,int)) or isinstance(mem_kvar,bool) or mem_kvar<1: raise ValueError(f"mem_kvar={mem_kvar!r}: entero >= 1")   # JB
    _MCv=int(mem_canales); _KVv=int(mem_kvar)   # JB: 0 = una sola lista de ganadoras (b6 EXACTO); 1 = DOS CANALES (forma y variante) SUMADOS; 2 = dos canales con RELEVO POR CONFLICTO DE SIGNO
    if (not isinstance(mem_conj,int)) or isinstance(mem_conj,bool) or mem_conj not in (0,1): raise ValueError(f"mem_conj={mem_conj!r}: se espera 0 o 1")   # JB
    _CJv=int(mem_conj)   # JB: 0 = la via lenta contesta si ALGUNA celda conoce su direccion (b6 EXACTO: una DISYUNCION); 1 = contesta solo si LAS k la conocen (una CONJUNCION: la referencia pasa a ser la INTERSECCION de las k direcciones, que es lo que hace de la tupla de celdas un codigo)
    ...
    _FRMv=[_x9 for _x9 in range(_NP) if max(_PARv[_x9])<_D-int(fam_nvar)]   # JB: celdas de FORMA (las dos coordenadas dentro del bloque de forma): CIEGAS a la variante por construccion -> el CANAL DE FAMILIA
    _VARc=[_x9 for _x9 in range(_NP) if max(_PARv[_x9])>=_D-int(fam_nvar)]   # JB: celdas que TOCAN la variante: su bin de 2 bits YA separa a las hermanas -> el CANAL DE VARIANTE
    if _MCv and (not _FRMv or not _VARc): raise ValueError(f"mem_canales={mem_canales!r} con fam_nvar={fam_nvar!r}: uno de los dos canales queda vacio")   # JB
    ...
    def _topk_can():   # JB: LAS GANADORAS POR CANAL. k_ganadoras celdas de FORMA y mem_kvar celdas de VARIANTE, cada canal ordenado por SU PROPIO error (mismo criterio de b5, aplicado dentro de cada canal), desempate DETERMINISTA por indice, sin azar nuevo
        return (sorted(_FRMv,key=lambda _x9:(float(_MEv[_x9]),_x9))[:k_ganadoras],
                sorted(_VARc,key=lambda _x9:(float(_MEv[_x9]),_x9))[:_KVv])
    def _topk_leer():   # JB: las celdas que LEE la boca. Con mem_canales=0 son EXACTAMENTE las de b5/b6 (`_topk_b5`), y por eso toda la cadena de identidad se mantiene.
    ...
    for t in range(T):
        ...
        if _c4p and t>=_c4t and (_c4m=='inm' or (pos in objs and objs[pos]==_c4r)):   # B4: ENTREGA. 'inm' = en cuanto el emisor aprende; 'sen' = SENALAMIENTO (cuando el receptor esta ante el referente).
            ...
            _c4kp=[list(_PARv[_g5]) for _g5 in _topk_leer()]   # B5: DIAGNOSTICO (no decide ninguna prediccion)
            ...
            _c4kq=[list(_PARv[_g5]) for _g5 in _topk_leer()]
            _c4mbk=sorted(_n5 for _n5 in PAT if all(_bin4(_g5,PAT[_n5])==_bin4(_g5,_P4) for _g5 in _topk_leer()))   # B5: DIAGNOSTICO -- cuantos de los 32 estimulos caen en LAS k casillas del mensaje (con k=1 es _c4mb recalculado DESPUES de escribir)
            _c4mdk=sorted(_n6 for _n6 in PAT if all(_dir_var(_g6,PAT[_n6])==_dir_var(_g6,_P4) for _g6 in _topk_leer()))
            _c4lp={}   # JB: DIAGNOSTICO en el paso de la ENTREGA (justo despues de escribirse el mensaje): que valor lee la via lenta para CADA estimulo y por que via. Observado, no prometido: no decide ninguna prediccion, no consume rng y no cambia ninguna decision.
            for _n8 in PAT:
                _v8,_k8=_tabla_v15f(PAT[_n8])
                _e8=int(sum(1 for _g8 in _topk_leer() if _MNv[_g8,_dir_var(_g8,PAT[_n8])]>0))
                _c4lp[_n8]=[round(float(_v8),4),bool(_k8),_e8,int(len(_topk_leer()))]   # [valor, conocido, celdas que contestan EXACTO, k]   # B6: DIAGNOSTICO -- cuantos de los 32 estimulos caen en LAS k DIRECCIONES del mensaje (con memoria_variante=0 es exactamente _c4mbk)
        if pos in objs:
