"""O3_TERM_CIEGO (construido por experimentos/subida_n9/construye_n9.py desde carros/O3.py sha 0442c2884fcb0e11; UNICO cambio: COLA_TERM = 0, la muerte programada no lee la reserva del linaje). carros/O3.py — escuderia O3 (combo Opus: disenador + biologo + critico). Carrera de escuderias, RONDA 2 (ENMIENDA 5).
...
class Carro:
        ...
        for j in (0, 1):
            if v[j] < 0:
                if self._modo == 'term': piso = -1e9
                    ...
                    piso = PISO_RES if (RESERVA_COLA and self.cola_est <= COLA_RES) else PISO
    ...
    def actua(self, obs):
        ...
        term = TERMINAL and self.partos_cuerpo >= PARTOS_TERM and self.cola_est >= COLA_TERM
        ...
        elif M3 and self.cola_est >= COLA_EXIST and min(lev) < self.U + MARGEN:
            self._modo = 'exist'
            j = 0 if lev[0] <= lev[1] else 1
    ...
    def muere(self, info):
        if TERMINAL and self.partos_cuerpo >= PARTOS_TERM and self.cola_est >= COLA_TERM: self.st['cuerpos_term'] += 1
        return None
