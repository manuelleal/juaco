...
arnes identidad_n9c.py). Organismo: FABRICA (mitad cerebro del brazo REL de organismo_f9c) en cinco variantes construidas por
anclas (construye_n9c.py): NADA (== FABRICA) · BOCA (boca por gradiente de la deriva) · HER (el hijo recibe copia de lo
aprendido por el padre vivo) · CAND (BOCA + HER) · CRUZ (BOCA + copia con las filas de necesidad intercambiadas).
...
Formas validas (exactas):
  python experimentos/subida_n9c/corre_n9c.py --humo [--desde 14191] [--n 1] [--T 20000] [--brazos NADA,CAND,...]   (UN proceso)
...
SHAS = {os.path.join(GEN, 'pista2.py'): '4d2bee16e7961261', os.path.join(GEN, 'motor_convive.py'): 'd10cb9021f5d0f41',
        ...
        os.path.join(AQUI, 'carros', 'N9C_NADA.py'): 'b122f4bbee9fed13',
        os.path.join(AQUI, 'carros', 'N9C_BOCA.py'): '7764dc04674ca997',
        os.path.join(AQUI, 'carros', 'N9C_HER.py'): '601c31baf82b75fb',
        os.path.join(AQUI, 'carros', 'N9C_CAND.py'): '6a89fec2309db2c0',
        os.path.join(AQUI, 'carros', 'N9C_CRUZ.py'): '17d638026f02869e'}
BRAZOS = ('NADA', 'BOCA', 'HER', 'CAND', 'CRUZ')
...
PARES = (('CAND', 'NADA'), ('CAND', 'BOCA'), ('CAND', 'HER'), ('CAND', 'CRUZ'), ('BOCA', 'NADA'), ('HER', 'NADA'), ('CRUZ', 'BOCA'))
...
def parsea(argv):
    ...
    if a['humo']:
        ...
        a['desde'] = a['desde'] or 14191; a['n'] = a['n'] or 1; a['T'] = a['T'] or 20000; a['brazos'] = a['brazos'] or list(BRAZOS)
...
def veredicto(A, P, n_sem):
    ...
    K = 'R0_nacidos'; out = {}
    nada = A.get('NADA') or {}; cand = A.get('CAND') or {}; boca = A.get('BOCA') or {}; cruz = A.get('CRUZ') or {}
    ...
    k2 = f'G2 HERENCIA (CAND > BOCA en R0_nacidos, >= {need}/{n_sem})'
    k3 = f'G3 CONTENIDO (CAND > CRUZ en R0_nacidos, >= {need}/{n_sem})'
    k4 = f'G4 BOCA (CAND > HER en R0_nacidos, >= {need}/{n_sem})'
    ...
    x = g('CAND-BOCA', K); out[k2] = bool(x and x['gana'] >= need)
    x = g('CAND-CRUZ', K); out[k3] = bool(x and x['gana'] >= need)
    x = g('CAND-HER', K); out[k4] = bool(x and x['gana'] >= need)
    out['C-BOCA (control que puede ganar: la boca sola persiste en >= 15/20)'] = bool(boca and boca.get('persiste_estricto', 0) >= need)
    ...
    if not out[kA]: v = 'NO SE LEE (ancla fuera: el instrumento no reproduce el piso calibrado de FABRICA en v2)'
        ...
        v = 'FUNCIONA (un linaje del organismo propio persiste con flujo fijo de comida, y hace falta lo que el padre aprendio y la boca que lee su propio estado)'
    elif out[k1]:
        ...
        v = 'HAY ALGO MODESTO: SUBE SIN PERSISTIR (lo heredado sube el R0 de los nacidos por su contenido, sin sostener el linaje)'
