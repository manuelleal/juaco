"""organismo_g1 = organismo_v13q5.py (fae9c32b146fdbb4) + perilla `celdas` (M1, enjambre/grupo1): 15 lectores lineales
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.05,memoria_rechazo=20,mu_norm=True,div_signo=True,eta_s=0.0,clip_s=3.0,puerta=None,mundo='AB',regla='px0',fase2_en=None,sonda_final=False,lectura='lineal',regla_lenta='dos_canales',constante=False,lam_lenta=0.002,seleccion=None,sel_theta=0.6,sel_rho=0.05,sel_cupo=1,sel_calienta=0,sel_estad='cond',lab=False,celdas=None,cel_rho=0.02):   # xor: lectura de la via lenta; 3d: regla de la via lenta, termino constante y decaimiento; creacion_A A-1(ii): seleccion de rasgos conjuntivos por competencia
    ...
    if celdas not in (None,'par2'): raise ValueError(f"celdas={celdas!r}")   # enjambre/grupo1: un knob mal escrito NO puede caer en silencio al brazo original
            ...
            if mordio:
                ...
                if learn:
                    ...
                    if eta_s:   # v13: actualizacion de la via lenta (su tasa, mismo drenaje)
                        ...
                        if celdas is not None:   # enjambre/grupo1 M1: 15 lectores de 2 canales (pares de pixeles), k-WTA (k=1) por su propio error
    ...
    return dict(cel_modo=celdas,cel_ganadora=(None if celdas is None else list(_PAR[_CG])),cel_E=[round(float(x),4) for x in _CE],lab=lab,lenta_eventos=_ev5,fase2_en=fase2_en,seleccion=seleccion,sel_estad=sel_estad,sel_theta=sel_theta,sel_rho=sel_rho,sel_cupo=sel_cupo,
