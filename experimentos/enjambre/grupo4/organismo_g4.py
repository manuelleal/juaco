"""organismo_g4 = organismo_v13q5.py (fae9c32b146fdbb4) + perilla `tabla_g` (grupo4/M4: tabla por grupos de pixeles de grado g
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.05,memoria_rechazo=20,mu_norm=True,div_signo=True,eta_s=0.0,clip_s=3.0,puerta=None,mundo='AB',regla='px0',fase2_en=None,sonda_final=False,lectura='lineal',regla_lenta='dos_canales',constante=False,lam_lenta=0.002,seleccion=None,sel_theta=0.6,sel_rho=0.05,sel_cupo=1,sel_calienta=0,sel_estad='cond',tabla_g=None,lab=False):   # xor: lectura de la via lenta; 3d: regla de la via lenta, termino constante y decaimiento; creacion_A A-1(ii): seleccion de rasgos conjuntivos por competencia
    ...
    _g4_on=tabla_g is not None   # grupo4 (M4): tabla por grupos de pixeles + competencia; None/apagado = organismo_v13q5 exacto
    if _g4_on:
        _g4_g=int(tabla_g.get('g',2)); _g4_estad=tabla_g.get('estad','mse'); _g4_modo=tabla_g.get('modo','dura')
        _g4_tau=float(tabla_g.get('tau',1.0)); _g4_eta=float(tabla_g.get('eta',0.15)); _g4_rho=float(tabla_g.get('rho',0.05))
        _g4_alpha=float(tabla_g.get('alpha',1.0)); _g4_clip=float(tabla_g.get('clip',clip_s))
        _g4_grupos=list(itertools.combinations(range(6),_g4_g)); _g4_ng=len(_g4_grupos)
        _g4_tabla=np.zeros((_g4_ng,2**_g4_g)); _g4_vistos=np.zeros((_g4_ng,2**_g4_g),bool)
        _g4_ecomp=np.zeros(_g4_ng); _g4_npos=np.zeros(_g4_ng); _g4_abre=[]; _g4_ganador_prev=-1
    def _g4_idx(P,gi):   # grupo4: codigo binario (0..2**g-1) de los pixeles del grupo gi en el patron P
        _c=0
        for _p in _g4_grupos[gi]: _c=_c*2+(1 if P[_p]>0.5 else 0)
        return _c
    ...
    def lenta(P):   # 3d: lectura de la via lenta sola segun regla_lenta ('dos_canales' = la formula original, exacta); grupo4: tabla_g tiene prioridad si esta activo
        if _g4_on:
            ...
            if mordio:
                ...
                if learn:
                    ...
                    if eta_s:   # v13: actualizacion de la via lenta (su tasa, mismo drenaje)
                        ...
                        if _g4_on:   # grupo4 (M4): tabla por grupos + competencia; el estadistico se mide ANTES de actualizar la celda (no se filtra el dato consigo mismo)
                            for _gi in range(_g4_ng):
                                _c=_g4_idx(P_[kk],_gi); _g4_npos[_gi]+=1
                                _pred_antes=_g4_tabla[_gi,_c] if _g4_vistos[_gi,_c] else 0.0
                                if _g4_estad=='mse': _g4_ecomp[_gi]=(1-_g4_rho)*_g4_ecomp[_gi]-_g4_rho*((_ds-_pred_antes)**2)
                                else: _g4_ecomp[_gi]=(1-_g4_rho)*_g4_ecomp[_gi]+_g4_rho*(1.0 if (_pred_antes>0)==(_ds>0) else (0.0 if _pred_antes==0 else -1.0))
                                _g4_tabla[_gi,_c]=np.clip((_g4_tabla[_gi,_c]+_g4_eta*(_ds-_g4_tabla[_gi,_c])) if _g4_vistos[_gi,_c] else _ds,-_g4_clip,_g4_clip)
                                _g4_vistos[_gi,_c]=True
                            _g4_gan=int(np.argmax(_g4_ecomp))
                            if _g4_gan!=_g4_ganador_prev: _g4_abre.append((int(t),_g4_gan,list(_g4_grupos[_g4_gan]))); _g4_ganador_prev=_g4_gan
    ...
    return dict(lab=lab,lenta_eventos=_ev5,fase2_en=fase2_en,seleccion=seleccion,tabla_g=tabla_g,g4_ganador=(list(_g4_grupos[int(np.argmax(_g4_ecomp))]) if _g4_on else None),g4_abre=(_g4_abre if _g4_on else []),g4_npos=([int(x) for x in _g4_npos] if _g4_on else []),g4_abre_conjuntivo_correcto=(bool(_g4_on and _g4_g==2 and regla=='xor01' and tuple(_g4_grupos[int(np.argmax(_g4_ecomp))])==(0,1)) if _g4_on else None),sel_estad=sel_estad,sel_theta=sel_theta,sel_rho=sel_rho,sel_cupo=sel_cupo,
