"""organismo_g2 = organismo_v13q5.py (fae9c32b146fdbb4) + perilla `fision` (M2 -- FISION DE LA VIA LENTA POR CONFLICTO
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.05,memoria_rechazo=20,mu_norm=True,div_signo=True,eta_s=0.0,clip_s=3.0,puerta=None,mundo='AB',regla='px0',fase2_en=None,sonda_final=False,lectura='lineal',regla_lenta='dos_canales',constante=False,lam_lenta=0.002,seleccion=None,sel_theta=0.6,sel_rho=0.05,sel_cupo=1,sel_calienta=0,sel_estad='cond',lab=False,fision=None,fis_umbral=3.0,fis_rho=0.05):   # xor: lectura de la via lenta; 3d: regla de la via lenta, termino constante y decaimiento; creacion_A A-1(ii): seleccion de rasgos conjuntivos por competencia
    ...
    if fision not in (None,'conflicto'): raise ValueError(f"fision={fision!r}")   # M2 (enjambre grupo2): un knob mal escrito NO puede caer en silencio al brazo original
    _FC=[([_i],np.zeros(2)) for _i in range(6)]; _FE=np.full(64,1e9)   # M2: 6 lectores de UN canal {P_i,1}; error EMA por celda (holgado; tope duro 12 celdas)
    _Fmp=np.zeros(6); _Fmn=np.zeros(6); _Fsp=np.zeros((6,6)); _Fsn=np.zeros((6,6)); _Fcp=np.zeros(6); _Fcn=np.zeros(6)   # M2: masa y suma de patrones por canal y signo de R
    _Fhij=[]; _FG=0   # M2: canales que ya parieron (maximo 1 hija cada uno); indice en _FC de la celda ganadora (menor _FE)
    ...
    def lenta(P):   # 3d: lectura de la via lenta sola segun regla_lenta ('dos_canales' = la formula original, exacta)
        if fision is not None:   # M2: lee la celda GANADORA (menor error) con SU PROPIA base; hija = {P_i,P_j,P_i*P_j,1}
            _g,_w=_FC[_FG]
            ...
            if mordio:
                ...
                if learn:
                    ...
                    if eta_s:   # v13: actualizacion de la via lenta (su tasa, mismo drenaje)
                        ...
                        if fision is not None:   # M2 (enjambre grupo2): fision de la via lenta por conflicto de signo; SEÑAL = R de la propia mordida, nada mas
                            _Pk=P_[kk]
                            for _i in range(6):   # (a) masa y suma de patrones por canal activo, partida por signo de R
                                if _Pk[_i]==1.0:
                                    if R>0: _Fmp[_i]+=1; _Fsp[_i]+=_Pk; _Fcp[_i]+=1
                                    else:   _Fmn[_i]+=1; _Fsn[_i]+=_Pk; _Fcn[_i]+=1
                            for _c in range(len(_FC)):   # (b) cada celda actualiza con SU PROPIO residuo sobre SU PROPIA base (misma memoria que las celdas KW del tronco)
                                _cg,_cw=_FC[_c]
                            ...
                            for _i in range(6):   # (c) FISION: nace UNA hija del canal que MAS separa comida de veneno bajo i (v11: el mismo dist=P-mu, partido por signo); tope 1 por canal (6 en total)
                            ...
                            _FG=int(min(range(len(_FC)),key=lambda _c:_FE[_c]))   # (d) la via lenta lee, de aqui en adelante, la celda de menor error
    ...
    return dict(lab=lab,lenta_eventos=_ev5,fase2_en=fase2_en,fis_celdas=[list(g) for g,_ in _FC],fis_ganadora=list(_FC[_FG][0]),fis_n=len(_FC),seleccion=seleccion,sel_estad=sel_estad,sel_theta=sel_theta,sel_rho=sel_rho,sel_cupo=sel_cupo,
