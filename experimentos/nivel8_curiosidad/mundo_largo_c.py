"""mundo_largo_c = mundo_largo.py (9f74ff6b5941e5a5) + curiosidad por progreso de error (EMA lenta por celda, progreso del
patron, sesgo gamma_C con la retina vacia) + control de prioridad barajada. Generado por construye_curiosidad.py. NO
editar. Con gamma_C=0 es mundo_largo exacto (identidad obligatoria)."""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        ...
        gamma_C=0.0,ema_l=0.005,cur_barajada=False):   # mapa + largo + curiosidad
    ...
    def _prog(P):   # curiosidad: progreso medio de las 3 celdas del codigo del patron
        ...
        return float(_pc[kenyon(P)>0].mean())
            ...
            if mordio:
                ...
                if learn:
                    ...
                    if plast:
                        ...
                        err_l[idx]=(1-ema_l)*err_l[idx]+ema_l*abs(dlt)   # curiosidad
                        ...
                        for c in idx:
                            if div_signo:   # v11 (JUACO-EVO gen1/llm_2): divide por CONFLICTO DE SIGNO, hija ciega fuera de P, madre fija, fision del valor
                                ...
                                if Wb[c]*R<0 and abs(float(Wb[c]))>0.2 and float(kj@P)>float(KW[c]@P) and (~activa).any():
                                    ...
                                    mu[j]=P*(float(mu[c].sum())/P.sum()); err[c]=err[j]=0; err_l[c]=err_l[j]=0; splits+=1; split_t.append((t,kk))
                                ...
                                Wp[j]=Wp[c]; Wn[j]=Wn[c]; mu[j]=mu[c].copy(); err[c]=err[j]=0; err_l[c]=err_l[j]=0; splits+=1; split_t.append((t,kk))
