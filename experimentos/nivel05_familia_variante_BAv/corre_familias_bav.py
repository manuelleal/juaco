...
sys.path[:0] = [AQUI, JBA, JA, N12, CREA, CREB, os.path.join(RAIZ, 'organismo')]   # organismo/ PRIMERO (ERR-28)
import numpy as np
...
T_HUMO = 100000
SEM_HUMO = [912]                             # banda de HUMOS (901-920); 911 la gasto BA, 901-903 la junta
...
PLAN_HUMO = [('b4b', 'CANAL'), ('BA-v', 'CANAL'), ('BA-v', 'CORTADO'),
...
SHAS = lambda: dict(script=h16(os.path.abspath(__file__)),
                    constructor=h16(os.path.join(AQUI, 'construye_familias_bav.py')),
                    instrumento=h16(os.path.join(AQUI, 'organismo_familias_bav.py')),
                    arnes=h16(os.path.join(AQUI, 'identidad_familias_bav.py')),
                    preregistro=h16(os.path.join(AQUI, 'PREREGISTRO_bav.md')),
...
ORIGENES = [('origen_ba', SHA_BA_ESPERADO), ('origen_a1', SHA_A1_ESPERADO), ('origen_b6', SHA_B6_ESPERADO),
...
def humo(semillas, Tb, plan):
    ...
    CF._log['f'] = open(os.path.join(HUMOS, 'humo_bav_%s.log' % stamp), 'w', encoding='utf-8', newline='\n')
    CF._log['t0'] = time.time()
    if not cabecera("HUMO del candidato BA-v (nivel 5, criterio ERR-90). UN proceso, sin Pool (regla 3).",
    ...
    crudo = os.path.join(HUMOS, 'humo_bav_%s_crudo.json' % stamp)   # ERR-54 / ERR-42: el crudo, ANTES del analisis
    ...
    dj = os.path.join(HUMOS, 'humo_bav_%s.json' % stamp)
...
def serie(desde, n_sem, Tb, cel_act, npool):
    ...
    stamp = time.strftime('%Y%m%d_%H%M%S')
    CF._log['f'] = open(os.path.join(AQUI, 'serie_bav_s%d-%d_%s.log' % (SEEDS[0], SEEDS[-1], stamp)),
                        'w', encoding='utf-8', newline='\n')
    CF._log['t0'] = time.time()
    if not cabecera("SERIE del candidato BA-v (nivel 5, criterio ERR-90), Pool(%d)." % npool, cel_act,
    ...
    with mp.Pool(npool) as pool:
        ...
        for i, r in enumerate(pool.imap_unordered(tarea, tr, chunksize=1), 1):
            ...
            if i % 40 == 0 or i == len(tr):
                log("          %d/%d" % (i, len(tr)))
    crudo = os.path.join(AQUI, 'serie_bav_s%d-%d_%s_crudo.json' % (SEEDS[0], SEEDS[-1], stamp))
...
if __name__ == '__main__':
    ...
    if '--serie' in sys.argv:          # LA CONFIRMATORIA LA LANZA EL COORDINADOR (reglas 3 y 11)
        ...
        serie(arg('--desde', 961), arg('--n', 20), arg('--T', 100000), cel_act, npool)
        sys.exit(0)
