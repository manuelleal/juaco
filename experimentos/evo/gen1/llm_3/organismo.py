...
gen1/llm_3 (JUACO-EVO): + REPASO por celda con puerta de edad (Rc, tc); ver hipotesis.md. Copia mutada, no v10m.
"""
"""
...
R_REP=50; EDAD_REP=5000   # gen1/llm_3 REPASO: periodo del repaso y edad (pasos sin mordida) a partir de la cual una celda repasa
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
    ...
    Rc=np.zeros(NKMAX); tc=np.full(NKMAX,T); _ir=0   # gen1/llm_3 REPASO: traza episodica POR CELDA (ultimo R, paso de la ultima mordida); memoria declarada
    ...
    for t in range(T):
        ...
        if pos in objs:
            ...
            if mordio:
                ...
                if _ap:
                    dlt=R-Wb@kc
                    Rc[kc>0]=R; tc[kc>0]=t   # gen1/llm_3 REPASO: la celda recuerda su ultimo R y cuando la mordieron
                    ...
                    if plast:
                        ...
                        for c in idx:
                            if err[c]>theta and (~activa).any():
                                ...
                                tc[c]=tc[j]=T   # gen1/llm_3 REPASO: madre e hija no repasan hasta volver a ser mordidas
        _prev_on=pos if pos in objs else -1   # v9: para contar llegadas
        if _ap and t%R_REP==0:   # gen1/llm_3 REPASO: una celda activa que lleva > EDAD_REP pasos sin mordida re-ensena su ultimo R al codigo ACTUAL de su prototipo mu[c]; sin morder, sin energia, sin RNG, sin err/mu, sin divisiones
            _vc=np.where(activa&(t-tc>EDAD_REP)&(mu.sum(1)>0))[0]
            if len(_vc):
                _c=int(_vc[_ir%len(_vc)]); _ir+=1; _kr=kenyon(mu[_c]); _dr=Rc[_c]-(Wp-Wn)@_kr
                if lam: _jx=_kr>0; _mr=np.minimum(Wp[_jx],Wn[_jx]); Wp[_jx]-=lam*_mr; Wn[_jx]-=lam*_mr
                if _dr>0: Wp=np.clip(Wp+eta/3*_dr*_kr,0,3.)
                else:     Wn=np.clip(Wn+eta/3*aversion*(-_dr)*_kr,0,3.)
        E-=costo
