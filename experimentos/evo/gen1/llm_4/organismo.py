...
gen1/llm_4 (JUACO-EVO, clase PLASTICIDAD Y TASAS): reparto competitivo de la correccion RW por familiaridad celda-patron; hijo de organismo_v10m.
"""
"""
        ...
        if pos in objs:
            ...
            if mordio:
                ...
                if _ap:
                    ...
                    _ix=kc>0
                    _P=PAT[kk]; _sm=mu[_ix].sum(1); _fam=np.where(_sm>0,1/(1+((_P-mu[_ix]*(_P.sum()/np.maximum(_sm,1e-9))[:,None])**2).sum(1)),1.)   # llm_4: familiaridad celda-patron, mu normalizada como K5; celda virgen = 1
                    kf=kc.copy(); kf[_ix]=_fam*(_fam.size/_fam.sum())   # llm_4: reparto competitivo: la suma de tasas es la del padre, cada celda recibe en proporcion a su familiaridad
                    if dlt>0: _trunca=bool(((Wp[_ix]+eta*dlt*kf[_ix])>3.0).any())
                    else:     _trunca=bool(((Wn[_ix]+eta*aversion*(-dlt)*kf[_ix])>3.0).any())
                    if _trunca:
                        n_techo+=1
                        if t_techo is None: t_techo=t
                    if dlt>0: Wp=np.clip(Wp+eta*dlt*kf,0,3.)
                    else:     Wn=np.clip(Wn+eta*aversion*(-dlt)*kf,0,3.)
