...
JUACO-EVO gen1/llm_1: v10m + reparto del credito RW por compromiso opuesto (1 linea nueva, 4 tocadas; ver hipotesis.md).
"""
"""
        ...
        if pos in objs:
            ...
            if mordio:
                ...
                if _ap:
                    ...
                    _ix=kc>0
                    _op=(Wn if dlt>0 else Wp)[_ix]; _w=1/(1+20.*_op); _wc=kc*1.; _wc[_ix]=K*_w/_w.sum()   # llm_1: credito a las celdas sin compromiso opuesto; suma K
                    if dlt>0: _trunca=bool(((Wp[_ix]+eta*dlt*_wc[_ix])>3.0).any())
                    else:     _trunca=bool(((Wn[_ix]+eta*aversion*(-dlt)*_wc[_ix])>3.0).any())
                    if _trunca:
                        n_techo+=1
                        if t_techo is None: t_techo=t
                    if dlt>0: Wp=np.clip(Wp+eta*dlt*_wc,0,3.)
                    else:     Wn=np.clip(Wn+eta*aversion*(-dlt)*_wc,0,3.)
