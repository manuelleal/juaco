...
"""
import numpy as np
# gen1/llm_2 (JUACO-EVO, operador LLM, clase REGLA ESTRUCTURAL): padre organismo_v10m.py; unica diferencia: el bloque de division (ver hipotesis.md).
                ...
                if _ap:
                    ...
                    if plast:
                        ...
                        for c in idx:
                            ...
                            kj=np.clip(KW[c]*(1-0.05)+paso*dist,0,5)*(P>0)   # llm_2: la hija nace ciega fuera de los pixeles de P y hereda el 95% de la sintonia de la madre dentro de P
                            if Wb[c]*R<0 and abs(float(Wb[c]))>0.2 and float(kj@P)>float(KW[c]@P) and (~activa).any():   # llm_2: dispara el CONFLICTO DE SIGNO (valor consolidado vs refuerzo contrario), no el error acumulado; solo si la hija le gana a la madre en P; la madre no se mueve
                                j=int(np.where(~activa)[0][0]); activa[j]=True; KW[j]=kj
                                if R>0: Wp[j]=Wp[c]; Wn[j]=0.; Wp[c]=0.   # llm_2: fision del valor: la hija se lleva el signo nuevo, la madre conserva el viejo intacto
                                else:   Wn[j]=Wn[c]; Wp[j]=0.; Wn[c]=0.
                                mu[j]=P*(float(mu[c].sum())/P.sum()); err[c]=err[j]=0; splits+=1; split_t.append((t,kk))   # llm_2: la historia de la hija es P
