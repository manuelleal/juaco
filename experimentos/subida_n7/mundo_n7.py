"""mundo_n7 = experimentos/nivel10_composicion_v14/mundo_composicion_v14.py (a9098933b1950e3d, solo se leyo)
+ B-5 (desambiguar, transcrito de organismo/organismo_v142.py 17528d767fcebaf6) + norm_lenta (via lenta normalizada
por masa, M0 = 3.0). Generado por experimentos/subida_n7/construye_n7.py. NO editar a mano. Con desambiguar=0 y
norm_lenta=0 es mundo_composicion_v14 EXACTO (arnes: experimentos/subida_n7/identidad_n7.py)."""
"""mundo_composicion_v14 = experimentos/nivel7_hija_dispersa/mundo_hija_dispersa.py (d305d53186fcbcd6,
instrumento ya corrido: solo se leyo) + PUERTA POR EVIDENCIA DEL CODIGO EXACTO (perilla puerta_pat;
mask_rel/recic/n_cf ya estaban en el origen). Mundo 3T-k (composicion temporal) -- este es el
instrumento de C3 del PREREGISTRO_composicion_v14.md. Generado por construye_v14c.py. NO editar a mano.
Con puerta_pat=0 (y mask_rel=0, recic=0, n_cf=1 heredados) es mundo_temporal_k EXACTO (arnes: identidad_v14c.py)."""
"""mundo_hija_dispersa = mundo_temporal_k.py (68736baafe7c8cdb) + HIJA DISPERSA (mask_rel) + division diferida (n_cf) + reciclaje apagado (recic) + diagnostico del presupuesto de celdas.
Generado por construye_hija_dispersa.py. NO editar a mano. Con recic=0, mask_rel=0 y n_cf=1 es
mundo_temporal_k EXACTO (arnes: experimentos/nivel7_hija_dispersa/identidad_hija_dispersa.py)."""
"""mundo_temporal_k = mundo_temporal_v13.py (f9c3169b32f393d4) + profundidad de historia k (3T-k). Generado por
construye_3T_k.py. NO editar. Con k=1 es mundo_temporal_v13 exacto (identidad obligatoria). La regla del mundo
depende del slot MAS PROFUNDO; los intermedios son distractores. W, sep y solap se promedian sobre los rellenos."""
"""mundo_temporal_v13 = mundo_temporal_v11.py (807f4b357f9eac1a) + via lenta lineal sobre la entrada completa + puerta
de familiaridad (las reglas de organismo_v13). Generado por construye_reverificacion_v13.py. NO editar.
Con eta_s=0 y puerta=None es mundo_temporal_v11 exacto (control de inercia)."""
"""mundo_temporal_v11 = mundo_temporal_v9.py (18d96a1c79863bb0) + mu_norm (v10) + div_signo (v11).
Generado por construye_v11_generaliza.py. NO editar. Con mu_norm=False y div_signo=False es
mundo_temporal_v9 exacto (control KT1)."""
"""mundo_temporal_v9 = mundo_temporal_v8.py (ee7eac62ecab3313) + memoria de trabajo de rechazo (v9).
Generado por construye_reverificacion.py. NO editar. Con memoria_rechazo=0 es mundo_temporal_v8 (KT1)."""
"""mundo_temporal_v8 = mundo_temporal.py (1f447657faf00ef8) + drenaje de la parte comun (lam=0.05, linea
aritmeticamente identica a organismo/organismo_v8.py) + mordida del techo. Generado por
construye_mundo_v8.py. NO editar a mano. Con lam=0 es mundo_temporal.py (control K1).
Preregistro: experimentos/3T_confirmatorio/PREREGISTRO_3T_confirmatorio.md
"""
"""
3T — Mundo que exige ORDEN. Rama exploratoria. NO toca organismo/.

Copia de organismo_v7.py (v6 + regla de division 2L) con UN solo cambio de mundo:
el valor de morder depende de QUE SE MORDIO ANTES.

    morder A con last=='B'  -> comida  R=+1.0  dE=+0.8   (A|B)
    morder A con last=='A'  -> veneno  R=-3.0  dE=-0.4   (A|A)
    morder B (cualquiera)   -> neutro  R= 0.0  dE=+0.1

Cuerpo, patas, politica, hambre, muerte, renovacion, eta/tau_e/alpha/costo: identicos a v6/v7.
Regla 2L (theta=0.6, ema=0.02, paso=0.5, NKMAX=90): identica, sin recalibrar.

Brazos (ver PREREGISTRO.md):
  C1  : NIN=6,  sin plasticidad              (control v6)
  C1p : NIN=6,  con plasticidad              (control: dividir sin canal temporal)
  C2  : NIN=12 visible, sin plasticidad, rechazo hasta code(A|A) n code(A|B)==0   (techo)
  C2b : NIN=12 con columnas temporales a 0, sin plasticidad   (control de instrumento: == C1)
  C3  : NIN=12 con columnas temporales a 0, con plasticidad   (LA PREGUNTA)
  C3C : como C3 pero el canal temporal que VE el organismo es aleatorio (el mundo sigue
        usando el last real para el valor)                    (control de artefacto del clip)

Alineacion del RNG: C1/C1p/C2b/C3/C3C sortean KW como (NK,6) -> mismo flujo -> identicos
hasta la primera division. C3C usa un rng aparte (seed+100000) para el canal falso.
"""
import sys, numpy as np
import itertools   # 3T-k
try: sys.stdout.reconfigure(encoding='utf-8', errors='replace')
except Exception: pass

L = 40; NK = 30; NKMAX = 90; K = 3
PAT = {'A': np.array([1, 1, 0, 1, 0, 0.]), 'B': np.array([1, 0, 1, 0, 1, 0.])}
R_VAL = {'comida': 1.0, 'veneno': -3.0, 'neutro': 0.0}
E_VAL = {'comida': +0.8, 'veneno': -0.4, 'neutro': +0.1}
SIT = [('A', 'A'), ('A', 'B'), ('B', 'A'), ('B', 'B')]   # (actual, anterior)

ARMS = {
    #        nin  temporal_visible  plast  init        canal_falso
    'C1':  dict(nin=6,  plast=False, init='v6',    falso=False),
    'C1p': dict(nin=6,  plast=True,  init='v6',    falso=False),
    'C2':  dict(nin=12, plast=False, init='full',  falso=False),
    'C2b': dict(nin=12, plast=False, init='ciego', falso=False),
    'C3':  dict(nin=12, plast=True,  init='ciego', falso=False),
    'C3C': dict(nin=12, plast=True,  init='ciego', falso=True),
}


def valor(cur, last):
    """3T-k: last es la historia (h1..hk); manda el slot MAS PROFUNDO. Con k=1 es la regla original."""
    if cur == 'B': return 'neutro'
    hk = last[-1] if isinstance(last, tuple) else last
    return 'comida' if hk == 'B' else 'veneno'


def run(seed, arm='C1', T=100000, learn=True, eta=.03, tau_e=.85, alpha=1.2,
        hambre_boca=2.0, aversion=1.0, costo=.002, nobj=4,
        theta=0.6, ema=0.02, paso=0.5, early=5000, nkmax=NKMAX, wclip=3.0, lam=0.05, memoria_rechazo=20, mu_norm=False, div_signo=False, eta_s=0.0, clip_s=3.0, puerta=None, kprof=1, recic=0, tau_r=0, mask_rel=0, del_s=0.25, del_c=0.25, n_cf=1, ema_c=0.05, puerta_pat=0, pat_shuf=0, pat_min=0, desambiguar=0, norm_lenta=0):
    # nkmax y wclip son parametros SOLO para el diagnostico POST-HOC de los dos
    # confusores preregistrados (agotamiento del pool y saturacion de canales).
    # Con los valores por defecto (90, 3.0) el codigo es identico al congelado en v7:
    # equivalencia verificada campo a campo en las 120 corridas (ver equivalencia.py).
    cfg = ARMS[arm]; NIN = cfg['nin'] if cfg['nin'] == 6 else 6 * (kprof + 1); plast = cfg['plast']; falso = cfg['falso']   # 3T-k
    FILL = list(itertools.product('AB', repeat=kprof - 1))   # rellenos de los distractores (k=1: uno, vacio)
    def hist_de(p, fill=None): return (tuple('A' * (kprof - 1)) if fill is None else tuple(fill)) + (p,)   # h1..hk con hk = p
    rng = np.random.default_rng(seed)
    rng2 = np.random.default_rng(seed + 100000)        # canal falso, flujo separado

    Wl = rng.uniform(.1, .4, (2, 9))
    KW = np.zeros((nkmax, NIN)); activa = np.zeros(nkmax, bool); activa[:NK] = True

    def inp(cur, prev):
        if NIN == 6: return PAT[cur]
        h = prev if isinstance(prev, tuple) else hist_de(prev)   # una letra = slot profundo con relleno canonico 'A'
        return np.concatenate([PAT[cur]] + [PAT[x] for x in h])

    def code(P):
        v = KW @ P; v = np.where(activa, v, -1e9); return set(np.argsort(v)[-K:])

    # --- inicializacion (muestreo por rechazo) ---
    if cfg['init'] == 'full':                     # C2: 12 columnas aleatorias
        KW[:NK] = rng.uniform(0, 1, (NK, NIN))
        while len(code(inp('A', 'A')) & code(inp('A', 'B'))) != 0:
            KW[:NK] = rng.uniform(0, 1, (NK, NIN))
    else:                                         # C1/C1p/C2b/C3/C3C: mismo flujo que v6
        KW[:NK, :6] = rng.uniform(0, 1, (NK, 6))
        while len(code(inp('A', 'A')) & code(inp('B', 'B'))) != 0:
            KW[:NK, :6] = rng.uniform(0, 1, (NK, 6))

    def kenyon(P):
        k = np.zeros(nkmax); k[list(code(P))] = 1; return k

    Wp = np.zeros(nkmax); Wn = np.zeros(nkmax)
    ncod={}; _ord=[]   # B: evidencia del CODIGO EXACTO (mordidas por codigo) y orden de aparicion
    def _key(_k): return frozenset(np.flatnonzero(_k).tolist())
    def _ev(_k):   # evidencia que LEE la puerta: la propia, o (control) la del codigo vecino en el orden de aparicion
        _q=_key(_k)
        if not pat_shuf: return ncod.get(_q,0)
        if _q not in ncod or len(_ord)<2: return 0
        return ncod[_ord[(_ord.index(_q)+1)%len(_ord)]]
    def _fam(_k):   # B: la puerta. puerta_pat>0 -> evidencia del codigo exacto; si no, celdas consolidadas (v13 EXACTO)
        if puerta_pat: return _ev(_k)>=puerta_pat and int((np.abs((Wp-Wn)[_k>0])>0.2).sum())>=pat_min
        return int((np.abs((Wp-Wn)[_k>0])>0.2).sum())>=puerta
    t_act = np.full(nkmax, -10**9, dtype=np.int64); n_act = np.zeros(nkmax, dtype=np.int64)   # B: reloj y cuenta de activacion por celda
    div_bloq = 0; n_recic = 0; n_des = 0; M0 = 3.0   # n7: divisiones B-5 y masa de referencia de norm_lenta   # B: divisiones que el pool lleno bloqueo / celdas recicladas
    Wps = np.zeros(NIN); Wns = np.zeros(NIN)   # v13: via LENTA lineal sobre la entrada completa (inerte si eta_s=0)
    def valor_tot(P):   # v13: valor que usa la boca: suma (sin puerta) o la rapida si el patron le es familiar, si no la lenta
        _k = kenyon(P); _f = float((Wp - Wn) @ _k); _s = float((Wps - Wns) @ P)
        return _f + _s if puerta is None else (_f if _fam(_k) else _s)
    err = np.zeros(nkmax); mu = np.zeros((nkmax, NIN))
    mus = np.zeros((nkmax, NIN)); div_diag = []   # B2: EMA de P firmada por el error; diagnostico de division
    mup = np.zeros((nkmax, NIN)); mun = np.zeros((nkmax, NIN))   # B3: medias condicionadas por el signo de R
    _rng3 = np.random.default_rng(seed + 200000)   # B3: solo para los controles (mask_rel=3 y 4); RNG aparte
    _ns = NIN // 6; _bl = list(range(_ns))
    if _ns > 2:   # control A: intercambio FORZADO del slot profundo con un distractor (nunca degenera en la identidad)
        _a = 1 + int(_rng3.integers(_ns - 2)); _bl[_a], _bl[_ns - 1] = _bl[_ns - 1], _bl[_a]
    _PERM = np.concatenate([np.arange(6 * b, 6 * b + 6) for b in _bl]).astype(np.int64)
    zp = np.zeros(nkmax); zn = np.zeros(nkmax); ncf = np.zeros(nkmax, dtype=np.int64)   # B3: normalizadores y conflictos
    el = np.zeros_like(Wl); tr = np.zeros(9)
    pos = 0; E = 1.0; objs = {}; tipos = ['A', 'B']; last = ('B',) * kprof   # 3T-k: historia h1..hk
    splits = 0; split_t = []; deaths = 0; Rtot = 0.0; t_pool = None
    t_techo = None; n_techo = 0   # mordida del techo (solo lectura)
    _rech = {}   # v9: memoria de trabajo de rechazo (posicion -> paso hasta el que no es objetivo)

    def spawn():
        while len(objs) < nobj:
            x = int(rng.integers(L))
            if x not in objs: objs[x] = tipos[int(rng.integers(len(tipos)))]
    spawn()

    q = lambda t: min(t // (T // 4), 3)
    # contadores por cuarto: mordidas por situacion, visitas, mordidas cuyo anterior fue B
    n_AB = [0]*4; n_AA = [0]*4; n_B = [0]*4; prevB = [0]*4; nbit = [0]*4
    vis = {'A': [0]*4, 'B': [0]*4}
    e_AB = e_AA = e_B = e_prevB = e_nbit = 0        # ventana temprana t<early
    snaps = []

    def snapshot(t):
        cods = {f'{c}|{p}': code(inp(c, p)) for c, p in SIT}   # relleno canonico (k=1: exacto)
        _ov = lambda c: [len(code(inp(c, hist_de('A', f))) & code(inp(c, hist_de('B', f)))) for f in FILL]
        _sA = _ov('A'); _sB = _ov('B')
        union = set().union(*cods.values())
        wi = float(KW[activa, :6].sum()); wt = float(KW[activa, 6:].sum()) if NIN > 6 else 0.0
        idx = sorted(union)
        wi_c = float(KW[idx, :6].sum()); wt_c = float(KW[idx, 6:].sum()) if NIN > 6 else 0.0
        return dict(t=t, solap_A=(_sA[0] if kprof == 1 else round(float(np.mean(_sA)), 3)),
                    solap_B=(_sB[0] if kprof == 1 else round(float(np.mean(_sB)), 3)),
                    w_inst=round(wi, 3), w_temp=round(wt, 3),
                    rho=round(wt / (wt + wi), 4) if (wt + wi) > 0 else 0.0,
                    w_inst_cod=round(wi_c, 3), w_temp_cod=round(wt_c, 3),
                    rho_cod=round(wt_c / (wt_c + wi_c), 4) if (wt_c + wi_c) > 0 else 0.0,
                    celdas=int(activa.sum()), splits=splits,
                    W={f'{c}|{p}': (round(valor_tot(inp(c, p)), 3) if kprof == 1 else round(float(np.mean([valor_tot(inp(c, hist_de(p, f))) for f in FILL])), 3)) for c, p in SIT})   # 3T-k: media sobre rellenos

    def see():
        best = None
        for x, k in objs.items():
            if memoria_rechazo and _rech.get(x, -1) > t: continue   # v9
            dl = (pos - x) % L; dr = (x - pos) % L; d = min(dl, dr)
            if best is None or d < best[0]: best = (d, k, dl < dr)
        if best is None:   # v9: todo filtrado -> regla original
            for x, k in objs.items():
                dl = (pos - x) % L; dr = (x - pos) % L; d = min(dl, dr)
                if best is None or d < best[0]: best = (d, k, dl < dr)
        return best

    hitos = {0, T // 4, T // 2, 3 * T // 4}
    for t in range(T):
        if t in hitos: snaps.append(snapshot(t))
        hambre = np.clip(1 - E, 0, 1); d, k, left = see(); pat = PAT[k]
        x = np.concatenate([pat * 1.2, [1.5 if left else 0, 0 if left else 1.5, 1.0 if d == 0 else 0.]])
        noise = .15 + .5 * hambre
        V = Wl @ x; p = 1 / (1 + np.exp(-(V - .8) / noise)); u = p + rng.normal(0, .3, 2); m = np.zeros(2)
        if u.max() > .5: m[np.argmax(u)] = 1
        tr = tr * .7 + x
        if learn: el = el * tau_e + np.outer(m - p, tr)
        pos = (pos + int(m[1] - m[0])) % L; d2, _, _ = see(); Rp = .2 if d2 < d else 0.
        R = 0.
        if pos in objs:
            kk = objs[pos]
            prev_sent = (tuple(('A', 'B')[int(rng2.integers(2))] for _ in range(kprof)) if falso else last)   # 3T-k
            P = inp(kk, prev_sent); kc = kenyon(P); Wb = Wp - Wn; _wf = float(Wb @ kc); _ws = float((Wps - Wns) @ P)   # v13
            _wt = _wf + _ws if puerta is None else (_wf if _fam(kc) else _ws)   # v13
            Vb = alpha * _wt + hambre_boca * hambre + .5
            pb = 1 / (1 + np.exp(-Vb / .3)); mordio = rng.random() < pb
            vis[kk][q(t)] += 1
            if memoria_rechazo and not mordio: _rech[pos] = t + memoria_rechazo   # v9
            if mordio:
                _ky=_key(kc)
                if _ky not in ncod: _ord.append(_ky)
                ncod[_ky]=ncod.get(_ky,0)+1   # B: evidencia del codigo exacto
                est = valor(kk, last)                        # el MUNDO usa el last real
                R = R_VAL[est]; E = min(E + E_VAL[est], 1.5); Rtot += R
                qq = q(t); nbit[qq] += 1
                if last[-1] == 'B': prevB[qq] += 1
                if kk == 'A':
                    (n_AB if last[-1] == 'B' else n_AA)[qq] += 1
                else:
                    n_B[qq] += 1
                if t < early:
                    e_nbit += 1
                    if last[-1] == 'B': e_prevB += 1
                    if kk == 'A':
                        if last[-1] == 'B': e_AB += 1
                        else: e_AA += 1
                    else: e_B += 1
                last = (kk,) + last[:-1]   # 3T-k: desplaza la historia
                del objs[pos]; spawn()
                _rech.pop(pos, None)   # v9
                if learn:
                    dlt = R - _wt if puerta is None else R - _wf   # v13: sin puerta un error compartido; con puerta cada via el suyo
                    if eta_s:   # v13: via lenta
                        _ds = dlt if puerta is None else R - _ws
                        if lam: _mcs = np.minimum(Wps, Wns) * (P > 0); Wps = Wps - lam * _mcs; Wns = Wns - lam * _mcs
                        _Pl = P * (M0 / float(P @ P)) if norm_lenta else P   # n7: norm_lenta (paso repartido por la masa de la entrada)
                        if _ds > 0: Wps = np.clip(Wps + eta_s * _ds * _Pl, 0, clip_s)
                        else:       Wns = np.clip(Wns + eta_s * aversion * (-_ds) * _Pl, 0, clip_s)
                    if lam: ix=kc>0; mcom=np.minimum(Wp[ix],Wn[ix]); Wp[ix]-=lam*mcom; Wn[ix]-=lam*mcom   # drenaje de la parte comun, identico a organismo_v8
                    _ix = kc > 0
                    if dlt > 0: _trunca = bool(((Wp[_ix] + eta * dlt) > wclip).any())
                    else:       _trunca = bool(((Wn[_ix] + eta * aversion * (-dlt)) > wclip).any())
                    if _trunca:
                        n_techo += 1
                        if t_techo is None: t_techo = t
                    if dlt > 0: Wp = np.clip(Wp + eta * dlt * kc, 0, wclip)
                    else:       Wn = np.clip(Wn + eta * aversion * (-dlt) * kc, 0, wclip)
                    if plast:
                        idx = np.where(kc > 0)[0]
                        t_act[idx] = t; n_act[idx] += 1   # B: solo lectura si recic=0
                        err[idx] = (1 - ema) * err[idx] + ema * abs(dlt)
                        mu[idx] = (1 - ema) * mu[idx] + ema * P
                        mus[idx] = (1 - ema) * mus[idx] + ema * (P if dlt > 0 else -P)   # B2: traza error-firmada
                        if R > 0:   # B3: media condicionada de la clase positiva (con su normalizador)
                            mup[idx] = (1 - ema_c) * mup[idx] + ema_c * P; zp[idx] = (1 - ema_c) * zp[idx] + ema_c
                        elif R < 0:
                            mun[idx] = (1 - ema_c) * mun[idx] + ema_c * P; zn[idx] = (1 - ema_c) * zn[idx] + ema_c
                        for c in idx:
                            if div_signo:   # v11: conflicto de signo, hija ciega fuera de P, madre fija, fision del valor
                                dist = P - (mu[c] * (P.sum() / max(float(mu[c].sum()), 1e-9)) if mu_norm else mu[c])
                                if mask_rel in (2, 3, 4) and zp[c] > 1e-6 and zn[c] > 1e-6:   # B3: contexto O discriminador
                                    _mp = mup[c] / float(zp[c]); _mn = mun[c] / float(zn[c])
                                    _rel = (np.abs(_mp - _mn) > del_s) | (np.minimum(_mp, _mn) > 1.0 - del_c)
                                    if mask_rel == 3: _rel = _rel[_PERM]   # control A: el bloque del slot profundo se intercambia con el de un distractor (slot equivocado, misma forma)
                                    elif mask_rel == 4:   # control B: misma CANTIDAD de pixeles, posiciones al azar (separa 'menos pixeles' de 'los pixeles correctos')
                                        _nk = int(_rel.sum()); _rel = np.zeros(NIN, bool); _rel[_rng3.choice(NIN, _nk, replace=False)] = True
                                    _rel = (P > 0) & _rel
                                elif mask_rel == 1:   # B2: ciega fuera de lo RELEVANTE (traza firmada; refutada por plano)
                                    _mnr = mu[c] * (P.sum() / max(float(mu[c].sum()), 1e-9))
                                    _rel = (P > 0) & ((np.abs(mus[c]) > del_s) | (_mnr > 1.0 - del_c))
                                else:
                                    _rel = (P > 0)
                                kj = np.clip(KW[c] * (1 - 0.05) + paso * dist, 0, 5) * _rel
                                _conf = bool((Wb[c] * R < 0 or (desambiguar and R == 0)) and abs(float(Wb[c])) > 0.2)   # B3 + B-5 (tronco v14.2)
                                if _conf: ncf[c] += 1
                                if _conf and ncf[c] >= n_cf and float(kj @ P) > float(KW[c] @ P):
                                    _libre = bool((~activa).any()); _j2 = -1
                                    if not _libre:
                                        div_bloq += 1
                                        if recic:   # RCI: celda invisible a la puerta (|Wp-Wn|<=0.2), fria (t-t_act>tau_r), fuera del codigo; LRU
                                            _msk = activa & (np.abs(Wp - Wn) <= 0.2) & ((t - t_act) > tau_r)
                                            _msk[idx] = False
                                            _cd = np.where(_msk)[0]
                                            if _cd.size: _j2 = int(_cd[int(np.argmin(t_act[_cd]))]); n_recic += 1
                                    if _libre or _j2 >= 0:
                                        a1 = float(np.abs(dist).sum())
                                        ft = float(np.abs(dist[6:]).sum()) / a1 if (a1 > 0 and NIN > 6) else 0.0
                                        j = int(np.where(~activa)[0][0]) if _libre else _j2; activa[j] = True; KW[j] = kj
                                        if R > 0: Wp[j] = Wp[c]; Wn[j] = 0.; Wp[c] = 0.
                                        elif R < 0: Wn[j] = Wn[c]; Wp[j] = 0.; Wn[c] = 0.
                                        else: Wp[j] = 0.; Wn[j] = 0.; n_des += 1   # B-5: con R==0 la hija nace SIN valor y la madre conserva el suyo
                                        mu[j] = P * (float(mu[c].sum()) / P.sum()); err[c] = err[j] = 0; splits += 1; t_act[j] = t
                                        mus[j] = mus[c].copy()   # B2: la hija hereda la traza firmada de la madre
                                        mup[j] = mup[c].copy(); mun[j] = mun[c].copy(); zp[j] = zp[c]; zn[j] = zn[c]   # B3
                                        ncf[c] = 0; ncf[j] = 0   # B3: el conflicto queda resuelto
                                        if len(div_diag) < 400:
                                            div_diag.append((t, int(_rel.sum()), int((P > 0).sum()),
                                                             [round(float(np.abs((mup[c] / max(float(zp[c]), 1e-9) - mun[c] / max(float(zn[c]), 1e-9))[6 * _i:6 * _i + 6]).max()), 3) for _i in range(NIN // 6)],
                                                             [round(float(np.minimum(mup[c] / max(float(zp[c]), 1e-9), mun[c] / max(float(zn[c]), 1e-9))[6 * _i:6 * _i + 6].max()), 3) for _i in range(NIN // 6)],
                                                             int(ncf[c])))
                                        split_t.append((t, kk, (prev_sent[0] if kprof == 1 else prev_sent), round(ft, 3)))
                                        if not (~activa).any() and t_pool is None: t_pool = t
                            elif err[c] > theta and (~activa).any():
                                j = int(np.where(~activa)[0][0]); activa[j] = True
                                dist = P - (mu[c] * (P.sum() / max(float(mu[c].sum()), 1e-9)) if mu_norm else mu[c])   # v10
                                a1 = float(np.abs(dist).sum())
                                ft = float(np.abs(dist[6:]).sum()) / a1 if (a1 > 0 and NIN > 6) else 0.0
                                KW[j] = np.clip(KW[c] + paso * dist, 0, 5)
                                KW[c] = np.clip(KW[c] - paso * dist, 0, 5)
                                Wp[j] = Wp[c]; Wn[j] = Wn[c]; mu[j] = mu[c].copy()
                                err[c] = err[j] = 0; splits += 1
                                split_t.append((t, kk, (prev_sent[0] if kprof == 1 else prev_sent), round(ft, 3)))
                                if not (~activa).any() and t_pool is None: t_pool = t
        E -= costo
        if rng.random() < .003 and objs:
            _dx = list(objs)[int(rng.integers(len(objs)))]; del objs[_dx]; spawn(); _rech.pop(_dx, None)   # v9
        if learn: Wl = np.clip(Wl + eta * (1 + 2 * hambre) * (max(R, 0) + Rp) * el, 0, 1.5)
        if E <= 0: deaths += 1; E = .6; pos = int(rng.integers(L))

    snaps.append(snapshot(T))
    fin = snaps[-1]
    acc = [ (n_AB[i] / (n_AB[i] + n_AA[i])) if (n_AB[i] + n_AA[i]) > 0 else None for i in range(4) ]
    base = [ (prevB[i] / nbit[i]) if nbit[i] > 0 else None for i in range(4) ]
    lift = [ (round(acc[i] - base[i], 4) if acc[i] is not None and base[i] is not None else None) for i in range(4) ]
    e_acc = (e_AB / (e_AB + e_AA)) if (e_AB + e_AA) > 0 else None
    e_base = (e_prevB / e_nbit) if e_nbit > 0 else None
    return dict(
        seed=seed, arm=arm, k=kprof,
        W=fin['W'], sep=round(fin['W']['A|B'] - fin['W']['A|A'], 3),
        comp={f'{c}|{p}': (round(float(Wp @ kenyon(inp(c, p))), 3),
                           round(float(Wn @ kenyon(inp(c, p))), 3)) for c, p in SIT},
        solap_A=fin['solap_A'], solap_B=fin['solap_B'],
        solap_A_q=[s['solap_A'] for s in snaps],
        rho_q=[s['rho'] for s in snaps], rho_cod_q=[s['rho_cod'] for s in snaps],
        w_temp_q=[s['w_temp'] for s in snaps], w_inst_q=[s['w_inst'] for s in snaps],
        n_AB=n_AB, n_AA=n_AA, n_B=n_B, vis=vis, nbit=nbit, prevB=prevB,
        acc=[round(a, 4) if a is not None else None for a in acc],
        base=[round(b, 4) if b is not None else None for b in base], lift=lift,
        early=dict(n_AB=e_AB, n_AA=e_AA, n_B=e_B, nbit=e_nbit,
                   acc=round(e_acc, 4) if e_acc is not None else None,
                   base=round(e_base, 4) if e_base is not None else None,
                   lift=round(e_acc - e_base, 4) if (e_acc is not None and e_base is not None) else None),
        splits=splits, celdas=fin['celdas'], t_pool=t_pool, split_t=split_t,
        deaths=deaths, Rtot=round(Rtot, 2), E_fin=round(float(E), 3), snaps=snaps,
        t_techo=t_techo, n_techo=n_techo, lam=lam,
        div_bloq=int(div_bloq), n_recic=int(n_recic), recic=recic, tau_r=tau_r,
        mask_rel=mask_rel, div_diag=div_diag, n_cf=n_cf,
        puerta_pat=puerta_pat, pat_shuf=pat_shuf, pat_min=pat_min, n_cod=len(ncod),
        desambiguar=desambiguar, norm_lenta=norm_lenta, n_des=int(n_des),
        Wps_sum=round(float(Wps.sum()), 4), Wns_sum=round(float(Wns.sum()), 4),
        diag=dict(act=int(activa.sum()),
                  inv=int(((np.abs(Wp - Wn) <= 0.2) & activa).sum()),
                  frias=int((activa & ((T - t_act) > 10000)).sum()),
                  recic_ok=int((activa & (np.abs(Wp - Wn) <= 0.2) & ((T - t_act) > 10000)).sum()),
                  nunca=int((activa & (t_act < 0)).sum()),
                  n_act_med=float(np.median(n_act[activa])),
                  hist=[int(((np.abs(Wp - Wn) > b0) & (np.abs(Wp - Wn) <= b1) & activa).sum())
                        for b0, b1 in [(-1.0, 0.05), (0.05, 0.2), (0.2, 0.5), (0.5, 1.0), (1.0, 3.1)]]))


if __name__ == '__main__':
    import json
    print(json.dumps({k: run(1, k, T=20000)['W'] for k in ARMS}, indent=1))
