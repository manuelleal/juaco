"""organismo_v142gN = organismo/organismo_v142g.py (9e5f566cd6a7a4d2, CONGELADO: solo se leyo) + perilla norm_lenta (via lenta normalizada
por masa: el paso se multiplica por 3.0/(P.P)). Generado por experimentos/subida_n7/construye_n7.py. NO editar.
Con norm_lenta=0 es organismo_v142g.py EXACTO; con norm_lenta=1 se PREDICE identico en los mundos del tronco (P.P = 3).
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.05,memoria_rechazo=20,mu_norm=True,div_signo=True,eta_s=0.0,clip_s=3.0,puerta=None,mundo='AB',regla='px0',fase2_en=None,sonda_final=False,mask_rel=0,del_s=0.25,del_c=0.25,ema_c=0.05,puerta_pat=0,pat_shuf=0,pat_min=0,desambiguar=1,norm_lenta=0):
            ...
            if mordio:
                ...
                if learn:
                    ...
                    if eta_s:   # v13: actualizacion de la via lenta (su tasa, mismo drenaje)
                        ...
                        _Pl=P_[kk]*(3.0/float(P_[kk]@P_[kk])) if norm_lenta else P_[kk]   # n7: norm_lenta
