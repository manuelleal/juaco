"""organismo_v142N = organismo/organismo_v142.py (17528d767fcebaf6, CONGELADO: solo se leyo) + perilla norm_lenta (via lenta normalizada
por masa: el paso se multiplica por 3.0/(P.P)). Generado por experimentos/subida_n7/construye_n7.py. NO editar.
Con norm_lenta=0 es organismo_v142.py EXACTO; con norm_lenta=1 se PREDICE identico en los mundos del tronco (P.P = 3).
Arnes: experimentos/subida_n7/identidad_n7.py."""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.05,memoria_rechazo=20,mu_norm=True,div_signo=True,eta_s=0.15,clip_s=10.0,puerta=3,mask_rel=2,del_s=0.25,del_c=0.25,ema_c=0.05,puerta_pat=5,pat_shuf=0,pat_min=1,desambiguar=1,norm_lenta=0):
            ...
            if mordio:
                ...
                if learn:
                    ...
                    if eta_s:   # v13: actualizacion de la via lenta (su tasa, mismo drenaje)
                        ...
                        _Pl=PAT[kk]*(3.0/float(PAT[kk]@PAT[kk])) if norm_lenta else PAT[kk]   # n7: norm_lenta
