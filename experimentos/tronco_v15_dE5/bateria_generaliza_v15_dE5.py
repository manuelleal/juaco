"""bateria_generaliza_v15_dE5 = organismo/bateria_generaliza_v142.py (e5929942647756a5, CONGELADA: solo se leyo) con DOS
entradas nuevas en INSTRUMENTOS (organismo_v15_dE5_on -> organismo_v15_dE5g_on, y el control OFF), con los DIEZ
campos de la entrada del tronco organismo_v142 CAMPO A CAMPO (regla 14) mas las cuatro perillas de la dosis
EXPLICITAS. Umbrales G1/G2/K sin tocar. Salida datos/regresion_generaliza_dE5_*.
...
RAIZ = os.path.dirname(os.path.dirname(AQUI))   # dE5: experimentos/tronco_v15_dE5/ -> bundle/
...
REGLAS = ['px0', 'azar']
N_PARALELO = int(os.environ.get('JUACO_POOL', 14))   # ERR-86: el tamano del Pool se fija por JUACO_POOL
...
INSTRUMENTOS = {
    ...
    'organismo_v15_dE5_on': ('organismo_v15_dE5g_on', dict(eta_s=0.15, clip_s=10.0, puerta=3, mask_rel=2, del_s=0.25, del_c=0.25, ema_c=0.05, puerta_pat=5, pat_shuf=0, pat_min=1, desambiguar=1, eta_pred=0.03, ema_pred=0.05, clip_e=3.0, k_sorp=5.0)),   # dE5 ON: los DIEZ campos de la entrada del tronco, IDENTICOS (regla 14), + las CUATRO perillas de la dosis EXPLICITAS (ERR-38)
    'organismo_v15_dE5': ('organismo_v15_dE5g', dict(eta_s=0.15, clip_s=10.0, puerta=3, mask_rel=2, del_s=0.25, del_c=0.25, ema_c=0.05, puerta_pat=5, pat_shuf=0, pat_min=1, desambiguar=1, eta_pred=0.0, ema_pred=0.05, clip_e=3.0, k_sorp=0.0)),   # dE5 OFF (control pareado): los MISMOS campos con la dosis a cero
}
...
class _Serial:   # --sin-pool: HUMO de UN proceso que llega a ESCRIBIR el JSON (ERR-42) sin abrir ningun Pool.
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv:
        _log['f'] = open(os.path.join(RAIZ, 'datos', f'regresion_generaliza_dE5_{modulo}_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    inst = INSTRUMENTOS[modulo][0]
    log(f"=== REGRESIÓN DE GENERALIZACIÓN (Etapa 3) — {modulo}, {S} semillas ({seeds[0]}..{seeds[-1]}), {'SIN POOL (--sin-pool)' if '--sin-pool' in sys.argv else f'Pool({N_PARALELO})'} ===")
    _dir = {'organismo_v14g': os.path.join(RAIZ, 'organismo'), 'organismo_v142g': os.path.join(RAIZ, 'organismo'), 'organismo_v15_dE5g_on': AQUI, 'organismo_v15_dE5g': AQUI, 'organismo_v11g': GEN, 'organismo_v13g': os.path.join(RAIZ, 'experimentos', 'v13_dos_vias'),
    ...
    log(f"instrumento {inst} {h16(os.path.join(_dir, inst + '.py'))}"
        f"  organismo {h16(os.path.join(AQUI if os.path.exists(os.path.join(AQUI, modulo + '.py')) else os.path.join(RAIZ, 'organismo'), (modulo if modulo != 'organismo_v13_rapido' else 'organismo_v13') + '.py'))}  esta bateria {h16(os.path.abspath(__file__))}")
    trabajos = [(modulo, rg, s) for rg in REGLAS for s in seeds]
    res = []
    with _pool(mp) as pool:
    ...
    if _log['f']:
        dj = os.path.join(RAIZ, 'datos', f'regresion_generaliza_dE5_{modulo}_{stamp}.json')
        json.dump(dict(meta=dict(fecha=time.strftime('%Y-%m-%dT%H:%M:%S'), modulo=modulo, semillas=seeds, veredictos=V,
                                 sha_bateria=h16(os.path.abspath(__file__)), sha_organismo=h16(os.path.join(AQUI, modulo + '.py')) if os.path.exists(os.path.join(AQUI, modulo + '.py')) else h16(os.path.join(RAIZ, 'organismo', modulo + '.py')),
