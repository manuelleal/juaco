"""bateria_v15_dE5 = organismo/bateria_v142.py (6375d90e531b06e6, CONGELADA: solo se leyo) sobre **organismo_v15_dE5_on**
(la dosis ENCENDIDA: el examen mide AL CANDIDATO). Las SEIS etapas, los CRIT importados y los umbrales del
criterio v3' quedan INTACTOS. Salida datos/examen_dE5_*. Generado por construye_v15_dE5.py. NO editar."""
"""bateria_v142 = organismo/bateria_v14.py (72216f5415de0c86, CONGELADA: solo se leyo) apuntando a **organismo_v142**
...
"""bateria_v14 = examen v3' completo sobre organismo_v14 (TRONCO). Copia de experimentos/nivel10_composicion_v14/bateria_v14c.py
(94d035c5bfffa2e1), generada por anclas desde bateria_v13 (CONGELADA) cambiando solo el modulo. CONGELADA.
"""
"""bateria_v14c = organismo/bateria_v13.py (1a027bcb37eb536e, CONGELADO: solo se leyo) apuntando a
...
_D14 = os.path.dirname(os.path.abspath(__file__))   # tronco: organismo/
AQUI = _D14   # tronco: organismo/
...
def tarea_id(args):
    cual, esc, seed = args
    import organismo_v15_dE5_on as v13   # dE5: el CANDIDATO (v14.2 + sorpresa del mundo en la boca, k_sorp=5)
    if cual == 'v11':
        ...
        a, b = ref.run(seed, **ESC_ID[esc]), v13.run(seed, eta_s=0.0, puerta=None, div_signo=False, eta_pred=0.0, k_sorp=0.0, sesgo_fijo=0.0, **ESC_ID[esc])   # ERR-30: apaga TAMBIEN la dosis
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv:
        _log['f'] = open(os.path.join(RAIZ, 'datos', f'examen_dE5_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    prereg = os.path.join(RAIZ, 'experimentos', 'v13_dos_vias', 'PREREGISTRO_tronco_v13.md')
    log(f"=== Batería v13 — CRITERIO v3, {S} semillas ({seeds[0]}..{seeds[-1]}), {'SIN POOL (--sin-pool)' if '--sin-pool' in sys.argv else f'Pool({N_PARALELO})'} ===")
    log(f"sha organismo_v13 {h16(os.path.join(_D14, 'organismo_v15_dE5_on.py'))}  bateria_v13 {h16(os.path.abspath(__file__))}"
    ...
    if _log['f']:
        dj = os.path.join(RAIZ, 'datos', f'examen_dE5_{stamp}.json')
        meta = dict(fecha=time.strftime('%Y-%m-%dT%H:%M:%S'), semillas=S, semilla_inicial=seeds[0], veredictos=V, congelar=bool(todo and S >= 20),
                    sha_organismo_v13=h16(os.path.join(_D14, 'organismo_v15_dE5_on.py')), sha_bateria_v13=h16(os.path.abspath(__file__)),
                    sha_preregistro=h16(prereg), sha_organismo_v11=h16(os.path.join(RAIZ, 'organismo', 'organismo_v11.py')), sha_organismo_v10=h16(os.path.join(RAIZ, 'organismo', 'organismo_v10.py')),
