"""organismo_v15_dE5g = organismo/organismo_v142g.py (9e5f566cd6a7a4d2, instrumento de MUNDO DE REGLA del tronco v14.2;
SOLO se leyo) + el predictor de dE del bloque 6 y su sorpresa en la BOCA (k_sorp), escrito con las MISMAS
operaciones de numpy que organismo_vivo con n_nec=1 (Wpe (1,6), Wke (1,NKMAX), _sbE (1,)) para que las dos
implementaciones coincidan BIT A BIT. Con eta_pred=0 y k_sorp=0 es organismo_v142g EXACTO.
AVISO (regla 14): sus defectos de via lenta son eta_s=0.0, clip_s=3.0, puerta=None, mask_rel=0, puerta_pat=0;
la entrada de INSTRUMENTOS los pasa EXPLICITOS, campo a campo como la del tronco organismo_v142.
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.05,memoria_rechazo=20,mu_norm=True,div_signo=True,eta_s=0.0,clip_s=3.0,puerta=None,mundo='AB',regla='px0',fase2_en=None,sonda_final=False,mask_rel=0,del_s=0.25,del_c=0.25,ema_c=0.05,puerta_pat=0,pat_shuf=0,pat_min=0,desambiguar=1,eta_pred=0.0,ema_pred=0.05,clip_e=3.0,k_sorp=0.0,sesgo_fijo=0.0):
    ...
    return dict(sbarE=[round(float(_x),6) for _x in _sbE],sesgo_fijo=sesgo_fijo,sesgo_q=[(round(_sgq[_i]/_nsq[_i],6) if _nsq[_i] else 0.0) for _i in range(4)],enc_q=list(_nsq),desambiguar=desambiguar,des_splits=_ndes,des_t=_des_t,sobre=sobre,llegadas=llegadas,sin_objetivo=sin_objetivo,memoria_rechazo=memoria_rechazo,err_max=err_max,t_conflicto=t_conflicto,t_techo=t_techo,n_techo=n_techo,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),puerta_pat=puerta_pat,pat_shuf=pat_shuf,pat_min=pat_min,n_cod=len(ncod),
