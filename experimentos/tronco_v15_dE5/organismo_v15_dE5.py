"""organismo_v15_dE5 = experimentos/nivel11_mundo_vivo/organismo_vivo_rep2.py (96feb4918dc5d694; SOLO se leyo; cadena
organismo_vivo 20c0961c79de8825 <- organismo/organismo_v14.py feefc88b1fd8d434 = v14.1) + B-5 `desambiguar` (el organo
de v14.2, texto literal de creacion_B/construye_codigo.py, POR DEFECTO 1 como el TRONCO organismo_v142
17528d767fcebaf6) + `invertir_vivo_en` (T-C en el mundo vivo) + las lecturas de RECUPERACION t_ext_B / mord_post /
deaths_post / sbarE (SOLO LECTURA, no consumen rng).
LA PERILLA DEL CANDIDATO YA ESTABA AQUI: eta_pred / ema_pred / clip_e (predictor de dE del bloque 6) y k_sorp
(su sorpresa en la BOCA). dE5 = eta_pred=0.03, ema_pred=0.05, k_sorp=5.0. Con eta_pred=0 y k_sorp=0:
  vivo=0, n_nec=1, reproduccion=0, rep2=0  ->  organismo/organismo_v142.py BIT A BIT (claves de v14.2)
  desambiguar=0                            ->  organismo_vivo_rep2.py BIT A BIT (claves de rep2)
Arnes: identidad_v15_dE5.py. Generado por construye_v15_dE5.py. NO editar a mano."""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
    ...
    t_ext_B=None; deaths_post=0; mord_post={}; _sgq=[0.]*4; _nsq=[0]*4   # dE: M1 del bloque 6 (recuperacion), bocados/muertes TRAS el cambio, y el SESGO DE LA BOCA por cuarto. SOLO LECTURA
    ...
    for t in range(T):
        ...
        if invertir_vivo_en is not None and t==invertir_vivo_en: _EF=dict(_EF,comida=_EF['veneno'],veneno=_EF['comida'])   # T-C vivo: la comida pasa a quitar energia y el veneno a darla
        ...
        if pos in objs:
            ...
            if mordio:
                ...
                mord[kk][q(t)]+=1
                if invertir_en is not None and t>=invertir_en: mord_post[val[kk]]=mord_post.get(val[kk],0)+1   # dE: bocados TRAS el cambio, por valencia NUEVA
        ...
        if invertir_en is not None and t>=invertir_en and t_ext_B is None and valor(PAT['B'])>=0: t_ext_B=t   # dE: M1 del bloque 6 (recuperacion). No consume RNG
        ...
        if E<=0 or (vivo and Ag<=0):   # VIVO: DOS muertes posibles (con vivo=0 la segunda es imposible: Ag=A_ini y no baja)
            ...
            if invertir_en is not None and t>=invertir_en: deaths_post+=1   # dE
            if vivo: Ag=.6
    ...
    return dict(desambiguar=desambiguar,des_splits=_ndes,des_t=_des_t,t_ext_B=t_ext_B,mord_post=dict(mord_post),deaths_post=deaths_post,sbarE=[round(float(_x),6) for _x in _sbE],invertir_vivo_en=invertir_vivo_en,sesgo_fijo=sesgo_fijo,sesgo_q=[(round(_sgq[_i]/_nsq[_i],6) if _nsq[_i] else 0.0) for _i in range(4)],enc_q=list(_nsq),sobre=sobre,llegadas=llegadas,sin_objetivo=sin_objetivo,memoria_rechazo=memoria_rechazo,err_max=err_max,t_conflicto=t_conflicto,t_techo=t_techo,n_techo=n_techo,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),puerta_pat=puerta_pat,pat_shuf=pat_shuf,pat_min=pat_min,n_cod=len(ncod),
