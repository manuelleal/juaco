"""organismo_B.py — GENERADO POR construye_B.py DESDE organismo_f9c.py (9dd1fb91ecec35ae).
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
    ...
    if nodo_via not in (0,1,2,3): raise SystemExit('F9C/B: nodo_via es 0 (solo la via lenta = organismo_f9), 1 (las DOS vias CON evidencia = organismo_f9b), 2 (las dos vias SIN evidencia: leer llena la memoria, morder abre la puerta = organismo_f9c) o 3 (como 2 y ademas LEER HACE CRECER: plasticidad y division por conflicto)')
    ...
    D=6 if not retina else int(retina)   # B: dimension de la retina (retina=0 -> 6 = v14 EXACTO)
    ...
    cond=lambda: bool(mundoB) or len(code(PAT['A'])&code(PAT['B']))==objetivo_AB and (nuevo is None or solap_B is None or (len(code(PAT[nuevo])&code(PAT['B']))==solap_B and len(code(PAT[nuevo])&code(PAT['A']))==0))
    while not cond(): KW[objetivo_AB:NK]=rng.uniform(0,1,(NK-objetivo_AB,D))
    def kenyon(P): k=np.zeros(NKMAX); k[list(code(P))]=1; return k
    Wp=np.zeros((n_nec,NKMAX)); Wn=np.zeros((n_nec,NKMAX)); err=np.zeros(NKMAX); mu=np.zeros((NKMAX,D)); splits=0; el=np.zeros_like(Wl); tr=np.zeros(D+3)
    Wps=np.zeros((n_nec,D)); Wns=np.zeros((n_nec,D))   # v13: via LENTA, lineal sobre la retina (inerte si eta_s=0). VIVO: una fila por necesidad
    ...
    Wpe=np.zeros((n_nec,D)); Wke=np.zeros((n_nec,NKMAX)); _sbE=np.zeros(n_nec)   # VIVO: predictor VECTORIAL de dS y SORPRESA POR NECESIDAD (inerte con eta_pred=0)
    ...
    _IDX={_k2:_i2 for _i2,_k2 in enumerate(PAT)}   # B: con mundoB=0, PAT es A,B,C,D en ese orden (identico)
    ...
    tipos=(list(PAT) if mundoB else (list(estims) if estims else ['A','B']))   # B/VIVO: con dos estimulos, spawn() sortea igual que v14
    ...
    def _nace(_k,_m):   # H1: NACE UN CUERPO NUEVO. Memoria vacia + lo que `hereda` deje pasar. rng propio (ERR-60). El mundo (objs, tipos, val) NO se toca
        ...
        Wl[:]=_rh.uniform(.1,.4,(2,D+3)); el[:]=0; tr[:]=0   # la politica de locomocion NO se hereda en ningun brazo
        ...
        if _m is not None and 'Wps' in _m:
            if hereda=='baraja':
                _pm=_rb.permutation(D); _nbar+=int(bool((_pm==np.arange(D)).all()))   # misma magnitud, pixeles equivocados
        ...
        if E<=0 or (vivo and Ag<=0):   # VIVO: DOS muertes posibles (con vivo=0 la segunda es imposible: Ag=A_ini y no baja)
            ...
            if alma is not None:   # ALMA: (1) el NODO se llena con lo que el cuerpo que muere aprendio; (2) el alma elige UNA curita del MENU CERRADO
                ...
                if f9:   # F9: LAS MEDIDAS DEL CUERPO QUE MUERE, de _exph/_mordh ANTES de limpiarlas. Todo SOLO LECTURA.
                    ...
                    _mal9=[_x9 for _x9 in _exph if _x9[2]<2 and _EF[val[_x9[1]]][_x9[2]]<0]   # B: generalizado. En el mundo
            ...
            if muerte_real:   # H1: LA MUERTE BORRA AL INDIVIDUO. Nace el siguiente de la cola; si no hay, el linaje SE EXTINGUIO y el mundo pone un FUNDADOR
                ...
                if alma is not None and _con and nodo and _nodo:   # ALMA: EL NODO CENTRAL. El cuerpo CONECTADO nace con los mensajes de los que
                    ...
                    if nodo_or:   # F9C: NODO ORACULO (cota superior, voto del creador A). El contenido NO es lo que vivio el linaje: es la TABLA VERDADERA
                        ...
                        _or9=[[[float(_z9) for _z9 in PAT[_k9]],float(1.0 if _EF[val[_k9]][_n9]>0 else (-3.0 if _EF[val[_k9]][_n9]<0 else 0.0)),int(_n9)] for _n9 in range(min(n_nec,2)) for _k9 in PAT]   # B: generalizado (con mundoB=0, 'ABCD' exacto)
                        _msg=_or9*max(1,(8*nodo_lee)//len(_or9))   # B: el pool del oraculo es ~8x el presupuesto de lectura
                        #        (con el mundo de ABCD len(_or9)=8 -> (8*50)//8 = 50 copias: EXACTAMENTE el 400 de f9c).
                        #        ORIGINAL: 8*nodo_lee = 400 mensajes con nodo_lee=50: CINCUENTA copias EXACTAS de la tabla, de las que el recien
                    ...
                    for _it9 in range(min(nodo_lee,len(_msg)) if nodo_rel==1 else len(_msg)):
                        ...
                        if nodo_via:   # F9B/F9C: el mensaje entra TAMBIEN por la VIA RAPIDA, con la MISMA regla local de la mordida
                            ...
                            _Wb7=Wp[_n7]-Wn[_n7]; _df7=_R7-float(_Wb7@_kc7)   # B: el valor ANTES de actualizar (como en la mordida).
                            ...
                            _nvia+=1   # nodo_via 1 y 2: LEER NO DIVIDE (organismo_f9b / organismo_f9c EXACTOS). Declarado alli.
                    ...
                    if nodo_via: _fam9.append(int(sum(int(_fam(kenyon(PAT[_z9]),_nm)) for _z9 in PAT)))   # F9B/F9C: SOLO LECTURA (la puerta, recien leido)
    ...
    return dict(sobre=sobre,llegadas=llegadas,sin_objetivo=sin_objetivo,memoria_rechazo=memoria_rechazo,err_max=err_max,t_conflicto=t_conflicto,t_techo=t_techo,n_techo=n_techo,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),puerta_pat=puerta_pat,pat_shuf=pat_shuf,pat_min=pat_min,n_cod=len(ncod),
                solap={'AB':(0 if mundoB else len(code(PAT['A'])&code(PAT['B']))),'nB':len(code(PAT[nuevo])&code(PAT['B'])) if nuevo else None},W_lenta=W_lenta,Wps=[round(float(x),3) for x in Wps[_nm]],Wns=[round(float(x),3) for x in Wns[_nm]],**_ext)
