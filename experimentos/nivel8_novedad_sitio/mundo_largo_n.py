"""mundo_largo_n = mundo_largo.py (9f74ff6b5941e5a5) + NOVEDAD DE SITIO (reloj t_visita por posicion, novedad
min(1,(t-t_visita)/tau_N), sesgo gamma_N por direccion con la retina vacia sobre los sitios que M conoce) + controles
nov_barajada (novedad de otro sitio) y nov_cte (novedad constante). Generado por construye_novedad.py. NO editar.
Con gamma_N=0 es mundo_largo exacto (identidad obligatoria; clave extra nov_diag, diagnostico)."""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
    ...
    def _nov(p):   # novedad de sitio: tiempo sin pisar normalizado a tau_N y topado en 1 (fija en 1.0 en el control constante)
        return 1.0 if nov_cte else min(1.0,float(t-_tvis[p if _perm_n is None else _perm_n[p]])/tau_N)
    ...
    if sitios is not None:
        ...
        if nov_barajada:   # control 1: la novedad de cada sitio se lee en OTRO sitio (misma magnitud y dinamica, atribucion rota); RNG propio, no toca el del organismo
            _ps=np.array(sorted(_SIT)); _perm_n=np.arange(L); _perm_n[_ps]=_ps[np.random.default_rng(seed+900000).permutation(len(_ps))]
    def spawn():
    ...
    for t in range(T):
        ...
        if pos in objs:
            ...
            _tvis[pos]=t; _nvis[pos]+=1   # novedad: PISAR el sitio con el objeto presente lo pone 'visto ahora' (mismo evento que escribe el mapa; no hace falta morder)
    ...
    return dict(sobre=sobre,llegadas=llegadas,sin_objetivo=sin_objetivo,memoria_rechazo=memoria_rechazo,err_max=err_max,t_conflicto=t_conflicto,t_techo=t_techo,n_techo=n_techo,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),
                ...
                nov_diag=dict(tvis={int(x):int(_tvis[x]) for x in _SIT},visitas={int(x):int(_nvis[x]) for x in _SIT}))   # novedad: diagnostico (clave NUEVA, no criterio)
