...
def run(seed,T=120000,eta=0.02,tau_e=0.85,learn=True):
    ...
    W1=rng.uniform(0.1,0.5,(H,NI)); W2=rng.uniform(0.1,0.5,(NM,H)); Wh=rng.uniform(0,0.3,NM); Wd=rng.uniform(0.1,0.5,(NM,NI)); ed=np.zeros_like(Wd)
    ...
    for t in range(T):
        ...
        V=W2@h+Wd@x+Wh*hambre; p=1/(1+np.exp(-(V-2.0)/noise))
        u=p+rng.normal(0,0.3,NM); m=np.zeros(NM)
        if u.max()>0.5: m[np.argmax(u)]=1
        tr=tr*0.7+x; trh=trh*0.7+h
        if learn:
            e2=e2*tau_e+np.outer(m-p,trh); ed=ed*tau_e+np.outer(m-p,tr); e1=e1*tau_e+np.outer(h-ph,tr)*(W2.T@(m-p))[:,None]; eh=eh*tau_e+(m-p)*hambre
        ...
        if learn:
            ...
            W2+=g*e2; W1+=g*e1; Wh+=g*eh; Wd=np.clip(Wd+g*ed,0,1.5)
