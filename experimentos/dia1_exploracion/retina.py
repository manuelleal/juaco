...
def run(seed,T=80000,eta=0.02,tau_e=0.85,learn=True):
    ...
    for t in range(T):
        ...
        Vh=W1@x; ph=1/(1+np.exp(-(Vh-1.0)/noise)); h=(rng.random(H)<ph).astype(float)
        ...
        if pos in objs:
            ...
            else: R=-3.0; E-=0.9; ateB.append(t)
            del objs[pos]; spawn()
        delta=R-Rbar; Rbar=0.99*Rbar+0.01*R
        if learn:
            g=eta*(1+3*hambre)*delta
