...
NUEVAS = {'div_bloq', 'n_recic', 'diag', 'recic', 'tau_r'}
...
if __name__ == '__main__':
    ...
    print(f"gemelo mundo_k_B.py         {hashlib.sha256(open(B.__file__,'rb').read()).hexdigest()[:16]}")
    ok = tot = 0
    for etiq, cfg, seeds in CASOS:
        for s in seeds:
            a = norm(ORI.run(s, **cfg, **V13))
            b = norm(B.run(s, **cfg, **V13, recic=0, tau_r=0))
            tot += 1
            igual = a == b
            ok += igual
            if not igual:
