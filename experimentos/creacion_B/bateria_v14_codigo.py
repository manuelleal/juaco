"""bateria_v14_codigo = organismo/bateria_v14.py (72216f5415de0c86, CONGELADA: solo se leyo) sobre **organismo_v14_codigo_on**
(la perilla ENCENDIDA). Las SEIS etapas, los CRIT y los umbrales del criterio v3' INTACTOS. Salida datos/examen_codigo_*.
...
RAIZ = os.path.dirname(os.path.dirname(AQUI))   # B-5: experimentos/creacion_B/ -> bundle/
...
def tarea_id(args):
    cual, esc, seed = args
    import organismo_v14_codigo_on as v13   # B-5: la perilla ENCENDIDA (el examen mide AL CANDIDATO)
    if cual == 'v11':
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv:
        _log['f'] = open(os.path.join(RAIZ, 'datos', f'examen_codigo_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    ...
    log(f"sha organismo_v13 {h16(os.path.join(_D14, 'organismo_v14_codigo_on.py'))}  bateria_v13 {h16(os.path.abspath(__file__))}"
    ...
    if _log['f']:
        dj = os.path.join(RAIZ, 'datos', f'examen_codigo_{stamp}.json')
        meta = dict(fecha=time.strftime('%Y-%m-%dT%H:%M:%S'), semillas=S, semilla_inicial=seeds[0], veredictos=V, congelar=bool(todo and S >= 20),
                    sha_organismo_v13=h16(os.path.join(_D14, 'organismo_v14_codigo_on.py')), sha_bateria_v13=h16(os.path.abspath(__file__)),
