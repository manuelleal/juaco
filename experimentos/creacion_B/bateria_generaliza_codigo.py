"""bateria_generaliza_codigo = organismo/bateria_generaliza.py (9cf72581ebae7dea, CONGELADA: solo se leyo) con UNA entrada nueva:
organismo_v14_codigo_on -> organismo_v14g_codigo_on, con los MISMOS kwargs que la entrada del tronco (eta_s=0.15,
clip_s=10.0 explicitos). Umbrales G1/G2/K sin tocar. Salida datos/regresion_generaliza_codigo_*.
...
RAIZ = os.path.dirname(os.path.dirname(AQUI))   # B-5
...
INSTRUMENTOS = {
    ...
    'organismo_v14_codigo_on': ('organismo_v14g_codigo_on', dict(eta_s=0.15, clip_s=10.0, puerta=3, mask_rel=2, del_s=0.25, del_c=0.25, ema_c=0.05, puerta_pat=5, pat_shuf=0, pat_min=1)),   # B-5: los MISMOS kwargs que organismo_v14 (eta_s/clip_s explicitos: v14g los tiene apagados por defecto)
}
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv:
        _log['f'] = open(os.path.join(RAIZ, 'datos', f'regresion_generaliza_codigo_{modulo}_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    inst = INSTRUMENTOS[modulo][0]
    log(f"=== REGRESIÓN DE GENERALIZACIÓN (Etapa 3) — {modulo}, {S} semillas ({seeds[0]}..{seeds[-1]}), Pool({N_PARALELO}) ===")
    _dir = {'organismo_v14g': os.path.join(RAIZ, 'organismo'), 'organismo_v14g_codigo_on': AQUI, 'organismo_v11g': GEN, 'organismo_v13g': os.path.join(RAIZ, 'experimentos', 'v13_dos_vias'),
    ...
    if _log['f']:
        dj = os.path.join(RAIZ, 'datos', f'regresion_generaliza_codigo_{modulo}_{stamp}.json')
