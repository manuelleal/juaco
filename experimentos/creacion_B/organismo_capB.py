"""organismo_capB = organismo_capD13.py (fd8e10435801646c) + contador de evidencia por CODIGO EXACTO y
...
def run(seed,T=100000,learn=True,plan=None,pats=None,chk=None,
        ...
        plast=True,theta=0.6,ema=0.02,paso=0.5,lam=0.0,memoria_rechazo=20,mu_norm=False,div_signo=False,eta_s=0.0,clip_s=3.0,puerta=None,puerta_pat=0):
    ...
    return dict(split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),
                ...
                hist=hist,t_agot=t_agot,err_max=err_max,t_entra=t_entra,val=dict(val),t_techo=t_techo,techo_primero=techo_primero,n_techo=n_techo,mv_tot=mv_tot,mc_tot=mc_tot,nofam_fin=nofam_fin,nofam_est=nofam_est,ncod_est=ncod_est,puerta_pat=puerta_pat)
