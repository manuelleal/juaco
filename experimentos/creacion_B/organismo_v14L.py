"""organismo_v14L = organismo/organismo_v14.py (9bab8ac0685b1f21, CONGELADO: solo se leyo) + ORGANO DE
...
"""v14 = TRONCO desde el 18 sep 2026 (05:00): organismo_v13 (cc8b16b492d4d324) + HIJA DISPERSA por relevancia (mask_rel=2,
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.05,memoria_rechazo=20,mu_norm=True,div_signo=True,eta_s=0.015,clip_s=3.0,puerta=3,mask_rel=2,del_s=0.25,del_c=0.25,ema_c=0.05,puerta_pat=5,pat_shuf=0,pat_min=1,sem=0,nh=2000,kh=40,theta_sim=0.15,sem_min=0.3,tol_sem=0.5):
    ...
    return dict(sobre=sobre,llegadas=llegadas,sin_objetivo=sin_objetivo,memoria_rechazo=memoria_rechazo,err_max=err_max,t_conflicto=t_conflicto,t_techo=t_techo,n_techo=n_techo,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),puerta_pat=puerta_pat,pat_shuf=pat_shuf,pat_min=pat_min,n_cod=len(ncod),sem=sem,exp_hasta=dict(exp_hasta),n_mord=dict(_nmord),n_sem=int(n_sem),n_des=int(n_des),sem_log=sem_log,rel_arista=dict(_relia),n_nodos=len(almacen),
