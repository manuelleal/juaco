"""bateria_v13E_k3 = organismo/bateria_v13.py (1a027bcb37eb536e, CONGELADO: solo se leyo) apuntando a
organismo_v13E_k3 (organismo_v13p + dE-TEST fijo ON, dosis k_testE=3) en vez de organismo_v13. Las SEIS
etapas, los CRIT importados y los umbrales del criterio v3' quedan INTACTOS. Salida en
datos/examen_v13E_k3_<fecha>. ERR-30: el CRITERIO 5 esta ADAPTADO igual que en bateria_v13E.py (v3''
para organos en la boca): la reduccion a v11/v10 apaga TAMBIEN k_testE y eta_pred. Ver PREREGISTRO_dosis_dE.md.
Generado por construye_v13E.py --k 3. NO editar. La bateria original NO se modifico."""
...
def tarea_id(args):
    cual, esc, seed = args
    import organismo_v13E_k3 as v13   # dE-TEST dosis k_testE=3 (PREREGISTRO_dosis_dE.md)
    if cual == 'v11':
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv:
        _log['f'] = open(os.path.join(RAIZ, 'datos', f'examen_v13E_k3_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    ...
    log(f"sha organismo_v13 {h16(os.path.join(_D, 'organismo_v13E_k3.py'))}  bateria_v13 {h16(os.path.abspath(__file__))}"
    ...
    with mp.Pool(N_PARALELO) as pool:
        ...
        for c, nombre in (('v11', "v13E_k3(eta_s=0,puerta=None,k_testE=0,eta_pred=0) == v11  [v3'' ERR-30]"), ('v10', "v13E_k3(eta_s=0,puerta=None,div_signo=False,k_testE=0,eta_pred=0) == v10  [v3'' ERR-30]")):
    ...
    if _log['f']:
        dj = os.path.join(RAIZ, 'datos', f'examen_v13E_k3_{stamp}.json')
        meta = dict(fecha=time.strftime('%Y-%m-%dT%H:%M:%S'), semillas=S, semilla_inicial=seeds[0], veredictos=V, congelar=bool(todo and S >= 20),
                    sha_organismo_v13=h16(os.path.join(_D, 'organismo_v13E_k3.py')), sha_bateria_v13=h16(os.path.abspath(__file__)),
