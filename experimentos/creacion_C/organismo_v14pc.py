"""organismo_v14pc = organismo/organismo_v14g.py (1f1318480cd34cde, mundo de regla sobre el TRONCO v14)
+ APRENDER SIN MORDER: predictor de dE (bloque 6) + CONSOLIDACION POR CODIFICACION PREDICTIVA de la via lenta
en CADA ENCUENTRO, con canal de retorno elegible (directo / transpuesta / feedback alignment / control), y la
medida EXPOSICIONES HASTA CRITERIO. Con eta_c=0, eta_pred=0 y probe_cada=0 es organismo_v14g EXACTO.
Generado por experimentos/creacion_C/construye_codpred.py. NO editar. RAMA de creacion: no es tronco.
"""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.05,memoria_rechazo=20,mu_norm=True,div_signo=True,eta_s=0.0,clip_s=3.0,puerta=None,mundo='AB',regla='px0',fase2_en=None,sonda_final=False,mask_rel=0,del_s=0.25,del_c=0.25,ema_c=0.05,puerta_pat=0,pat_shuf=0,pat_min=0,eta_pred=0.0,clip_e=3.0,eta_c=0.0,canal='directo',n_pred_min=20,probe_cada=0,crit_expo=0.90):
    ...
    for t in range(T):
        ...
        if mundo!='AB' and t==fase2_en:   # v13g: sonda a priori (valor TOTAL) y entrada de los de test
            ...
            acc_lenta_f2=_acc_lenta()   # v14pc: el acierto de la via LENTA justo antes de que entren los de test
        ...
        if pos in objs:
            ...
            if mordio:
                R=R_VAL[val[kk]]; E=min(E+E_VAL[val[kk]],1.5); mord[kk][q(t)]+=1; _mor_tr+=int(mundo!='AB' and t<fase2_en)
                _ky=_key(kc)
    ...
    return dict(sobre=sobre,llegadas=llegadas,sin_objetivo=sin_objetivo,memoria_rechazo=memoria_rechazo,err_max=err_max,t_conflicto=t_conflicto,t_techo=t_techo,n_techo=n_techo,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),puerta_pat=puerta_pat,pat_shuf=pat_shuf,pat_min=pat_min,n_cod=len(ncod),
                ...
                canal=canal,mundo=mundo,regla=regla,tren=tren,test=test,W_apriori=W_apriori,codigos_f2=codigos_f2,primer=primer,W_final=({_k:valor(P_[_k]) for _k in P_} if mundo!='AB' else None),sonda=_sonda,codigos_fin=_cod_fin)
