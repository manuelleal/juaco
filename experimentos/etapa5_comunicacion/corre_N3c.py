"""Etapa 5 N3c (sentidos complementarios en un mundo con reaparicion en el mismo sitio, regen=50). Ejecuta PREREGISTRO_N3c.md:
...
if __name__ == '__main__':
    ...
    _log['f'] = open(os.path.join(RAIZ, 'datos', f'N3c_s{SEEDS[0]}-{SEEDS[-1]}_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    pre = os.path.join(AQUI, 'PREREGISTRO_N3c.md')
    log(f"ARRANQUE N3c (sentidos complementarios; mundo con regen=50; acierto balanceado; solo el receptor escucha) sobre v13. condiciones {list(COND)}, semillas {SEEDS[0]}-{SEEDS[-1]}. Pool({N_PARALELO}).")
    ...
    log(f"VEREDICTO N3c: {'TRANSFIERE entre sensores por conducta' if V['TRANSFIERE'] else 'NO (refutado o montaje invalido)'}")
    ...
    dj = os.path.join(RAIZ, 'datos', f'N3c_s{SEEDS[0]}-{SEEDS[-1]}_{stamp}.json')
