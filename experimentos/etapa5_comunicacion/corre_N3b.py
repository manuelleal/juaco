"""Etapa 5 N3b (sentidos complementarios, tras ERR-23) = experimento 3 del plan. Ejecuta PREREGISTRO_N3b.md:
...
if __name__ == '__main__':
    ...
    _log['f'] = open(os.path.join(RAIZ, 'datos', f'N3b_s{SEEDS[0]}-{SEEDS[-1]}_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    pre = os.path.join(AQUI, 'PREREGISTRO_N3b.md')
    log(f"ARRANQUE N3b (sentidos complementarios, tras ERR-23; acierto balanceado, solo el receptor escucha) sobre v13. condiciones {list(COND)}, semillas {SEEDS[0]}-{SEEDS[-1]}. Pool({N_PARALELO}).")
    ...
    log(f"VEREDICTO N3b: {'TRANSFIERE entre sensores por conducta' if V['TRANSFIERE'] else 'NO (refutado o montaje invalido)'}")
    ...
    dj = os.path.join(RAIZ, 'datos', f'N3b_s{SEEDS[0]}-{SEEDS[-1]}_{stamp}.json')
