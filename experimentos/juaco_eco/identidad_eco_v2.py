"""identidad_eco_v2.py — ARNES del runner de ECO v2 (corre_eco_v2.py). Un proceso, sin Pool. Nube, 24-sep-2026.
...
def main():
    import corre_eco_v2 as V
    t0 = time.time()
    print(f"IDENTIDAD ECO v2 (runner) · {time.strftime('%Y-%m-%d %H:%M:%S')} · python {sys.version.split()[0]} · shas {V.SHAS()}")
    chk("(M) mundos y brazos del preregistro", V.MUNDOS == {'w30': dict(esc=30, n0=30, tope=3000), 'w90': dict(esc=90, n0=90, tope=3000),
        ...
        and V.V2 == dict(T=120000, t_corte=60000) and V.VENTANAS == {'serie': 20011, 'replica': 20031})
    V.MOTOR[0] = V.ME2
    res = {}
    ...
    if os.path.exists(os.path.join(AQUI, 'motor_eco_rapido_org.py')):
        for m, T_, tc in (('w30', 3000, 2000), ('w90', 2500, 1500), ('w270', 1200, 800)):
            for b in V.BRAZOS:
                ...
                chk(f"(G) {m} {b}: gemelo == Python en trabajo (T {T_}, corte {tc})", not dist, f"(distintas {dist}; {p['seg']} s contra {g['seg']} s)")
    else:
        ...
        for m in V.MUNDOS:
            for i in range(20):
                for b in V.BRAZOS:
                    ...
                    R.append(dict(seed=20011 + i, mundo=m, brazo=b, sel_corte=sc, banco_on={'ensena': on, 'filtra0': 0.1},
    ...
    casos = [("FUNCIONA (3 de 3)", fake(todos, todos), 'FUNCIONA'),
             ...
             ("NO EVALUABLE (falsos positivos en w30)", fake(todos, todos, falso='w30'), 'NO EVALUABLE (AZAR'),
    ...
    pr = subprocess.run([sys.executable, os.path.join(AQUI, 'corre_eco_v2.py'), '--serie', '--ventana', 'serie', '--pool', '3', '--nada'],
...
if __name__ == '__main__':
    ...
    with contextlib.redirect_stdout(Tee()):
        ok = main()
    open(os.path.join(AQUI, 'identidad_eco_v2_salida.txt'), 'w', encoding='utf-8').write(buf.getvalue())
