...
echo "[$(date -u +%H:%M:%S)] cola: espero la serie de ECO v3" >> $S/cola_eco.log
until grep -q "VEREDICTO:" $S/eco_v3_serie.out || grep -q "Traceback" $S/eco_v3_serie.out; do sleep 30; done
sube experimentos/juaco_eco/datos/eco_v3_serie_s20111-20130 $S/eco_v3_serie.out "ECO v3 serie 20111-20130"
...
corre $S/eco_v21_serie.out   experimentos/juaco_eco/datos/eco_v21_serie_s20211-20230   "ECO v2.1 serie 20211-20230"   $PY experimentos/juaco_eco/corre_eco_v21.py --serie --ventana serie --pool 3
...
for f in eco_v3_serie eco_v21_serie eco_v21_replica eco_v3_replica eco_v12_serie eco_v12_replica; do echo "== $f"; grep "VEREDICTO" $S/$f.out | tail -2 | cut -c1-260; done
