...
"""organismo_v13q_A = organismo_v13q.py (0b59eb03858df3a8) + knob regla_lenta='dos_canales'|'delta' (Agente A, bloque 3c,
PUENTE_xor.md). 'dos_canales' es el codigo original sin tocar (identidad obligatoria). 'delta' reemplaza los dos canales
no negativos Wps/Wns (+drenaje lam) por UN solo vector con signo Ws, actualizacion Ws += eta_s*error*phi(P) con el error
residual de la via lenta (R - Ws@phi si puerta no es None; dlt compartido si puerta es None, igual que el original),
tope simetrico clip(-clip_s,+clip_s), SIN drenaje (no hay parte comun que drenar con un solo vector). Copia de trabajo:
NO se edita el original organismo_v13q.py. Generado a mano por el Agente A."""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.05,memoria_rechazo=20,mu_norm=True,div_signo=True,eta_s=0.0,clip_s=3.0,puerta=None,mundo='AB',regla='px0',fase2_en=None,sonda_final=False,lectura='lineal',regla_lenta='dos_canales'):   # xor: lectura de la via lenta; A: regla_lenta de la via lenta ('dos_canales'=original, 'delta'=Ws con signo)
    ...
    Ws=np.zeros(_NF)   # A: vector con signo de la via lenta, solo se usa/mueve si regla_lenta=='delta'
    ...
    def lenta(P):   # A: lectura de la via lenta sola, segun regla_lenta ('dos_canales' = formula original, exacta)
        _ph=phi(P); return float(Ws@_ph) if regla_lenta=='delta' else float((Wps-Wns)@_ph)
    ...
    for t in range(T):
        ...
        if mundo!='AB' and t==fase2_en:   # v13g: sonda a priori (valor TOTAL) y entrada de los de test
            ...
            W_lenta_apriori={_k:lenta(P_[_k]) for _k in P_}   # 3b: la via lenta sola
        ...
        R=0.
        if pos in objs:
            kk=objs[pos]; kc=kenyon(P_[kk]); Wb=Wp-Wn; _wf=float(Wb@kc); _ws=lenta(P_[kk])   # v13: las dos vias (xor: phi)
            ...
            if mordio:
                ...
                if learn:
                    ...
                    if eta_s:   # v13: actualizacion de la via lenta (su tasa, mismo drenaje)
                        ...
                        if regla_lenta=='delta':   # A: un solo vector con signo, sin drenaje (no hay parte comun entre dos canales)
                            Ws=np.clip(Ws+eta_s*_ds*phi(P_[kk]),-clip_s,clip_s)
                        else:   # 'dos_canales': regla original v13/v13q, sin tocar (identidad obligatoria)
    ...
    W_lenta={k:round(lenta(P_[k]),3) for k in P_}   # v13: lectura de la via lenta sola (xor: phi)
    ...
    return dict(sobre=sobre,llegadas=llegadas,sin_objetivo=sin_objetivo,memoria_rechazo=memoria_rechazo,err_max=err_max,t_conflicto=t_conflicto,t_techo=t_techo,n_techo=n_techo,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),
                solap=None if mundo!='AB' else {'AB':len(code(P_['A'])&code(P_['B'])),'nB':len(code(P_[nuevo])&code(P_['B'])) if nuevo else None},W_lenta=W_lenta,Wps=[round(float(x),3) for x in Wps],Wns=[round(float(x),3) for x in Wns],Ws=[round(float(x),3) for x in Ws],regla_lenta=regla_lenta,mundo=mundo,regla=regla,tren=tren,test=test,W_apriori=W_apriori,W_lenta_apriori=W_lenta_apriori,familiar_apriori=familiar_apriori,codigos_f2=codigos_f2,primer=primer,W_final=({_k:valor(P_[_k]) for _k in P_} if mundo!='AB' else None),sonda=_sonda,codigos_fin=_cod_fin)
