"""organismo_v13q_C = organismo_v13q.py (0b59eb03858df3a8) + knob constante (Agente C, bloque 3c, PUENTE_xor.md).
constante=False (default) = organismo_v13q.py EXACTO (identidad obligatoria). constante=True agrega 1 entrada fija
en 1.0 al final de phi(P): cuadratica pasa de 21 a 22 entradas (el termino constante/bias que phi no tenia).
Copia de trabajo para el trio XOR; NO es el instrumento congelado, NO se edita organismo_v13q.py."""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.05,memoria_rechazo=20,mu_norm=True,div_signo=True,eta_s=0.0,clip_s=3.0,puerta=None,mundo='AB',regla='px0',fase2_en=None,sonda_final=False,lectura='lineal',constante=False):   # xor: lectura de la via lenta; C: constante=True agrega termino constante/bias a phi
    ...
    _NF=(6 if lectura=='lineal' else 21)+(1 if constante else 0)   # xor: lineal = 6 px; cuadratica = 6 px + 15 productos de pares; random15 = 6 px + 15 bits fijos al azar por patron; C: +1 si constante
    ...
    def phi(P):   # C: constante=True agrega 1 entrada fija en 1.0 al final (termino constante/bias de phi)
