"""SONDA DE DIAGNOSTICO — NO es un organismo del experimento.
Copia EXACTA de organismo_v7e.py (sha16 3118c6d563542da2) + UNA linea que exporta los codigos
finales de cada estimulo, para clasificar solapamiento. Ver PREREGISTRO_exp2b.md seccion 7.
Su control (bit-identidad con v7e en las claves compartidas) lo corre corre_bug01_exp2b.py y es
condicion de aborto.
"""
"""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
    ...
    codigos={k:sorted(int(i) for i in code(PAT[k])) for k in PAT}   # <-- UNICA linea anadida (sonda)
    return dict(codigos=codigos,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),
