"""organismo_v7h = organismo_v7e.py (3118c6d563542da2) + inversiones seriadas + contadores por fase
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.0,invertir_cada=None,crit_v=-2.5,crit_c=0.5):
        ...
        if pos in objs:
            ...
            if mordio:
                ...
                if learn:
                    ...
                    if _trunca:
                        n_techo+=1
                        if t_techo is None: t_techo=t; techo_primero=(kk,'Wp' if dlt>0 else 'Wn',fase)
                ...
                if FS[kk]['n'][fase] is None:
                    ...
                    if (val[kk]=='veneno' and _w<=crit_v) or (val[kk]=='comida' and _w>=crit_c): FS[kk]['n'][fase]=FS[kk]['mord'][fase]
        E-=costo
    ...
    return dict(fases=FS,dfase=dfase,t_techo=t_techo,techo_primero=techo_primero,n_techo=n_techo,mv_tot=mv_tot,mc_tot=mc_tot,err_max=err_max,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),
