"""organismo_v7g = organismo_v7e.py (3118c6d563542da2) + piso del decaimiento + 3 fases.
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.0,piso=0.0,revertir_en=None,sin_B=None,crit_miedo=-2.5):
    ...
    for t in range(T):
        ...
        if pos in objs:
            ...
            if mordio:
                ...
                if learn:
                    dlt=R-Wb@kc
                    if lam: ix=kc>0; mcom=np.maximum(np.minimum(Wp[ix],Wn[ix])-piso,0.0); Wp[ix]-=lam*mcom; Wn[ix]-=lam*mcom   # BUG-01: decae el EXCESO de la parte comun sobre el piso
    ...
    return dict(nB=nB,nB3=nB3,n1=n1,n3=n3,wB_ext=wB_ext,wB_pre=wB_pre,wB_post=wB_post,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),
