"""mundo_largo_e = mundo_largo.py (9f74ff6b5941e5a5) + ESCALA DEL RECUERDO: en _sesgo_M el valor de cada sitio
recordado entra recortado a [-sat_M,+sat_M], de modo que un veneno recordado deja de pesar 3 comidas en la brujula
(la BOCA sigue decidiendo con valor() entero). Controles: sat_barajada (el recorte se aplica en otro sitio) y
kappa_M (atenuacion uniforme, misma magnitud media con la asimetria intacta). Generado por construye_escala.py.
NO editar. Con sat_M=None, kappa_M=1.0, sat_barajada=False es mundo_largo exacto (identidad obligatoria; clave
extra esc_diag, diagnostico)."""
"""mundo_largo = mundo_mapa.py (207d6a1954336b18) + pool de 50 patrones, inyeccion cada T_nuevo pasos (nuevo o reciclado),
cambio de regla sin aviso y lecturas por bloque. Generado por construye_mundo_largo.py. NO editar. Con pool=None,
T_nuevo=None, invertir_largo=None es mundo_mapa exacto (identidad obligatoria)."""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        ...
        sat_M=None,kappa_M=1.0,sat_barajada=False):   # mapa + largo + escala del recuerdo
    ...
    def _vM(p):   # escala: valor recordado de p tal como lo lee la BRUJULA (la boca sigue usando valor() entero)
        ...
        return (1.0 if _v>=0 else -1.0)*max(0.0,abs(_v)-_d)
    ...
    if pool is not None: val.update({n:vl for n,_,vl in pool})   # largo
    ...
    _ren={}; _vistos=[]   # largo (se llenan en el bloque de sitios)
    if sitios is not None:
        ...
        if sat_barajada:   # control 1: permutacion fija de las 8 posiciones de sitio (M solo se escribe ahi); RNG propio, no toca el del organismo
            _ps=np.array(sorted(_SIT)); _perm_s=np.arange(L); _perm_s[_ps]=_ps[np.random.default_rng(seed+950000).permutation(len(_ps))]
    def spawn():
    ...
    def _sesgo_M():   # mapa: con la retina vacia, valor recordado por direccion, descontado por distancia
        if sat_M is not None or kappa_M!=1.0:   # escala: mismo sesgo, pero el valor recordado entra recortado (y/o atenuado por kappa_M, control de lectura)
            ...
            return gamma_M*kappa_M*np.array([_si,_sd])
    ...
    _iny=[]; _curva=[]; _cbin=[0]*(T//1000+1); _dbin=[0]*(T//1000+1)   # largo: lecturas
    split_t=[]; mord={k:[0]*4 for k in PAT_L}; vis={k:[0]*4 for k in PAT_L}; deaths=0; log=[]
    ...
    for t in range(T):
        ...
        if invertir_largo is not None and t==invertir_largo:   # largo: cambio de regla sin aviso (los 4 iniciales y los presentes)
            for n in set(_vistos[:4])|set(_SIT.values()): val[n]='veneno' if val[n]=='comida' else 'comida'
        hambre=np.clip(1-E,0,1); d,k,left=see(contar=True); pat=PAT_L[k] if k!='vacio' else _VACIO
        ...
        if pos in objs:
            ...
            _nvis[pos]+=1   # escala: lectura del mecanismo (pisar un sitio con el objeto presente; mismo evento que escribe el mapa)
            kk=objs[pos]; kc=kenyon(PAT_L[kk]); Wb=Wp-Wn; _wf=float(Wb@kc); _ws=float((Wps-Wns)@PAT_L[kk])   # v13: las dos vias
            ...
            if mordio:
                ...
                if val[kk]=='comida': _cbin[t//1000]+=1   # largo
                ...
                if learn:
                    ...
                    if eta_s:   # v13: actualizacion de la via lenta (su tasa, mismo drenaje)
                        ...
                        if lam: _mcs=np.minimum(Wps,Wns)*(PAT_L[kk]>0); Wps=Wps-lam*_mcs; Wns=Wns-lam*_mcs
                        if _ds>0: Wps=np.clip(Wps+eta_s*_ds*PAT_L[kk],0,clip_s)
                        else:     Wns=np.clip(Wns+eta_s*aversion*(-_ds)*PAT_L[kk],0,clip_s)
        ...
        if E<=0: deaths+=1; E=.6; pos=int(rng.integers(L)); _dbin[t//1000]+=1   # (+largo)
        if log_cada and t%log_cada==0: log.append((t,)+tuple(round(valor(PAT_L[k]),2) for k in 'ABCD'))
    _tel=None
    ...
    W={k:round(valor(PAT_L[k]),2) for k in PAT_L}   # v13: valor total
    W_lenta={k:round(float((Wps-Wns)@PAT_L[k]),3) for k in PAT_L}   # v13: lectura de la via lenta sola
    ...
    return dict(sobre=sobre,llegadas=llegadas,sin_objetivo=sin_objetivo,memoria_rechazo=memoria_rechazo,err_max=err_max,t_conflicto=t_conflicto,t_techo=t_techo,n_techo=n_techo,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),
                solap={'AB':len(code(PAT_L['A'])&code(PAT_L['B'])),'nB':len(code(PAT_L[nuevo])&code(PAT_L['B'])) if nuevo else None},W_lenta=W_lenta,Wps=[round(float(x),3) for x in Wps],Wns=[round(float(x),3) for x in Wns],
                ...
                esc_diag=dict(visitas={int(x):int(_nvis[x]) for x in _SIT}))   # escala: diagnostico (clave NUEVA, no criterio)
