"""organismo_v142g_fanin = organismo/organismo_v142g.py (9e5f566cd6a7a4d2, instrumento de mundo de regla del TRONCO
...
"""organismo_v142g = instrumento de MUNDO DE REGLA del tronco v14.2 (lo usa bateria_generaliza_v142.py).
= organismo/organismo_v14g.py (1f1318480cd34cde, solo se leyo) + B-5 con
`desambiguar=1` POR DEFECTO, igual que el tronco. Con desambiguar=0 es organismo_v14g EXACTO.
AVISO (regla 14): sus defectos de via lenta son eta_s=0.0, clip_s=3.0, puerta=None, mask_rel=0,
puerta_pat=0 -- la entrada de INSTRUMENTOS los pasa EXPLICITOS, campo a campo como la de organismo_v14.
Generado POR ANCLAS desde experimentos/creacion_B/organismo_v14g_codigo.py (ae9231070a95c801) por construye_v142.py.
CONGELADO: NO editar a mano."""
"""organismo_v14g_codigo = organismo/organismo_v14g.py (1f1318480cd34cde, solo se leyo) + perilla `desambiguar` (B-5).
Con desambiguar=0 es organismo_v14g EXACTO. Generado por construye_codigo.py. NO editar."""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.05,memoria_rechazo=20,mu_norm=True,div_signo=True,eta_s=0.0,clip_s=3.0,puerta=None,mundo='AB',regla='px0',fase2_en=None,sonda_final=False,mask_rel=0,del_s=0.25,del_c=0.25,ema_c=0.05,puerta_pat=0,pat_shuf=0,pat_min=0,desambiguar=1,fanin=6,diag=False):
    if mundo=='AB': P_=PAT; tren=['A','B']; test=[]   # v13g: con 'AB' es v13 exacto
    else: P_,tren,test,val_regla=(split_rejilla(seed,regla) if mundo=='rejilla' else split_regla(seed,regla)); fase2_en=T//2 if fase2_en is None else fase2_en
    ...
    if diag:   # FANIN diag: foto inicial (tras el sorteo y el bucle cond) + cobertura de pixeles por las 30 celdas sorteadas
        _lec=(KW[:NK]>0).sum(0); _diag['cobertura']=dict(pixeles_leidos=int((_lec>0).sum()),lecturas_por_pixel=[int(x) for x in _lec],
                                                    min_lecturas=int(_lec.min()),fanin_inicial=[int(x) for x in (KW[:NK]>0).sum(1)]); _diag['ini']=_foto(P_)
    ...
    for t in range(T):
        ...
        if pos in objs:
            ...
            if diag and puerta is not None:   # FANIN diag: que via decide en CADA encuentro, nunca vistos (test) y vistos (tren; tren2 = desde T/2)
    ...
    if diag: _diag.update(fin=_foto(P_),llamadas=_dg['llamadas'],empates=_dg['empates'],empates_cero=_dg['empates_cero'],encuentros={_q:_dg[_q] for _q in _dg if _q[:4] in ('enc_','rap_')})
