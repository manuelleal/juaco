"""bateria_v142_fanin = organismo/bateria_v142.py (6375d90e531b06e6, CONGELADA: solo se leyo) apuntando a
...
sys.path[:0] = [AQUI, _D14, os.path.join(RAIZ, 'experimentos', 'bug01')]
EVENTO = 50000
...
def tarea_id(args):
    ...
    import organismo_v142_fanin as v13   # FANIN: copia por anclas del tronco v14.2 (fanin=6 == tronco)
    if cual == 'v11':
...
if __name__ == '__main__':
    ...
    argv = [a for i, a in enumerate(sys.argv[1:], 1) if a not in ('--log', '--desde', '--fanin', '--pool', '--humo') and sys.argv[i - 1] not in ('--desde', '--fanin', '--pool')]
    ...
    if '--log' in sys.argv or HUMO:
        _log['f'] = open(os.path.join(_DAT, f'examen_v142_fanin{FANIN}{"_humo" if HUMO else ""}_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    ...
    log(f"sha organismo_v13 {h16(os.path.join(AQUI, 'organismo_v142_fanin.py'))}  bateria_v13 {h16(os.path.abspath(__file__))}"
    ...
    if _log['f']:
        ...
        meta = dict(fanin=FANIN, humo=HUMO, T=T_ETAPA, fecha=time.strftime('%Y-%m-%dT%H:%M:%S'), semillas=S, semilla_inicial=seeds[0], veredictos=V, congelar=bool(todo and S >= 20),
                    sha_organismo_v13=h16(os.path.join(AQUI, 'organismo_v142_fanin.py')), sha_bateria_v13=h16(os.path.abspath(__file__)),
                    sha_preregistro=h16(prereg), sha_organismo_v11=h16(os.path.join(_D14, 'organismo_v11.py')), sha_organismo_v10=h16(os.path.join(_D14, 'organismo_v10.py')),
