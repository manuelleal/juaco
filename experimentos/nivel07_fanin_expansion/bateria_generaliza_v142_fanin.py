"""bateria_generaliza_v142_fanin = organismo/bateria_generaliza_v142.py (e5929942647756a5, CONGELADA: solo se leyo)
...
INSTRUMENTOS = {
    ...
    'organismo_v142': ('organismo_v142g', dict(eta_s=0.15, clip_s=10.0, puerta=3, mask_rel=2, del_s=0.25, del_c=0.25, ema_c=0.05, puerta_pat=5, pat_shuf=0, pat_min=1)),   # TRONCO v14.2 = v14.1 + B-5: kwargs CAMPO A CAMPO iguales a los de organismo_v14 (regla 14)
    'organismo_v142_fanin': ('organismo_v142g_fanin', dict(eta_s=0.15, clip_s=10.0, puerta=3, mask_rel=2, del_s=0.25, del_c=0.25, ema_c=0.05, puerta_pat=5, pat_shuf=0, pat_min=1)),   # TRONCO v14.2 (18 sep, noche) = v14.1 + B-5: kwargs CAMPO A CAMPO iguales a los de organismo_v14 (regla 14)
}
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv or HUMO:
        _log['f'] = open(os.path.join(_DAT, f'regresion_generaliza_{modulo}{_SUF}_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    ...
    _dir = {'organismo_v142g_fanin': AQUI, 'organismo_v14g': os.path.join(RAIZ, 'organismo'), 'organismo_v142g': os.path.join(RAIZ, 'organismo'), 'organismo_v11g': GEN, 'organismo_v13g': os.path.join(RAIZ, 'experimentos', 'v13_dos_vias'),
    ...
    if _log['f']:
        ...
        json.dump(dict(meta=dict(fanin=FANIN, humo=HUMO, T=T_REGLA, fecha=time.strftime('%Y-%m-%dT%H:%M:%S'), modulo=modulo, semillas=seeds, veredictos=V,
