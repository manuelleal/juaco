"""organismo_v142_fanin = organismo/organismo_v142.py (17528d767fcebaf6, TRONCO v14.2 CONGELADO: solo se leyo)
...
"""organismo_v142 = TRONCO v14.2 desde el 18 sep 2026 (noche). v14.2 = v14.1 + B-5.
v14.1 = organismo/organismo_v14.py (feefc88b1fd8d434, CONGELADO: solo se leyo).
B-5 DESAMBIGUAR CODIGOS (nivel 4, declarado en dos series el 18-sep 09:07 y 09:12): la division por
conflicto de signo de v11 se dispara TAMBIEN cuando una celda consolidada (|Wb[c]|>0.2), bajo una retina
distinta (kj@P > KW[c]@P), recibe R == 0 (ausencia de consecuencia); la hija nace SIN valor y la madre
conserva el suyo. Memoria nueva: CERO. Constantes nuevas: CERO. El rng no se toca.
LO UNICO QUE CAMBIA respecto de v14.1 es el defecto de la perilla: `desambiguar=1`. Con desambiguar=0
es organismo_v14 EXACTO bit a bit (organismo/identidad_v142.py). En los mundos del tronco R in (+1,-3):
la regla es INERTE POR CONSTRUCCION (examen 8/8 con splits identicos, generalizacion 40/40 identicas,
coste 0 % exacto). Donde SI actua: el mundo vivo, donde repara el alias de codigo (18/18 semillas ALIAS:
|W[sal]| 0.0, veneno -3.0, evitacion x7 -> x1, muertes a la mitad).
Generado POR ANCLAS desde experimentos/creacion_B/organismo_v14_codigo.py (a4eeca90fb605c78) por construye_v142.py.
CONGELADO: NO editar a mano. Evidencia: registro/REGISTRO_etapas_1_2.md (B-5 y replica), PROPUESTA_v14.md."""
"""organismo_v14_codigo = organismo/organismo_v14.py (feefc88b1fd8d434, TRONCO v14.1 CONGELADO: solo se leyo) + perilla
`desambiguar` (B-5): la division por conflicto de signo de v11 se dispara TAMBIEN cuando una celda consolidada,
bajo una retina distinta (kj@P > KW[c]@P), recibe R == 0 (ausencia de consecuencia); la hija nace sin valor y la
madre conserva el suyo. Con desambiguar=0 es organismo_v14 EXACTO y no se toca el rng (identidad_codigo.py).
Generado por construye_codigo.py. NO editar a mano."""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.05,memoria_rechazo=20,mu_norm=True,div_signo=True,eta_s=0.15,clip_s=10.0,puerta=3,mask_rel=2,del_s=0.25,del_c=0.25,ema_c=0.05,puerta_pat=5,pat_shuf=0,pat_min=1,desambiguar=1,fanin=6,diag=False):
    ...
    if fanin<6:   # FANIN: cada celda sorteada lee SOLO `fanin` pixeles distintos; RNG PROPIO, el rng del organismo no se toca
        ...
        KW[:NK]=KW[:NK]*_MF
    _dg={'llamadas':0,'empates':0,'empates_cero':0,'enc_test':0,'rap_test':0,'enc_tren':0,'rap_tren':0,'enc_tren2':0,'rap_tren2':0}   # FANIN diag: empates en la frontera del 'ganan K' (solo lectura)
    def code(P):
    ...
    if diag:   # FANIN diag: foto inicial (tras el sorteo y el bucle cond) + cobertura de pixeles por las 30 celdas sorteadas
        _lec=(KW[:NK]>0).sum(0); _diag['cobertura']=dict(pixeles_leidos=int((_lec>0).sum()),lecturas_por_pixel=[int(x) for x in _lec],
                                                    min_lecturas=int(_lec.min()),fanin_inicial=[int(x) for x in (KW[:NK]>0).sum(1)]); _diag['ini']=_foto(PAT)
    ...
    if diag: _diag.update(fin=_foto(PAT),llamadas=_dg['llamadas'],empates=_dg['empates'],empates_cero=_dg['empates_cero'],encuentros={_q:_dg[_q] for _q in _dg if _q[:4] in ('enc_','rap_')})
    return dict(fanin=fanin,diag_fanin=(_diag if diag else None),desambiguar=desambiguar,des_splits=_ndes,des_t=_des_t,sobre=sobre,llegadas=llegadas,sin_objetivo=sin_objetivo,memoria_rechazo=memoria_rechazo,err_max=err_max,t_conflicto=t_conflicto,t_techo=t_techo,n_techo=n_techo,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),puerta_pat=puerta_pat,pat_shuf=pat_shuf,pat_min=pat_min,n_cod=len(ncod),
