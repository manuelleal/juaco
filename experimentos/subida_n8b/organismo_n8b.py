"""organismo_n8b = experimentos/subida_n8/organismo_flujo.py (14afed5aa16e09bf) + perillas `recicla` (organo de
...
"""organismo_flujo = experimentos/nivel10_composicion_v14/organismo_capBD_on.py (506b5c08441fd54e) + perillas
...
"""organismo_capBD = experimentos/nivel4_puerta_codigo/organismo_capB.py (8070351683837077, instrumento ya
...
"""organismo_capB = experimentos/v13_reverificacion/organismo_capD13.py (fd8e10435801646c) + puerta por
...
def run(seed,T=100000,learn=True,plan=None,pats=None,chk=None,
        ...
        plast=True,theta=0.6,ema=0.02,paso=0.5,lam=0.0,memoria_rechazo=20,mu_norm=False,div_signo=False,eta_s=0.0,clip_s=3.0,puerta=None,puerta_pat=5,pat_shuf=0,pat_min=1,mask_rel=2,del_s=0.25,del_c=0.25,ema_c=0.05,ventana=0,p_viejo=0.25,fusion=0,recicla=0,prueba=0):
    ...
    def _fundir(_excl,_t):   # n8: FUSION -- libera una celda juntando dos activas fuera del codigo actual
        ...
        return True   # B: evidencia del CODIGO EXACTO (mordidas por codigo) y orden de aparicion
    ...
    return dict(split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),puerta_pat=puerta_pat,pat_shuf=pat_shuf,pat_min=pat_min,n_cod=len(ncod),
                solap={'AB':len(code(pats[n1])&code(pats[n2])),'nB':len(code(pats[nv])&code(pats[n2])) if nv else None},
                hist=hist,n_fus=_nfus[0],fus_log=_fus_log,n_rec=_nrec[0],rec_log=_rec_log,recicla=recicla,prueba=prueba,n_prueba=_npru[0],enc_nofam=_enc[1],enc_tot=_enc[0],ventana=ventana,fusion=fusion,t_agot=t_agot,err_max=err_max,t_entra=t_entra,val=dict(val),t_techo=t_techo,techo_primero=techo_primero,n_techo=n_techo,mv_tot=mv_tot,mc_tot=mc_tot,nofam_fin=nofam_fin,nofam_est=nofam_est,ncod_est=ncod_est)
