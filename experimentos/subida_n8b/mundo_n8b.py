"""mundo_n8b = experimentos/subida_n8/mundo_n8.py (b0b57d7ff02afe4e) con el peso del patron como parametro y el
...
def mundo(semilla, n_est=N_EST, d_pix=D_PIX, reciclado=False, n0=N0_RECICLADO, peso=PESO):
    ...
    combos = list(itertools.combinations(range(d_pix), peso))
    perm = g.permutation(len(combos))
    if n_est > len(combos):
        raise SystemExit(f"C({d_pix},{peso})={len(combos)} < n_est={n_est}")
    vals = []
