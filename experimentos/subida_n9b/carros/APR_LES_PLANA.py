"""APR_LES_PLANA (construido por experimentos/subida_n9b/construye_n9b.py desde carrera_escuderias/carros/APR.py sha 4402aa5142065c72; instrumento de LESION de la prediccion de si, perillas LES_PRED / LES_MUNDO; apagadas = APR bit a bit). carros/APR.py — APR (camino A, experimentos/aprende_barrer): FABRICA + la OPCION APRENDIDA de morder lo malo conocido.
...
MODO = 'aprende'
P_AZAR = None
# ---- LESIONES (subida_n9b; instrumento, no mecanismo; apagadas = APR bit a bit, arnes identidad_n9b.py)
LES_PRED = 'plana'  # None | 'plana' (post = u actual) | 'cruz' (efecto sentido aplicado a la OTRA necesidad)
LES_MUNDO = False   # True: g0 y dist con lo bueno para la necesidad NO activa (control de especificidad)
...
class Carro:
    ...
    def actua(self, obs):
        ...
        if pos in objs:
            ...
            if OPCION: mordio = self._opcion(obs, kk, mordio, _u9, pb)
    ...
    def _apr_init(self, ctx):
        ...
        self._aL = int(ctx['L']); self._les = dict(n=0, post_real=0.0, post_usado=0.0, cambia_post=0, caida_real=0, caida_usada=0, cambia_mundo=0)
    ...
    def _opcion(self, obs, kk, mf, u9, pb):
        ...
        na = self._na
        buenas = {k for k, z in self._adS.items() if z[(1 - na) if LES_MUNDO else na] / z[2] > 0}   # lo que el linaje SINTIO bueno para la necesidad activa
        ...
        if LES_PRED or LES_MUNDO: post = self._lesiona(post, E, Ag, u, m, obs, pos, na, dmin)
    ...
    def _lesiona(self, post, E, Ag, u, m, obs, pos, na, dmin):
        """LESION (subida_n9b). Devuelve el post que USA la decision (y el TD). plana: u; cruz: el efecto sentido de la
        letra sobre la OTRA necesidad. Con LES_MUNDO cuenta las decisiones donde g0/dist difieren de los verdaderos."""
        pu = post
        if LES_PRED == 'plana': pu = u
        elif LES_PRED == 'cruz': pu = min(E + m[1] / m[2], Ag + m[0] / m[2])
        elif LES_PRED is not None: raise SystemExit(f"APR: LES_PRED invalido {LES_PRED!r}")
        s = self._les; s['n'] += 1; s['post_real'] += post; s['post_usado'] += pu
        s['cambia_post'] += int(abs(pu - post) > 1e-12); s['caida_real'] += int(post < u - 1e-9); s['caida_usada'] += int(pu < u - 1e-9)
        if LES_MUNDO:
            bv = {k for k, z in self._adS.items() if z[na] / z[2] > 0}; dv = None; L = self._aL
            for x, k in obs['objs'].items():
                if x != pos and k in bv:
                    d = min((pos - x) % L, (x - pos) % L)
                    if dv is None or d < dv: dv = d
            s['cambia_mundo'] += int(dv != dmin)
        return pu
    ...
    def _apr_salida(self):
        ...
        return dict(modo=MODO, p_azar=P_AZAR, alfa=ALFA_Q, gamma=GAMMA, beta=BETA, r_parto=R_PARTO, r_muerte=R_MUERTE, bin_edad=list(BIN_EDAD), bin_gen=list(BIN_GEN),
                    ...
                    letras={k: [round(z[0] / z[2], 3), round(z[1] / z[2], 3), int(z[2])] for k, z in sorted(self._adS.items())}, **st, **({'lesion': dict(self._les, pred=LES_PRED, mundo=LES_MUNDO)} if (LES_PRED or LES_MUNDO) else {}))
