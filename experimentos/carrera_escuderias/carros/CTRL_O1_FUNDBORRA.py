"""carros/CTRL_O1_FUNDBORRA.py — CARRO DE CONTROL (ERR-101, serie S-FUNDBORRA). NO es una escuderia.

Copia de carros/O1.py (sha 99436afa2715f028) con UNA sola linea agregada en nace(): si el cuerpo nace como FUNDADOR
(info['fundador'] = True: el linaje se extinguio y el mundo pone uno nuevo), la memoria de valencias del linaje
(self.suma, self.n: letra -> suma de dS y numero de mordidas) se BORRA y arranca vacia, como el primer fundador de la
corrida (que la crea vacia en __init__). Los hijos que nacen de la cola la siguen heredando igual que en O1. Nada mas cambia.
"""
...
class Carro:
    ...
    def nace(self, info):
        ...
        if info.get('fundador'): self.suma = {}; self.n = {}   # CTRL_O1_FUNDBORRA (ERR-101): el FUNDADOR nace sin la tabla del linaje
        m = info.get('memoria')
        if m:
