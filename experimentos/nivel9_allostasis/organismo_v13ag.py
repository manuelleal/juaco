"""organismo_v13ag = experimentos/v13_dos_vias/organismo_v13g.py (2a80e125f8593bf2, mundo de regla) + el mismo
predictor de dE y la misma modulacion eta_ef que organismo_v13a.py (MISMAS ANCLAS). Sirve para medir
generalizacion px0/azar (G1/G2 de organismo/bateria_generaliza.py) con las perillas de cada brazo.
Con eta_pred=0 y k_sorpresa=0 es organismo_v13g EXACTO. Generado por construye_allostasis.py. NO editar.
"""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
        eta=.03,tau_e=.85,alpha=1.2,hambre_boca=2.0,aversion=1.0,costo=.002,nobj=4,log_cada=None,plast=True,theta=0.6,ema=0.02,paso=0.5,solap_AB=None,lam=0.05,memoria_rechazo=20,mu_norm=True,div_signo=True,eta_s=0.0,clip_s=3.0,puerta=None,eta_pred=0.0,k_sorpresa=0.0,clip_e=3.0,buf_sorpresa=20,sorpresa_barajada=False,vent_sorpresa=2000,mundo='AB',regla='px0',fase2_en=None,sonda_final=False):
            ...
            if mordio:
                ...
                if learn:
                    ...
                    if eta_pred:   # v13a: predice la energia que rinde el bocado y aprende de SU error (regla delta, tasa propia)
                        _dEp=float(Wpe@P_[kk])+float(Wke@kc); _ee=E_VAL[val[kk]]-_dEp; _sr0=abs(_ee); _sr=_sr0
    ...
    return dict(sobre=sobre,llegadas=llegadas,sin_objetivo=sin_objetivo,memoria_rechazo=memoria_rechazo,err_max=err_max,t_conflicto=t_conflicto,t_techo=t_techo,n_techo=n_techo,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),
                ...
                mundo=mundo,regla=regla,tren=tren,test=test,W_apriori=W_apriori,codigos_f2=codigos_f2,primer=primer,W_final=({_k:valor(P_[_k]) for _k in P_} if mundo!='AB' else None),sonda=_sonda,codigos_fin=_cod_fin)
