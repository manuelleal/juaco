"""bateria_v143 = organismo/bateria_v142.py (6375d90e531b06e6, CONGELADA: solo se leyo) apuntando a **organismo_v143**
...
def tarea_id(args):
    cual, esc, seed = args
    import organismo_v143 as v13   # v14.3 = v14.2 + N (norm_lenta=1 por defecto)
    if cual == 'v11':
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv:
        _log['f'] = open(os.path.join(RAIZ, 'datos', f'examen_v143_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    ...
    log(f"sha organismo_v13 {h16(os.path.join(_D14, 'organismo_v143.py'))}  bateria_v13 {h16(os.path.abspath(__file__))}"
    ...
    if _log['f']:
        dj = os.path.join(RAIZ, 'datos', f'examen_v143_{stamp}.json')
        meta = dict(fecha=time.strftime('%Y-%m-%dT%H:%M:%S'), semillas=S, semilla_inicial=seeds[0], veredictos=V, congelar=bool(todo and S >= 20),
                    sha_organismo_v13=h16(os.path.join(_D14, 'organismo_v143.py')), sha_bateria_v13=h16(os.path.abspath(__file__)),
