...
LETRA = {
    ...
    'T-B': "G1 >= 0.80, G2 >= 0.85, azar G1 en [0.35, 0.65], azar G2 en [0.42, 0.58], K 20/20 (bateria_generaliza, 20 semillas)",
...
# los numeros que la letra fija (regla14() los compara con los modulos de donde se importan)
NUM = dict(TA_muertes=1.10, TA_r_delta=10.0, TA_margen=10.0, z=1.645, n_vivo=80, TC_margen=12.5, TF_razon=1.25,
           TB_g1=0.80, TB_g2=0.85, TB_azar=(0.35, 0.65), TB_azar2=(0.42, 0.58), TB_cob=6, n_examen=20,
...
ARNES_ESPERADO = 'RESULTADO: 106/106'
...
PRED = {
    ...
    'T-B': dict(p=0.55, frase="G1 1.000, G2 >= 0.93, K 20/20, azar G1 en banda (0.97); RIESGO: azar G2 en [0.42, 0.58] 0.55 (mediana del tronco 0.434)"),
    ...
    'serie': dict(p=0.50, frase="P(PASA una serie) 0.47-0.52; P(serie y replica) 0.22-0.27; SIN la banda de azar de T-B: 0.85 y 0.73"),
}
