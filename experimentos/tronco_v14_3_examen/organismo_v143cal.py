"""organismo_v143cal = v14.3 en el MUNDO VIVO del criterio v4 (instrumento del examen, no se congela con el tronco):
...
"""organismo_v3cal = experimentos/nivel11_mundo_vivo/organismo_vivo_rep2.py (96feb4918dc5d694; SOLO se leyo; cadena
organismo_vivo_rep aa823d56c2d4213c <- organismo_vivo 20c0961c79de8825 <- organismo/organismo_v14.py feefc88b1fd8d434 = v14.1)
+ B-5 `desambiguar` (el organo de v14.2, texto literal; por defecto 1, como el TRONCO organismo_v142 17528d767fcebaf6)
+ `invertir_vivo_en` (la reversion del mundo vivo, T-C ii)
+ `placebo = k`: consume k sorteos del MISMO rng por paso y DESCARTA el valor (LA PERILLA DEL BLOQUE A-CAL).
  placebo=0, desambiguar=1, vivo=0, n_nec=1  ->  organismo/organismo_v142.py BIT A BIT (claves de v14.2)
  placebo=0, desambiguar=0                   ->  organismo_vivo_rep2.py BIT A BIT (claves de rep2)
  placebo=1                                  ->  DEBE diferir de placebo=0 (control del arnes)
El brazo PEOR no usa perilla nueva: `costo`/`costo_a` ya son argumentos del tronco.
Memoria nueva: CERO. Arnes: identidad_criterio_v3.py. Generado por construye_criterio_v3.py. NO editar a mano."""
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
    ...
    return dict(desambiguar=desambiguar,des_splits=_ndes,des_t=_des_t,invertir_vivo_en=invertir_vivo_en,placebo=placebo,sobre=sobre,llegadas=llegadas,sin_objetivo=sin_objetivo,memoria_rechazo=memoria_rechazo,err_max=err_max,t_conflicto=t_conflicto,t_techo=t_techo,n_techo=n_techo,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),puerta_pat=puerta_pat,pat_shuf=pat_shuf,pat_min=pat_min,n_cod=len(ncod),
