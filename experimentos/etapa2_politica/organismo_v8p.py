"""organismo_v8p = organismo/organismo_v8.py (dca7d5c3a162f5d4) + contadores de la frontera hambre-supervivencia.
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
    ...
    VH={'veneno':[0]*5,'comida':[0]*5}; MH={'veneno':[0]*5,'comida':[0]*5}; PH={'veneno':[0.0]*5,'comida':[0.0]*5}; dq=[0]*4; t_ext=None   # frontera hambre (lectura)
    tipos=['A','B']
    def spawn():
        while len(objs)<nobj:
    ...
    for t in range(T):
        ...
        if pos in objs:
            ...
            if mordio:
                ...
                if invertir_en is not None and t>=invertir_en and t_ext is None and kk=='B' and float((Wp-Wn)@kenyon(PAT['B']))>=0: t_ext=t
        E-=costo
    ...
    return dict(VH=VH,MH=MH,PH=PH,dq=dq,t_ext=t_ext,err_max=err_max,t_conflicto=t_conflicto,t_techo=t_techo,n_techo=n_techo,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),
