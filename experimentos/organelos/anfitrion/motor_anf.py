"""motor_anf.py (CONSTRUIDO por experimentos/organelos/anfitrion/construye_anf.py desde darwin/motor_endo.py, sha 7e10329cf0cb9fd0; NO editar a mano).
CONTROL DEL ANFITRION: SIM = control_anfitrion; ranura ctl; gancho siembra. Lo que sigue es el docstring del origen.

motor_endo.py (CONSTRUIDO por experimentos/organelos/darwin/construye_endo.py desde juaco_eco/motor_eco3.py, sha 2eec9830792d9822; NO editar a mano).
...
class Cuerpo:
    __slots__ = ('lin', 'k', 'gen', 'padre', 'tn', 'c', 'id', 'obs', 'pos', 'E', 'Ag', 'gv', 'gv0', 'tB', 'tD',
                 'ult_mordida', 'vol_kk', 'hijos', 'fund', 'vivo', 'hvivos', 'pc', 'g', 's', 'sm', 'sm_nac', 'sm0', 'ctl')   # ENDO G2 + ANF A3
    ...
    def __init__(self, lin, k, gen, padre, tn, c, ident, vista, pos, E, fund, pc=None):
        ...
        self.hijos = 0; self.fund = fund; self.vivo = True; self.hvivos = 0; self.pc = pc; self.g = None; self.s = None; self.sm = None; self.sm_nac = 0; self.sm0 = None; self.ctl = None   # ENDO G2 + ANF A3 · pc = cuerpo padre (vivo o no)
...
def run_solapadas(seed, carros, T=100000, pizarra=1, compat=0, rep_acum=0, escala=1, telem=True, diag=1, mundo_n=None,
                  fundador_limpio=0, tope_cuerpos=TOPE_DEF, muestra=MUESTRA, reposicion='fija', r_rep=R_REP, eco=None, simb=None):
    import pista2 as P
    n = len(carros)
        ...
        for j in orden:
            ...
            if b.E <= 0 or b.Ag <= 0:
                ...
                if l.nac >= NAC_MAX:   # ENDO G5 (nube-9)
                    if SB is None: raise SystemExit('PISTA2: mas de 100000 cuerpos en un linaje: la semilla colisionaria (ERR-60)')
