"""motor_cruce.py — CONSTRUIDO por experimentos/organelos/cruce/construye_cruce.py desde experimentos/carrera_escuderias/pista.py
...
AQUI = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))), 'experimentos', 'carrera_escuderias')   # CRUCE: la carrera (solo se LEE)
...
def run(seed, carros, T=100000, pizarra=1, compat=0, rep_acum=0, escala=1, telem=True, diag=1, mundo_n=None, fundador_limpio=0, cruce=None):
    ...
    def ctx_de(i, t0=0):
        return dict(id=ids[i], indice=i, n_linajes=n, T=T, L=L, PAT={k: v.copy() for k, v in CF['PAT'].items()},
                    ...
                    costo_a=M['costo_a'], rep_X=M['rep_X'], cupo=CUPO, ancho=ANCHO, fabrica=cfg_fabrica(), **_cx(i, t0))
            ...
            if l.E <= 0 or l.Ag <= 0:
                ...
                if l.esfund and fundador_limpio:   # ENMIENDA 5: instancia NUEVA, sin nada del objeto viejo; sin nace()
                    ...
                    c.nace(dict(t=t, k=l.nac, fundador=l.esfund, memoria=(_m['mem'] if _m is not None else None),
                                rng_hijo=hijo(i, l.nac), **_info_gen(i)))
    ...
    for i, l in enumerate(lin):
        ...
        out.append(d)
    return dict(linajes=out, pizarra_log=piz_log, **_cruce_out(),
