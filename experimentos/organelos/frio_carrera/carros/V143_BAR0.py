"""V143_BAR0.py — frio_carrera (F1 en la pista de la carrera): V143 + el nacido lee el nodo del linaje sin neutras.
GENERADO por experimentos/organelos/frio_carrera/construye_frio_carrera.py desde tronco_v14_3/carros_v143/V143.py (sha 2a03048a7f1525e5).
NO editar a mano. Perillas: RES0 = 1; BAR = 1; TELEM = 1. Con RES0 = BAR = TELEM = 0 es V143 bit a bit. Sigue el docstring de V143.
...
RES0 = 1; BAR = 1; TELEM = 1   # frio_carrera: LA UNICA LINEA QUE CAMBIA ENTRE VARIANTES (0/0/0 = V143 bit a bit)
...
class Carro:
    def __init__(self, ctx):
        ...
        self._apr_init(ctx)
        self._v3_init(ctx)
        self._fc_init(ctx)   # frio_carrera: solo contadores (sin rng)
    ...
    def nace(self, info):
        ...
        if self._con and self._nodo:
            _msg = self._fc_msg(info) if (RES0 or BAR or TELEM) else list(self._nodo)   # frio_carrera
            ...
            if TELEM: self._fc_sel(_msg, _sel9)   # frio_carrera (solo lectura)
    ...
    def salida(self):
        ...
        return dict(
            ...
            **({'frio_carrera': self._fc_salida()} if TELEM else {}))   # frio_carrera: telemetria (ERR-96: no puntua)
