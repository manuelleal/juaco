...
motor_eco_rapido_fam.py — GEMELO COMPILADO (numba) de experimentos/juaco_eco/motor_eco.py (run_solapadas) con los cerebros
FABRICA / FABRICA_ECO (el de motor_eco_rapido.py) y FAMB_RES0_ECO (ECO v1.2: LA FAMILIA PASA SU TABLA EN EL PARTO).
...
registro/EQUIPO.md). Arnes: identidad_eco_rapido_fam.py (su salida va en identidad_eco_rapido_fam_salida.txt).

Es una COPIA de motor_eco_rapido.py (que no se toca) mas el cerebro FAMB_RES0_ECO. NO es un mundo nuevo ni un organismo
nuevo: la MISMA fisica de la pista v2, el MISMO cerebro FABRICA, el MISMO flujo de azar y, con eco=dict(...), la MISMA
plomeria de JUACO-ECO. Misma firma que motor_eco.run_solapadas y la misma salida (TODAS las claves, d['carro'] incluido).

LO NUEVO (FAMB_RES0_ECO = FABRICA_ECO + tabla de la familia; MODO = 'res', SIN0 = 1), por cuerpo:
  - la tabla RECIBIDA (el nodo): por (necesidad, letra) la R instalada al nacer (tbH / tbR). El nodo del carro es esa tabla
    repetida NODO_LEE veces (ordenada por necesidad y letra); el gemelo guarda la tabla y la repeticion es aritmetica.
  - lo VIVIDO (_mordh): por (necesidad, letra) la R de la mordida MAS RECIENTE del cuerpo (mdH / mdR). al_parir solo lee
    eso de _mordh (la R mas reciente por clave).
  - al_parir (en el parto, sin azar): la tabla del padre = por (necesidad, letra), lo vivido si lo vivio, si no lo recibido;
    orden (necesidad, letra) = el sorted() del carro. Telemetria del padre: partos y dado[:50] (tamano de cada tabla dada).
  - nace (tras Wl y KW del rng del hijo, como FABRICA): SIN0 (quita R == 0), instala la tabla y la LEE por la via lenta:
    NODO_LEE veces, la entrada restante de MAYOR sorpresa |R - (Wps - Wns)[n] . P| (fila sumada de izquierda a derecha
    desde 0.0, como el .sum(1) de NumPy en filas de 6: comprobado en el arnes) y, en empate, la de MAYOR indice original
    (np.lexsort((-indice, -sorpresa))[0]); luego LAM, ETA_S, AVERSION y CLIP_S DEL GENOMA DEL HIJO, con el producto
    escalar de 6 por BLAS (ddot) y los mismos redondeos que resultado(). Telemetria: recibido, lecturas, lect_div.
  - muere() NO se compila: en la pista v2 el cerebro del muerto se descarta en el mismo paso (b.c = None) y lo unico que
    hace muere() (anadir al nodo las ultimas NODO_K mordidas, curitas) no lo lee nadie (arnes (M): el original con
    muere() anulado da la MISMA salida). salida() de los vivos se calcula con una instancia real del carro.

QUE SE DELEGA EN NUMPY / PYTHON Y POR QUE (regla 9) — lo mismo que motor_eco_rapido.py:
  ...
  - Generator: el del mundo, el de la pista, el de fundadores y el de reaparicion de cada linaje se crean en Python y se
    ...
    numba.typed.List de Generators. Todas las extracciones las hace numba sobre el estado real de NumPy.
  ...
  - argsort con EMPATE en la frontera del top-K (codigo de Kenyon): a NumPy (np.argsort) por `objmode`.
  - lexsort de la lectura del nodo: NO se delega; la clave (sorpresa, indice) no tiene empates (los indices son
    distintos), asi que el primero del lexsort es el maximo lexicografico, sin depender del algoritmo de orden.
  - sumas: las de 6 elementos (mu[c].sum(), P.sum(), la fila de la sorpresa): NumPy suma n < 8 de izquierda a derecha
    desde 0.0 y el bucle lo reproduce (comprobado en el arnes; la asociacion 'primero + resto' NO: control). No hay sumas
    de >= 8 reales en el nucleo.
  - muestra de genes, foto del corte, checkpoint, ind_cb y salida() del carro: en Python con el codigo del original.
  - ninguna funcion recursiva (cache=True).

FUERA DE ALCANCE (el gemelo ABORTA con ValueError en vez de callar): cualquier carro que no sea FABRICA / FABRICA_ECO /
FAMB_RES0_ECO con el sha fijado (APR, O1..O4, la variante SIN0 = 0, ...), la pizarra con escrituras, y un checkpoint del
ORIGINAL o del gemelo viejo (formato propio; la firma lo distingue y aborta). Si cambian motor_eco.py, los carros o
pista2.py (sha fijados abajo), el gemelo se niega a correr.
"""
...
SHA = {os.path.join(AQUI, 'motor_eco.py'): 'bca3033878b59622',
       ...
       os.path.join(FRIO, 'carros', 'BAR0_ECO.py'): 'a4b899611bd71197',   # F1 FRIO
...
CEREBROS = {'FABRICA.py': '2ebee3e99ea5a33a', 'FABRICA_ECO.py': 'f1163009cb5193a2', 'FAMB_RES0_ECO.py': '94ea78589bc2ce24',
            'BAR0_ECO.py': 'a4b899611bd71197'}   # F1 FRIO: + BAR0
BARAJA = 'BAR0_ECO.py'   # F1 FRIO: la misma tabla con las R permutadas (MODO = 'bar', SIN0 = 1); LFAM = 2
FAMILIA = 'FAMB_RES0_ECO.py'   # el cerebro con tabla de la familia (MODO = 'res', SIN0 = 1)
FIRMA_GEMELO = 'motor_frio_rapido v1'
...
NBI = 33   # I_SO0..I_SO0+3 = sin_objetivo
...
ERR_MSG = {ERR_NAC: 'motor_frio_rapido: mas de 1e9 cuerpos en un linaje (guardia ERR-60, subida por ERR-146)',
           ERR_RECH: f'motor_eco_rapido_fam: mas de {RC} entradas vigentes de rechazo en un cuerpo (sube RC)',
           ERR_NCOD: f'motor_eco_rapido_fam: mas de {NCOD} codigos distintos en un cuerpo (sube NCOD)',
           ERR_NST: f'motor_eco_rapido_fam: mas de {NST} divisiones en un cuerpo (sube NST)'}
...
def _bucle_exp_numpy():
    ...
    if name != b'exp' or nargs != 2 or not functions or not types_p:
        raise RuntimeError('motor_eco_rapido_fam: no se reconoce la estructura PyUFuncObject de np.exp (version de NumPy)')
    ...
    if not idx: raise RuntimeError('motor_eco_rapido_fam: np.exp sin bucle float64')
...
def _autoprueba_exp():
    ...
    if not np.array_equal(ref.view(np.int64), out.view(np.int64)):
        raise RuntimeError('motor_eco_rapido_fam: el bucle de np.exp no reproduce np.exp: el gemelo no puede ser identico aqui')
    return B
...
def _tramo(t0, t1, pi, pf, PATM, EFF, RV, GI, C0i, C0f,
    ...
    sin0 = pi[P_SIN0]; nlee = pi[P_NLEE]; mnodo = pi[P_MNODO]; trazn = pi[P_TRAZN] != 0
            ...
            if bi[s, I_GV] >= bf[s, F_RX]:
                if wi[W_TOTAL] >= tope:
                    ...
                    if k >= LIM_NAC:   # F1 FRIO, ERR-146
                        wi[W_ERR] = ERR_NAC; return ST_ERR, t
                    mt = 0
                    if LFAM[lin]:   # mem = c.al_parir(...) del padre (antes de crear al hijo, como el original)
                    ...
                    if LFAM[lin]:   # ch.nace(dict(..., memoria=mem, ...)): SIN0 + tabla + lectura del nodo
                        _nace_fam(s3, mt, tn, tk, tR, sin0, nlee, mnodo, bi, bf, Wps, Wns, tbR, tbH, PATM, sce, rst, w6, mdot)
...
def _salida_carro(st, s, mod, ctx):
    ...
    c.rng = st['GL'][s]
    if os.path.basename(mod.__file__) in (FAMILIA, BARAJA):   # F1 FRIO: + BAR0 · el nodo (tabla recibida x max(1, NODO_LEE)), lecturas, lect_div y _n10
...
def _nace_aislado(g, r_cuerpo, r_hijo, tabla, sin0=1, nodo=0, mdot=0):
    """UNA ranura: _consts(g) + _crea(r_cuerpo) + _nace(r_hijo) + _nace_fam(tabla) con las MISMAS funciones del nucleo.
    tabla = [(necesidad, letra, R), ...] (la memoria que da al_parir, en su orden). Devuelve el estado de la ranura (dict)."""
    ...
    _nace_fam(0, len(tabla), tn, tk, tR, int(sin0), nlee, int(nodo), st['bi'], st['bf'], st['Wps'], st['Wns'], st['tbR'], st['tbH'],
...
def run_solapadas(seed, carros, T=100000, pizarra=1, compat=0, rep_acum=0, escala=1, telem=True, diag=1, mundo_n=None,
    ...
    for p_, h_ in SHA.items():
        if _h16(p_) != h_:
            raise SystemExit(f"motor_eco_rapido_fam: {os.path.relpath(p_, RAIZ)} cambio (sha {_h16(p_)} != {h_}); rehacer el gemelo y su arnes")
    for e_, m_ in mods:
        f_ = os.path.basename(getattr(m_, '__file__', '') or '')
        if f_ not in CEREBROS or _h16(m_.__file__) != CEREBROS[f_]:
            raise ValueError(f"motor_eco_rapido_fam: solo compila los cerebros FABRICA / FABRICA_ECO / FAMB_RES0_ECO con su sha (carro {e_!r}: "
        ...
        if f_ == FAMILIA and (getattr(m_, 'MODO', None) != 'res' or getattr(m_, 'SIN0', None) != 1):
            raise ValueError(f"motor_eco_rapido_fam: el carro {e_!r} no es MODO = 'res' con SIN0 = 1")
    if (CF['NKMAX'], CF['K'], kw['n_nec'], tuple(P.TIPOS)) != (NKMAX, K, NNEC, ('A', 'B', 'C', 'D')):
        raise ValueError('motor_eco_rapido_fam: NKMAX/K/n_nec/TIPOS de fabrica distintos de los compilados')
    if T < 4: raise ValueError('motor_eco_rapido_fam: T < 4 no se compila (el original divide por T // 4 = 0 en el primer encuentro)')
    if not (isinstance(kw['nodo_lee'], int) and kw['nodo_lee'] >= 0):
        raise ValueError('motor_eco_rapido_fam: nodo_lee de fabrica no es un entero >= 0')
    if E_ is not None and E_['estado'] is not None and not isinstance(E_['estado'], (bytes, bytearray)):
        raise ValueError('motor_eco_rapido_fam: estado debe ser el blob (bytes) de un checkpoint del gemelo')
    modo = dict(exp=0, dot=0, turno=0, sin0=1, nodo=0); modo.update(_modo or {})
    ...
    if [ME.NOMBRES[j] for j in range(len(ME.GENES)) if ME.GENES[j][2]] != ['memoria_rechazo', 'NK', 'rep_X']:
        raise ValueError('motor_eco_rapido_fam: los genes enteros no son memoria_rechazo, NK, rep_X')
    PATM = np.array([CF['PAT'][x] for x in P.TIPOS], float)
    ...
    if E_ is not None and E_['estado'] is not None:   # E7: reanudar (blob del GEMELO)
        ...
        st['pi'][P_TRAZN] = int(_traza_nac is not None)
        _t0 = int(blob['t'])
    else:
