"""lee.py — EXPLORATORIO, no es dato. Lee datos/*.json y arma la tabla por brazo (mediana sobre semillas de la mediana por semilla del R0 real
de los 9 linajes; cruzan/persisten; fundadores; establecimiento; fundador contra hijo), pareado contra v143 y contra o1 con la MISMA semilla,
y la regla de parada (R0 real >= 0.85 en >= 3/5 semillas). python lee.py [--T 100000] [--md] [--semillas 38001-38005]"""
...
def carga(T, rango):
    ...
    for f in glob.glob(os.path.join(DATOS, f"*_T{T}.json")):
        ...
        if rango and not (rango[0] <= d['seed'] <= rango[1]): continue
...
def fund_hijos(corridas):
    ...
    f = lambda v, h: dict(n=len(v), vida=med(v), sin_parir=(round(sum(1 for x in h if x == 0) / len(h), 3) if h else None), hijos_med=(round(st.mean(h), 2) if h else None))
...
def resumen(corridas):
    ...
    est = [l for l in todos if l['fund_post10k'] == 0 and l['nac_reales'] >= 5]
    return dict(n_sem=len(corridas), R0_med=med(list(por_sem.values())), por_sem=por_sem, R0_lin_med=med([l['R0_real'] for l in todos]),
                ...
                establecidos=len(est), R0_est=med([l['R0_real'] for l in est]),
                vida=med([l['vida_med'] for l in todos]), muertes=med([l['muertes'] for l in todos]), fund=med([l['fundadores'] for l in todos]),
                fund0=sum(1 for l in todos if l['fundadores'] == 0), fund_pre1k=med([sum(1 for t in l['telem']['t_fund'] if t <= 1000) for l in todos]),
                ...
                piz=med([c['pizarra_n'] for c in corridas]), coherente=sum(l['coherente'] for l in todos), seg=med([c['seg'] for c in corridas]))
...
def main():
    ...
    a = ap.parse_args()
    rango = tuple(int(x) for x in a.semillas.split('-')) if a.semillas else None
    R = carga(a.T, rango)
    res = {b: resumen(list(c.values())) for b, c in R.items()}
    orden = sorted(res, key=lambda b: -(res[b]['R0_med'] or 0))
    sep = ' | ' if a.md else '  '
    hdr = ['brazo', 'sem', 'R0 real med', 'por semilla', 'sem>=.85', 'sem>=.90', 'cruzan', 'sem may', 'persist', 'establ (R0)', 'fund med', 'fund=0', 'fund<=1k', 'muert', 'vida',
           'B+D causa', 'fund: n/vida/sin parir', 'hijos: n/vida/sin parir/hijos', 'vs v143 gana/dif', 'vs o1 gana/dif']
    ...
    for b in orden:
        ...
        fila = [b, str(r['n_sem']), str(r['R0_med']), str([r['por_sem'][s] for s in sorted(r['por_sem'])]), f"{r['sem_085']}/{r['n_sem']}", f"{r['sem_090']}/{r['n_sem']}",
                f"{r['cruzan']}/{r['lin']}", f"{r['sem_mayoria']}/{r['n_sem']}", f"{r['persisten']}/{r['lin']}", f"{r['establecidos']}/{r['lin']} ({r['R0_est']})",
                str(r['fund']), f"{r['fund0']}/{r['lin']}", str(r['fund_pre1k']), str(r['muertes']), str(r['vida']), str(r['BD']),
                f"{fh['fund']['n']}/{fh['fund']['vida']}/{fh['fund']['sin_parir']}",
                f"{fh['hijos']['n']}/{fh['hijos']['vida']}/{fh['hijos']['sin_parir']}/{fh['hijos']['hijos_med']}",
                f"{pv['gana']}/{pv['n']} {pv['dif_med']:+}" if pv and pv['n'] else '-',
                f"{po['gana']}/{po['n']} {po['dif_med']:+}" if po and po['n'] else '-']
        print(('| ' if a.md else '') + sep.join(fila) + (' |' if a.md else ''))
    print()
    for b in orden:
        r = res[b]; fh = fund_hijos(list(R[b].values()))
        print(f"{b:10s} causas h/s/v/sal {r['causas']['hambre']}/{r['causas']['sed']}/{r['causas']['veneno']}/{r['causas']['sal']} · fund causas {fh['causa_fund']} · "
              f"hijos causas {fh['causa_hijos']} · mord B+D {r['mordBD']} A+C {r['mordAC']} · sin bueno {r['sin_bueno']} · casi inm {r['casi_inm']} · "
              f"fund>10k {r['fund_post']} · pizarra {r['piz']} · coherente {r['coherente']}/{r['lin']} · s {r['seg']}")
