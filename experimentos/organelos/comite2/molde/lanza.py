"""lanza.py — EXPLORATORIO. Cola de corridas: UN proceso por corrida, a lo sumo MAX a la vez (4), sin Pool. Salta las que ya tienen JSON.
    python lanza.py --brazos v143,piz,... --desde 38001 --n 5 [--T 100000] [--max 4]"""
import argparse, os, subprocess, sys, time

AQUI = os.path.dirname(os.path.abspath(__file__)); DATOS = os.path.join(AQUI, 'datos')
ap = argparse.ArgumentParser(); ap.add_argument('--brazos', required=True); ap.add_argument('--desde', type=int, default=38001)
ap.add_argument('--n', type=int, default=5); ap.add_argument('--T', type=int, default=100000); ap.add_argument('--max', type=int, default=4)
a = ap.parse_args()
a.max = min(a.max, 4)
...
vivos = []; t0 = time.time(); os.makedirs(DATOS, exist_ok=True); log = open(os.path.join(AQUI, 'lanza.log'), 'a', encoding='utf-8')
...
while cola or vivos:
    ...
    while cola and len(vivos) < a.max:
        b, s = cola.pop(0)
        p = subprocess.Popen([sys.executable, os.path.join(AQUI, 'corre_molde.py'), '--brazo', b, '--semilla', str(s), '--T', str(a.T)],
