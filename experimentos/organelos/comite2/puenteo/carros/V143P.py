...
"""V143P.py — COPIA de tronco_v14_3/carros_v143/V143.py (sha 2a03048a7f1525e5) para comite2/puenteo, con actua() partido en
actua_patas() + actua_boca() + actua_cierra(). Nada mas cambia. Lo que sigue es el docstring de V143.
...
class Carro:
    ...
    # ---------------------------------------------------------------- PUENTEO (comite2/puenteo): actua() PARTIDO EN TRES PIEZAS.
    # actua() == el actua de V143 linea por linea y sorteo por sorteo (arnes_puenteo.py: V143P == V143 bit a bit). Las tres piezas
    # existen para que HIB.py pueda sustituir UNA decision (mov o muerde) por la de O1 sin romper la fisica ni el orden de sorteos.
    def actua(self, obs):
        mov = self.actua_patas(obs)
        pos = (obs['pos'] + mov) % self.L
        mordio = self.actua_boca(obs, pos)
        if pos in obs['objs']: self.actua_cierra(obs, pos, obs['objs'][pos], mordio)
        return dict(mov=mov, muerde=mordio, escribe=None)

    def actua_patas(self, obs):
        """PIEZA 1, PATAS: objetivo (_see) + motor estocastico (consume rng.normal). Devuelve mov. Guarda lo que la boca necesita."""
        PAT = self.PAT
        ...
        mov = int(m[1] - m[0])
        self._pp = (d, hambre, _cue2)
        return mov

    def actua_boca(self, obs, pos):
        """PIEZA 2, BOCA: con el cuerpo ya en pos. Progreso (Rp) + decision de morder (consume rng.random si hay objeto).
        Devuelve mordio (False si no hay objeto). NO aplica los efectos que dependen de la decision FINAL (ver actua_cierra)."""
        PAT = self.PAT; t = obs['t']; objs = obs['objs']; rng = self.rng; _na = self._na
        d, hambre, _cue2 = self._pp
        ...
        if pos in objs:
            ...
            if FILTRO and kk in self._v3o:   # tronco_v14_3 FILTRO: con meta, lo recordado malo no se muerde (mismo sorteo consumido)
                self._v3['vetos'] += int(mordio); mordio = False
            elif OPCION: mordio = self._opcion(obs, kk, mordio, _u9, pb)
            self._enc = (pos, kk, kc, Wb, _wf, _ws)
        return mordio

    def actua_cierra(self, obs, pos, kk, mordio):
        """PIEZA 3, CIERRE: efectos que dependen de la decision FINAL de morder (la de V143 o la puenteada): telemetria del filtro,
        memoria de rechazo y, si la opcion APR abrio una decision en este paso, la accion que aprende es la que se EJECUTA."""
        t = obs['t']
        ...
        if OPCION and self._apend is not None and self._apend[3] == t and self._apend[1] != int(mordio):
            self._apend = (self._apend[0], int(mordio), self._apend[2], t)
