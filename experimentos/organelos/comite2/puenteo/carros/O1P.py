...
"""O1P.py — COPIA de carrera_escuderias/carros/O1.py (sha 99436afa2715f028) para comite2/puenteo, con actua() partido en decide() + boca().
Nada mas cambia. Lo que sigue es el docstring de O1.
...
class Carro:
    ...
    # ------------------------------------------------------------ INTERFAZ
    # ---------------------------------------------------------------- PUENTEO (comite2/puenteo): actua() PARTIDO EN decide() + boca().
    # actua() == el actua de O1 linea por linea (arnes_puenteo.py: O1P == O1 bit a bit). decide() da el blanco, el mov y el modo
    # limpieza; boca(k, lev, limpia) dice si O1 morderia la letra k estando en ese estado. HIB.py usa las dos por separado.
    def actua(self, obs):
        d = self.decide(obs); pos = obs['pos']; objs = obs['objs']
        p2 = (pos + d['mov']) % self.L
        muerde = self.boca(objs[p2], d['lev'], d['limpia']) if p2 in objs else False
        return dict(mov=d['mov'], muerde=muerde, escribe=None)

    def boca(self, k, lev, limpia):
        muerde = bool(self._quiere(k, lev, limpia))
        if muerde and k not in self.n: self.st['pruebas'] += 1
        if muerde and limpia and self._val(k) is not None and (self._val(k) < 0).any(): self.st['limpias'] += 1
        return muerde
    ...
    def decide(self, obs):
        ...
        return dict(mov=mov, tgt=tgt, lev=lev, limpia=limpia)
