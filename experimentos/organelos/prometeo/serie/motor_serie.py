...
# motor_serie.py: CONSTRUIDO por construye_serie.py desde motor_prometeo.py (sha d3367e86c45d40ab). NO EDITAR A MANO.
...
ECO_DEF = dict(refunda=1, p_mut=0.0, sigma=0.15, genoma=None, n_sombra=0, cada_gen=0, ckpt_cada=0, ckpt_fn=None,
               ...
               codigo=None, c_on=True, c_sos=True, cambio=None, mudo=0)   # CODIGO v0 (C1, C4)   # ORGANELOS (G1)
...
class Cuerpo:
    __slots__ = ('lin', 'k', 'gen', 'padre', 'tn', 'c', 'id', 'obs', 'pos', 'E', 'Ag', 'gv', 'gv0', 'tB', 'tD',
                 'ult_mordida', 'vol_kk', 'hijos', 'fund', 'vivo', 'hvivos', 'pc', 'g', 's', 'gr', 'gs', 'cin', 'mb', 'kc')   # ORGANELOS: gr, gs; CODIGO: cin, mb
    ...
    def __init__(self, lin, k, gen, padre, tn, c, ident, vista, pos, E, fund, pc=None):
        ...
        self.cin = None; self.mb = None; self.kc = None   # CODIGO v0 (C2): la cinta y las ultimas 20 mordidas (1 = mala)
    ...
    for t in range(_t0, T):
        ...
        for j in orden:
            ...
            if b.E <= 0 or b.Ag <= 0:
                ...
                if E_ is not None: F.g = _gf; F.s = _sf; F.gr = _grf; F.gs = _gsf; F.cin = _cif; F.mb = ([] if _CO else None); F.kc = (_kc(_cif) if _CO else None)
            ...
            if b.gv >= (M['rep_X'] if b.g is None else b.g[I_RX]):
                if (b.kc is not None and not _MUDO and _kit_veto(b, t)) or hasattr(c, 'quiere_parir') and not c.quiere_parir(dict(t=t, E=b.E, Ag=b.Ag, cola=l.vivos - 1,
                    ...
                    if E_ is not None:   # E3/E4: mutacion del hijo y de sus sombras (rng propios)
                        ...
                        if _CO and len(ES.setdefault('kit_nac', [])) < 60000:   # PROMETEO: presencia del kit por nacimiento
                            _w = _kc(_cih); ES['kit_nac'].append([t, len(_cih), (0 if _w is None else sum(1 for f in _w for v in f if v != 0.0)), int(CD.regla_hgt(_cih) is not None), sum(1 for s_ in _grh if tuple(s_[:4]) != (1, 3, 0, 0)), int(bool(OS.armados(_cih) - OS.armados(_dci))), int(bool(OS.armados(_cih)))])   # SERIE (S4, ERR-144)
                    ...
                    H = Cuerpo(i, k, b.gen + 1, b.k, t, ch, hid, vista, b.pos, _dt, 0, pc=b); H.g = _gh; H.s = _sh; H.gr = _grh; H.gs = _gsh; H.cin = _cih; H.mb = ([] if _CO else None); H.kc = (_kc(_cih) if _CO else None)
