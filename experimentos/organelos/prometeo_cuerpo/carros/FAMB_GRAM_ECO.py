...
# carros\FAMB_GRAM_ECO.py: CONSTRUIDO por construye_cuerpo.py desde codigo\carros\FAMB_GRAM_ECO.py (sha 2cee0a8510c997b9). NO EDITAR A MANO.
"""carros/FAMB_GRAM_ECO.py (CONSTRUIDO por experimentos/organelos/gramatica/construye_gramatica.py desde experimentos/juaco_eco/carros/FAMB_ORG_ECO.py, sha 75d5f4118079ff15; NO editar a mano).
ORGANELOS: el organo de transmision lo describe la GRAMATICA del cuerpo (ctx['gramatica'], gramatica_def.py); sin ella es FAMB_ORG_ECO. Lo que sigue es el docstring del origen.

carros/FAMB_ORG_ECO.py (CONSTRUIDO por experimentos/juaco_eco/construye_eco_org.py desde carros/FAMB_RES0_ECO.py, sha 94ea78589bc2ce24; NO editar a mano).
ECO v2: los organos ensena (el padre pasa la tabla) y filtra0 (el hijo quita lo neutro) se leen del genoma del cuerpo (umbral 1.0). Lo que sigue
es el docstring del origen.

carros/FAMB_RES0_ECO.py (CONSTRUIDO por experimentos/juaco_eco/construye_eco_familia.py desde subida_n10b/carros/FAMB_RES.py, sha 2addb7ca5b9d031d; NO editar a mano).
...
carros/FAMB_RES.py (CONSTRUIDO por experimentos/subida_n10b/construye_familia_b.py desde carros/FABRICA.py,
sha 2ebee3e99ea5a33a; NO editar a mano). SUBIDA_N10B: la familia pasa su TABLA en el parto (MODO = 'res'). Con MODO = 'nada' es FABRICA.
...
SIN0 = 1   # ECO v1.2 (subida_n10c): el hijo descarta las entradas neutras (R == 0) de la tabla recibida
_TABLA = ('res', 'res1', 'bar')
...
class Carro:
    ...
    def actua(self, obs):
        ...
        mov = int(m[1] - m[0]); pos = (pos + mov) % self.L; _sv2 = self._see(pos, objs, t); Rp = .2 if (_sv2 is not None and d is not None and _sv2[0] < d) else 0.   # CUERPO (niebla)
