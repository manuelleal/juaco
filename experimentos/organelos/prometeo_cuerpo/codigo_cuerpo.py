...
# codigo_cuerpo.py: CONSTRUIDO por construye_cuerpo.py desde prometeo\codigo_prometeo.py (sha aac93f0a43b1c827). NO EDITAR A MANO.
# codigo_prometeo.py: CONSTRUIDO por construye_prometeo.py desde codigo_def.py (sha 1096202c8afaa3d1). NO EDITAR A MANO.
...
def _args_rango(op):
    return {'SUM': ((0, N_RASGOS - 1), (-3, 3)), 'EJE': ((0, len(EJES) - 1), (-3, 3)),
            ...
            'CABLE': ((0, N_SEN - 1), (0, N_DEC - 1), (-3, 3)), 'HGT': ((0, len(P_HGT) - 1), (0, len(L_HGT) - 1)),
            'PARTE': ((0, len(PARTES) - 1),)}[op]   # PROMETEO + CUERPO


NUMERICO = {('SUM', 1), ('EJE', 1), ('CABLE', 2)}   # argumentos que mutan en +-1 (los demas son categoricos)
...
def valida(cinta):
    ...
    for ins in cinta:
        ins = tuple(ins)
        if not ins or ins[0] not in OPS + KIT + CUERPO: raise SystemExit(f"CODIGO: instruccion {ins}")
...
def desarrolla(cinta, G0, lo, hi, enteros):
    ...
    x = [0.0] * N_RASGOS; gram = []; st = dict(pasos=0, llamadas=0, corte=0, cable=None, parte=None)   # PROMETEO: cable = W[d][s]; CUERPO: parte = copias por parte
...
def al_azar(r):
    op = ALF[0][int(r.random() * len(ALF[0]))]   # PROMETEO
