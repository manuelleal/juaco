...
# motor_cuerpo.py: CONSTRUIDO por construye_cuerpo.py desde prometeo\motor_prometeo.py (sha d3367e86c45d40ab). NO EDITAR A MANO.
# motor_prometeo.py: CONSTRUIDO por construye_prometeo.py desde motor_fable.py (sha 46750da69ef8cce1). NO EDITAR A MANO.
...
ECO_DEF = dict(refunda=1, p_mut=0.0, sigma=0.15, genoma=None, n_sombra=0, cada_gen=0, ckpt_cada=0, ckpt_fn=None,
               ...
               partes_on=True, partes_costo=True, niebla=None, partes_mag=None)   # CODIGO v0 (C1, C4) + CUERPO   # ORGANELOS (G1)
...
import codigo_cuerpo as CD   # CUERPO (antes codigo_prometeo)
...
def carga_eco(ident):
    ...
    nom = f"carro_cuerpo_{ident}"   # CUERPO: nombre propio (no choca con el carro original en el mismo proceso)
...
class Cuerpo:
    __slots__ = ('lin', 'k', 'gen', 'padre', 'tn', 'c', 'id', 'obs', 'pos', 'E', 'Ag', 'gv', 'gv0', 'tB', 'tD',
                 'ult_mordida', 'vol_kk', 'hijos', 'fund', 'vivo', 'hvivos', 'pc', 'g', 's', 'gr', 'gs', 'cin', 'mb', 'kc', 'pt')   # ORGANELOS: gr, gs; CODIGO: cin, mb
    ...
    def __init__(self, lin, k, gen, padre, tn, c, ident, vista, pos, E, fund, pc=None):
        ...
        self.cin = None; self.mb = None; self.kc = None; self.pt = None   # CODIGO v0 (C2): la cinta y las ultimas 20 mordidas (1 = mala)
...
def run_solapadas(seed, carros, T=100000, pizarra=1, compat=0, rep_acum=0, escala=1, telem=True, diag=1, mundo_n=None,
    ...
    for t in range(_t0, T):
        ...
        for j in orden:
            ...
            if b.E <= 0 or b.Ag <= 0:
                ...
                if E_ is not None: F.g = _gf; F.s = _sf; F.gr = _grf; F.gs = _gsf; F.cin = _cif; F.mb = ([] if _CO else None); F.kc = (_kc(_cif) if _CO else None); F.pt = (_pc(_cif) if _CO else None)
            ...
            if b.gv >= (M['rep_X'] if b.g is None else b.g[I_RX]):
                if (b.kc is not None and _kit_veto(b, t)) or hasattr(c, 'quiere_parir') and not c.quiere_parir(dict(t=t, E=b.E, Ag=b.Ag, cola=l.vivos - 1,
                    ...
                    H = Cuerpo(i, k, b.gen + 1, b.k, t, ch, hid, vista, b.pos, _dt, 0, pc=b); H.g = _gh; H.s = _sh; H.gr = _grh; H.gs = _gsh; H.cin = _cih; H.mb = ([] if _CO else None); H.kc = (_kc(_cih) if _CO else None); H.pt = (_pc(_cih) if _CO else None)
    ...
    _cout = {} if not (_CO or _CB is not None) else dict(codigo=dict(   # CODIGO v0 (C6)
        ...
        prometeo=dict(kit=ES.get('kit'), kit_nac=ES.get('kit_nac', []), hgt_ev=ES.get('hgt_ev', []), cod_corte=ES.get('cod_corte'),
                      banco_final=[list(e[4]) for e in ES['banco'] if e[4] is not None], alfabeto=list(CD.ALF[0])),
