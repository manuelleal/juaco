...
> **INTEGRACIÓN EN EL REPO (22-sep-2026).** Este texto (§0–§11) es COPIA LITERAL del preregistro EXTERNO de
> `JUACO-EXO/equipos/fase10/F0_fable_solo/PREREGISTRO_mundo_fase10.md` (sha `60d4825d1e547e56`; copia intacta en
> `externo/F0_fable_solo/`). Lo externo es **hipótesis, no dato**: sus números (§10 arnés, §11 humo) son de ELLOS y se
> conservan como procedencia; los del repo están en `INFORME_INTEGRACION.md`. Cambios del integrador: (1) rutas relativas
> al repo en `construye_*`, `identidad_*`, `corre_*` (el constructor regenera `mundo_fase10.py` con el MISMO sha
> `84e97674f709a900`); (2) datos y humos al `datos/` y `datos/humo/` del repo; (3) **ENMIENDA 1 (§12)**, escrita ANTES del
> humo del repo; (4) semillas de la serie **2441–2460 / réplica 2461–2480** (2401–2440 quedan reservadas a la letra externa,
> nunca corridas). Todo lo demás (brazos, puertas, umbrales, predicciones de F0) queda con la letra de F0.
...
## 12. ENMIENDA 1 — calibración de F1 sobre el instrumento de F0 (integrador, 22-sep-2026; escrita y guardada ANTES del humo del repo)

**ERR: `ERR-? pendiente` (lo numera el coordinador con `/juaco-err` AL COMMITEAR esta enmienda, antes de correr la serie; regla 11 de EQUIPO).**
Motivo del ERR: esta enmienda cambia perillas del MUNDO **después** de que el humo de F0 viera caer el ancla V-6 (RENACE 0.063). Es una
recalibración a posteriori en el sentido de la regla 3, y se declara así. Lo que la hace admisible (y lo que el ERR debe fijar): (a) V-6 es
una puerta de **validez del instrumento** (¿es éste el mundo con el coste de vida del mundo vivo?), no una puerta de la hipótesis H10;
(b) ningún umbral de puerta cambia (V-1..V-6, M10-1..M10-5, PLAC quedan con la letra de F0); (c) la calibración elegida no se eligió mirando
este instrumento sino otro (F1), y se juzga en semillas que no usó ninguno de los dos (5 y 6 en el humo; **2441–2480** en la serie);
(d) si el ancla NO se conserva en el humo del repo, **no hay enmienda 2 con estas semillas**: se para y decide el director.

**Qué cambia (cuatro perillas, las que F1 midió en su mundo; `corre_mundo_fase10.MUNDO`):**

| perilla F0 | letra F0 | ENMIENDA 1 | origen en F1 (`F1_fable_poderes/corre_mundo_fase10.py`, bloque MUNDO) |
|---|---|---|---|
| `parches` | 6 | 6 (sin cambio) | `nsit10 = 6` |
| `cap` | 5 | **25** | `stock10 = 25` |
| `regen` | 30 | **50** | `regen10 = 50` |
| `cambia_cada` | 5 000 | **12 500** | `cambio10 = 12 500` (F1 REGISTRO 20:12–20:35: barrido 10 000/12 500/15 000/20 000, 4 semillas, T = 40 000) |

**Qué NO cambia, y por qué la transferencia es una PREDICCIÓN y no un dato.** El mundo de F1 donde se midió el ancla (NADA 0.111,
INMORTAL 0.915, una semilla) **no es** éste: F1 tiene 8 tipos en 9 px con valencias "volteadas" balanceadas (`flip10`), sin mudanza por
edad del sitio (sin `vida_parche`), sin cambio de familia entera, rng del mundo `880000+…`, e INMORTAL **sin** mapa. El instrumento de F0
conserva: 16 tipos en 10 px, `vida_parche = 8000`, `cambia_fam = 4` (con 12 500 hay 7 cambios; el 4.º, en t = 50 000, intercambia una
familia entera con la de su eje), `r_vis = 8`, `usa_M = olvida_M = 1` en RENACE, rng del mundo `920000+…`. Los doce brazos, el CUERPO
(campo a campo con `corre_f9`) y el NODO no cambian.

**Humo del repo (UN proceso, 6 corridas, T = 100 000; `python corre_mundo_fase10.py --humo`; JSON en `datos/humo/`):**
RENACE s5, RENACE s6, NADA s5, NADA s6, ORÁCULO s5, ORÁCULO_SIN_MAPA s5. Semillas 5 y 6: ya usadas en el repo (un humo no reserva),
fuera de las sondas de calibración de F1 (1–4) y del humo de F0 (1–2). Bloquean el instrumento HH1, HH2, HH6 (los de F0).
**Ancla (§2.1.6 del encargo, sin tocar):** mediana R₀(NADA) ∈ [0.10, 0.30] **y** mediana R₀(RENACE) ∈ [0.80, 1.30] → "CONSERVA".

**Predicciones firmadas del integrador (antes del humo):**

| qué | rango | P(dentro del ancla) |
|---|---|---|
| R₀ NADA (mediana s5, s6) | 0.08–0.35 | 0.60 |
| R₀ RENACE (mediana s5, s6) | 0.25–1.00 | **0.30** (predigo que CAE por debajo: en este instrumento RENACE lleva el mapa que F0 y F1 midieron como dañino, hay cambio de familia entera y 16 tipos; y la boca que lee una sola fila —H-BOCA, §0 de F0— es la misma en los dos) |
| ANCLA entera | — | **0.20** |
| H-MAPA (dirección, n = 1): R₀ ORÁCULO ≤ R₀ ORÁCULO_SIN_MAPA | — | 0.60 |

**Si el ancla CONSERVA:** la serie con esta letra se puede correr (coordinador, `--serie --desde 2441 --pool 7`, réplica 2461) pero el
integrador **recomienda no correrla** hasta decidir la boca (§5 y §6 de `externo/_evaluacion/PARA_JUACO_mundo_fase10.md`): M10-1 caería
por la misma causa. **Si NO conserva:** la ENMIENDA 1 queda refutada como calibración; el ancla de F1 no se transfiere al instrumento de
F0 y la elección de instrumento vuelve al director (instrumento F1 entero, o instrumento F0 con otra calibración elegida en semillas
nuevas con su ERR).

## 13. Resultado del humo de la ENMIENDA 1 (añadido DESPUÉS; §12 sellado antes, sha `29cb585e1156b198`)
`datos/humo/mundo_fase10_humo_20260922_123306.json` (sha `340d9414b4822ed8`), 112 s, un proceso; identidad interna 5/5; HH1, HH2, HH6 SÍ.
R₀: RENACE s5 **0.306**, s6 **0.214** (mediana **0.26**, fuera de [0.80, 1.30]) · NADA s5 0.146, s6 0.150 (mediana **0.148**, dentro) ·
ORÁCULO s5 0.188 vs ORÁCULO_SIN_MAPA s5 0.265. **ANCLA: NO SE CONSERVA → la ENMIENDA 1 queda refutada como calibración; la serie NO se corre
con esta letra; no hay enmienda 2 con estas semillas (decide el director).** Predicciones del integrador: NADA 0.08–0.35 ✓; RENACE 0.25–1.00 ✓
(0.26, en el borde); ancla entera P = 0.20 → cayó como se predijo; H-MAPA (ORÁCULO ≤ ORÁCULO_SIN_MAPA) ✓. Causa de muerte dominante del
inmortal: `limpio` 129/169 y 151/223 (veneno sin alias mordido), como en F0.
