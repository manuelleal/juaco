"""organismo_boca2 = mundo_fase10 (84e97674f709a900) + perilla boca2 (construye_boca2.py). mundo_fase10 = experimentos/nivel09_cuerpo_nuevo/organismo_f9.py (3a821884394d66c9; aqui solo se LEYO; cadena
...
_PAT_TRONCO=PAT   # F10: los patrones del tronco, para que run() pueda tomar otros (`pats`) sin tocar el global
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
    ...
    PAT=(dict(_PAT_TRONCO) if pats is None else dict(pats)); _D=len(next(iter(PAT.values())))   # FAMILIAS: tamano de la retina (6 con los patrones del tronco -> v14.1 exacto)
    ...
    Wp=np.zeros((n_nec,NKMAX)); Wn=np.zeros((n_nec,NKMAX)); err=np.zeros(NKMAX); mu=np.zeros((NKMAX,_D)); splits=0; _ndes=0; _des_t=[]; el=np.zeros_like(Wl); tr=np.zeros(_D+3)
    Wps=np.zeros((n_nec,_D)); Wns=np.zeros((n_nec,_D))   # v13: via LENTA, lineal sobre la retina (inerte si eta_s=0). VIVO: una fila por necesidad
    ...
    Wpe=np.zeros((n_nec,_D)); Wke=np.zeros((n_nec,NKMAX)); _sbE=np.zeros(n_nec)   # VIVO: predictor VECTORIAL de dS y SORPRESA POR NECESIDAD (inerte con eta_pred=0)
    ...
    _mnec=[0,0]; _bxor=[[0]*len(PAT) for _ in range(n_nec)]; _exor=[[0]*len(PAT) for _ in range(n_nec)]   # VIVO: muertes [por energia, por agua] y tabla NECESIDAD x ESTIMULO (solo lectura)
    _IDX={_k2:_i2 for _i2,_k2 in enumerate(PAT)}
    ...
    _Mpat=np.zeros((L,_D)); _Mset=np.zeros(L,bool); _VACIO=np.zeros(_D)   # mapa (mundo_mapa):  tabla posicion -> ultimo patron visto
    ...
    pos=0; E=1.0; Ag=A_ini; objs={}; val=(dict(VAL_VIVO) if vivo else {'A':'comida','B':'veneno'}) if vals is None else dict(vals)   # F10: valencia inicial por tipo del mundo nuevo
    ...
    def _snap():   # H1: la memoria que se lleva el hijo, congelada EN EL MOMENTO DEL PARTO (no en la muerte del padre)
        ...
        if f10: _m['gen']=_gen   # F10: la generacion del hijo = la del padre + 1 (se fija al parto, con la memoria)
        return _m
    def _nace(_k,_m):   # H1: NACE UN CUERPO NUEVO. Memoria vacia + lo que `hereda` deje pasar. rng propio (ERR-60). El mundo (objs, tipos, val) NO se toca
        nonlocal E,Ag,_prev_on,_nbar,_gen
        ...
        Wl[:]=_rh.uniform(.1,.4,(2,_D+3)); el[:]=0; tr[:]=0   # la politica de locomocion NO se hereda en ningun brazo
        if usa_M: _Mpat[:]=0; _Mset[:]=False   # F10: el mapa es memoria del INDIVIDUO (H-1: la muerte borra al individuo); el hijo nace sin mapa
        ...
        if hereda=='M1+pares' and _m is not None and 'KW' in _m:
            ...
            KW[:]=0; activa[:]=False; KW[:NK]=_rh.uniform(0,1,(NK,_D)); activa[:NK]=True
            if solap_AB: KW[:solap_AB]=0; KW[:solap_AB,0]=5.0
            while not cond(): KW[objetivo_AB:NK]=_rh.uniform(0,1,(NK-objetivo_AB,_D))
        if _m is not None and 'Wps' in _m:
            if hereda=='baraja':
                _pm=_rb.permutation(_D); _nbar+=int(bool((_pm==np.arange(_D)).all()))   # misma magnitud, pixeles equivocados
        ...
        if f10: _gen=(int(_m['gen'])+1 if (_m is not None and 'gen' in _m) else 1)   # F10: fundador = generacion 1; hijo = padre + 1
    for t in range(T):
        ...
        if usa_M and olvida_M and _Mset[pos] and pos not in objs and pos not in _pend: _Mset[pos]=False   # F10 mapa: donde recordaba algo y hoy no hay nada (ni pendiente de reaparecer), olvida: percepcion de ausencia (los parches se mueven)
        ...
        d,k,left=see(contar=True); pat=PAT[k] if k!='vacio' else _VACIO
        ...
        if pos in objs:
            ...
            if boca2==1 and n_nec==2 and not _cue2: _wt=_wt+min(0.0,_vnec(1-_na,PAT[kk],kc))   # BOCA2 VETO: la OTRA necesidad solo puede vetar (lectura de su fila, sin memoria nueva, sin sorteos)
        ...
        if E<=0 or (vivo and Ag<=0):   # VIVO: DOS muertes posibles (con vivo=0 la segunda es imposible: Ag=A_ini y no baja)
            ...
            if alma is not None:   # ALMA: (1) el NODO se llena con lo que el cuerpo que muere aprendio; (2) el alma elige UNA curita del MENU CERRADO
                ...
                if f9:   # F9: LAS MEDIDAS DEL CUERPO QUE MUERE, de _exph/_mordh ANTES de limpiarlas. Todo SOLO LECTURA.
                    ...
                    _mal9=[_x9 for _x9 in _exph if val.get(_x9[1])==('veneno' if _x9[2]==0 else 'sal')]   # F10: lo malo = la valencia ACTUAL (en el mundo del tronco es exactamente B / D)
                    _bue9=[_x9 for _x9 in _exph if val.get(_x9[1])==('comida' if _x9[2]==0 else 'agua')]   # F10: lo bueno = la valencia ACTUAL (en el mundo del tronco es exactamente A / C)
            ...
            if muerte_real:   # H1: LA MUERTE BORRA AL INDIVIDUO. Nace el siguiente de la cola; si no hay, el linaje SE EXTINGUIO y el mundo pone un FUNDADOR
                ...
                _nace(_nac,_m)
                if f10: _al0=_npares_alias()   # F10: alias estructural del recien nacido (su KW es nuevo)
                if alma is not None and _con and nodo and _nodo:   # ALMA: EL NODO CENTRAL. El cuerpo CONECTADO nace con los mensajes de los que
                    ...
                    if nodo_or:   # F9C: NODO ORACULO (cota superior, voto del creador A). El contenido NO es lo que vivio el linaje: es la TABLA VERDADERA
                        ...
                        _or9=[[[float(_z9) for _z9 in PAT[_k9]],float(1.0 if _EF[val[_k9]][_n9]>0 else (-3.0 if _EF[val[_k9]][_n9]<0 else 0.0)),int(_n9)] for _n9 in range(min(n_nec,2)) for _k9 in PAT]
        ...
        if rep_mide:   # REP: VENTANA DE VIABILIDAD = rep_X pasos SEGUIDOS con E y Ag >= rep_umbral -> un DESCENDIENTE VIABLE (la medida). Con rep_coste>0 reproducirse CONSUME los dos recursos (el mecanismo)
            ...
            if _gv>=rep_X:
                ...
                if muerte_real:   # H1: el padre PAGA la dote del hijo y lo pone en la cola; la memoria se congela AQUI, no en la muerte del padre
                    ...
                    _cola.append(_snap())
        if log_cada and t%log_cada==0: log.append((t,)+tuple(round(valor(PAT[k]),2) for k in PAT))
    ...
    if boca2: _ext.update(boca2=int(boca2))   # BOCA2: clave nueva SOLO con la perilla encendida
