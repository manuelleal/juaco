...
experimentos/nivel09_cuerpo_nuevo_b2/organismo_f9c.py (sha 9dd1fb91ecec35ae, SOLO SE LEYO, NO se edito a mano).
...
"""organismo_f9c.py — GENERADO POR construye_f9c.py DESDE organismo_f9.py (3a821884394d66c9),
...
"""
import numpy as np
import collections as mu9_collections   # MURO (diagnostico_muro): instrumento de SOLO LECTURA, no consume rng, no cambia el flujo
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
    ...
    mu9_pres={'A':0,'B':0,'C':0,'D':0}; mu9_steps=0; mu9_W=200   # MURO: presencia acumulada por tipo y ventana (pasos) de fraccion mala
    mu9_fbw=mu9_collections.deque(maxlen=mu9_W); mu9_pre_bad=[]; mu9_pre_causa=[]; mu9_pre_t=[]   # MURO: ventana deslizante y capturas PRE-MUERTE (solo lectura)
    ...
    for t in range(T):
        ...
        if E<=0 or (vivo and Ag<=0):   # VIVO: DOS muertes posibles (con vivo=0 la segunda es imposible: Ag=A_ini y no baja)
            mu9_pre_bad.append(round(sum(mu9_fbw)/(len(mu9_fbw)*nobj),4) if mu9_fbw else None)   # MURO: fraccion mala media en los ultimos mu9_W pasos ANTES de esta muerte
    ...
    return dict(sobre=sobre,llegadas=llegadas,sin_objetivo=sin_objetivo,memoria_rechazo=memoria_rechazo,err_max=err_max,t_conflicto=t_conflicto,t_techo=t_techo,n_techo=n_techo,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),puerta_pat=puerta_pat,pat_shuf=pat_shuf,pat_min=pat_min,n_cod=len(ncod),
                solap={'AB':len(code(PAT['A'])&code(PAT['B'])),'nB':len(code(PAT[nuevo])&code(PAT['B'])) if nuevo else None},W_lenta=W_lenta,Wps=[round(float(x),3) for x in Wps[_nm]],Wns=[round(float(x),3) for x in Wns[_nm]],**_ext,muro=dict(pres=dict(mu9_pres),pasos=mu9_steps,nobj=int(nobj),decay_p=0.003,pre_death_bad=list(mu9_pre_bad),pre_death_causa=list(mu9_pre_causa),pre_death_t=list(mu9_pre_t),ventana=mu9_W))
