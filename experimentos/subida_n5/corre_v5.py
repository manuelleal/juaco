"""NIVEL 5 -- SUBIDA (equipo n5): **V-5** (B-5 trasplantado a la tabla de referencia) sobre el candidato BA-v, con
...
puertas ABSOLUTAS en vez de relativas a CORTADO). Copia por anclas del runner del candidato BA
(`experimentos/junta_fase5/BA/corre_familias_ba.py`, f511d1bbe34887af), con TRES anadidos obligados:

  1. **`b4b` en las celdas por defecto de la serie** (ERR-89: la base de R6 no se corrio y el cociente no
     existia). Si alguien la quita con `--celdas`, el runner la VUELVE A PONER y lo dice en el log.
  2. **R6 SE IMPRIME con su letra exacta** (`muertes <= 1.5x b4b` Y `okU >= okU(b4b) - 0.10`), y con ella las
     SIETE puertas absolutas del criterio nuevo, UNA LINEA POR PUERTA, con `n/d` cuando el brazo no se corrio
     (regla derivada de ERR-89: una puerta que el runner no imprime NO EXISTE, y el humo la muestra).
  3. **Diagnostico del coste**: mordidas totales, de comida y de veneno, y `frac_regalo` (la energia que el
     organismo saca de resucitar) -- lo que a los crudos de BA les faltaba para explicar las 104 muertes.
...
ERR-87: este runner **NO LEE NINGUN JSON** (no hay `lee_json`, no hay "el ultimo de un prefijo"): todo lo que
usa lo calcula en la misma corrida, asi que la regla del prefijo + sello exacto no tiene donde aplicarse aqui.
...
CELDAS (la MISMA tabla y el MISMO mensaje; solo cambia COMO SE LEE):
  ...
  A1      = dos tipos + min + `exige_dir`   -> el candidato del creador A (el control que mas importa)
  BA-v    = dos tipos + min + `conj_tipo`=2 -> **EL CANDIDATO**
  BA-v-sh = BA-v + `baraja_msg`=1           -> **CONTROL: la MEMORIA BARAJADA** (misma R, casilla permutada)
  BA      = dos tipos + min + `conj_tipo`=1 -> el candidato de la junta (se puede pedir, no va por defecto)

    python experimentos/nivel05_familia_variante_BAv/corre_familias_bav.py --humo
    python experimentos/nivel05_familia_variante_BAv/corre_familias_bav.py --serie --desde 961 --pool 6
"""
...
HUMOS = os.path.join(AQUI, 'datos', 'humo')   # V-5: los humos en experimentos/subida_n5/datos/humo/
sys.path[:0] = [AQUI, os.path.join(RAIZ, 'experimentos', 'nivel05_familia_variante_BAv'), JBA, JA, N12, CREA, CREB, os.path.join(RAIZ, 'organismo')]   # organismo/ PRIMERO (ERR-28)
import numpy as np
...
T_HUMO = 100000
SEM_HUMO = [25791]                           # banda de HUMOS de V-5: 25781-25799 (sin uso en el repo, grep 23-sep)
...
SHA_BA_ESPERADO = '1f196ee786b2040d'         # organismo_familias_ba.py: EL ORIGEN del instrumento
...
SHA_BAV_ESPERADO = '2dca0a3e239481f0'         # organismo_familias_bav.py: EL ORIGEN del instrumento V-5
...
CELDAS = {
    ...
    'BA-v':    dict(_DT, combina='min', msg_elige=0, conj_tipo=2),                 # EL CANDIDATO
    'BA-v-sh': dict(_DT, combina='min', msg_elige=0, conj_tipo=2, baraja_msg=1),   # CONTROL: memoria BARAJADA
...
PLAN_HUMO = [('BA-v5', 'CANAL'), ('BA-v5', 'PAR'), ('BA-v5', 'BAR-T'),
...
def corre(base, cel, seed, msg, Ti, nk=30, ktop=3, nkmax=90):
    import organismo_familias_v5 as BAV   # V-5: el instrumento nuevo (con v5=0 es organismo_familias_bav bit a bit)
    kw = B2R.resuelve(KW_R, Ti)
    kw.update(nk=nk, ktop=ktop, nkmax=nkmax)
    _n, _m, _p, _o, _ph = PLANT[base]
    fsr = seed + SEM_OTRO if _o else seed
...
def humo(semillas, Tb, plan):
    ...
    CF._log['f'] = open(os.path.join(HUMOS, 'humo_v5_%s.log' % stamp), 'w', encoding='utf-8', newline='\n')
    CF._log['t0'] = time.time()
    if not cabecera("HUMO del candidato V-5 (nivel 5, subida_n5, criterio ERR-90). UN proceso, sin Pool (regla 3).",
    ...
    for c, b in plan:
        for s in semillas:
            ...
            log("        V-5: v5=%s  partos msg/bocado %s/%s (1.o en t=%s)  subcasillas %s  lee(hermana) tras el mensaje %s  1.a exposicion de la hermana [t, valor, o, fam, mordio, ...] %s"
                % (r['v5'], r['v5_split_msg'], r['v5_split_mord'], r['v5_t1'], r['v5_sub'], r['v5_lee_par'], r['par1']))

    crudo = os.path.join(HUMOS, 'humo_v5_%s_crudo.json' % stamp)   # ERR-54 / ERR-42: el crudo, ANTES del analisis
    ...
    contrastes(tab)
    dj = os.path.join(HUMOS, 'humo_v5_%s.json' % stamp)
    ...
    log("LECTURA (1 semilla: NO es evidencia; con n = 1 ni la linea base se reproduce). Lo que el humo SI "
        ...
        "impriman) y el MECANISMO: `LEE(ref)` = [valor, habla, exactas de FORMA, exactas de VARIANTE]. "
        "Lo que hay que ver: BA-v CANAL positivo con 3/3 y 3/3 y comiendo; BA-v-sh CANAL SIN leer al "
        "referente (la baraja le quita la direccion) y pareciendose a CORTADO.")
    CF._log['f'].close()
...
def serie(desde, n_sem, Tb, cel_act, npool):
    ...
    stamp = time.strftime('%Y%m%d_%H%M%S')
    CF._log['f'] = open(os.path.join(AQUI, 'serie_v5_s%d-%d_%s.log' % (SEEDS[0], SEEDS[-1], stamp)),
                        'w', encoding='utf-8', newline='\n')
    CF._log['t0'] = time.time()
    if not cabecera("SERIE del candidato V-5 (nivel 5, subida_n5, criterio ERR-90), Pool(%d)." % npool, cel_act,
    ...
    with mp.Pool(npool) as pool:
        ...
        for i, r in enumerate(pool.imap_unordered(tarea, tr, chunksize=1), 1):
            ...
            if i % 40 == 0 or i == len(tr):
                log("          %d/%d" % (i, len(tr)))
    crudo = os.path.join(AQUI, 'serie_v5_s%d-%d_%s_crudo.json' % (SEEDS[0], SEEDS[-1], stamp))
    ...
    contrastes(tabla(res, cel_act, BRAZOS_BASE))
    CF._log['f'].close()
...
if __name__ == '__main__':
    ...
    if any(s in PROHIBIDAS or s not in BANDA_HUMO for s in sem):
        raise SystemExit("HUMO: %s fuera de la banda de humos de V-5 (25781-25799)." % sem)
    plan = PLAN_HUMO if '--plan' not in sys.argv else \
