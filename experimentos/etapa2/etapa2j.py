...
import numpy as np
L=40
PA=np.array([1,1,0,1,0,0.]); PB=np.array([1,0,1,0,1,0.]); PC=np.array([0,1,1,0,0,1.]); PD=np.array([0,0,1,0,1,1.])
VIS={'A':[0]*4,'B':[0]*4,'D':[0]*4}; HB=[]; TL=[]
def run(seed,forzar_DB=1,T=100000,eta=.03,tau_e=.85,hambre_boca=2.0,aversion=1.0,olvido_muerte=.0,learn=True,nobj=4,costo=.002,invertir_en=None):
    rng=np.random.default_rng(seed)
    Wl=rng.uniform(.1,.4,(2,9)); NK=30; KW=rng.uniform(0,1,(NK,6)); Wb=np.zeros(NK)   # cuerpo fungiforme
    def ov(P,Q): return len(set(np.argsort(KW@P)[-3:])&set(np.argsort(KW@Q)[-3:]))
    while not (ov(PA,PB)==0 and ov(PD,PB)==forzar_DB and ov(PD,PA)==0): KW=rng.uniform(0,1,(NK,6))   # 2J: D∩B forzado, A∩B=0, D∩A=0
    Wp=np.zeros(NK); Wn=np.zeros(NK); Wb=Wp-Wn   # 2F: circuito apetitivo y aversivo separados
    def kenyon(pat):
    ...
    def spawn():
        while len(objs)<nobj:
            ...
            if x not in objs:
                tipos=['A','B','D'] if hayC[0] else ['A','B']
    ...
    def see():
        ...
        d,k,left=best; pat={'A':PA,'B':PB,'D':PD}[k]
        return pat,d,left
    comida='A'; pre=[]; tA=None; tB=None; wbq=[]; hB=[]; mordB=[0,0,0,0]; mordA=[0,0,0,0]; mordC=[0,0,0,0]; wA=[]; wC=[]
    for t in range(T):
        if invertir_en is not None and t==invertir_en: hayC[0]=True; pre=[Wb@kenyon(PA),Wb@kenyon(PB)]
        ...
        if pos in objs:
            pat={'A':PA,'B':PB,'D':PD}[objs[pos]]   # FIX sincronía
            kc=kenyon(pat); Wb=Wp-Wn; alpha=1.2; Vb=alpha*(Wb@kc)+hambre_boca*hambre+0.5   # 2H: solo la política                       # sesgo fijo: la boca quiere
            pb=1/(1+np.exp(-Vb/.3)); mordio=float(rng.random()<pb); mordidas+=mordio
            q=min(t//(T//4),3); VIS[objs[pos]][q]+=1; HB.append((objs[pos],q,hambre,mordio))
            if mordio:
                q=min(t//(T//4),3)
                if objs[pos]=='A': mordA[q]+=1
                if objs[pos]=='D': mordC[q]+=1
                if objs[pos]=='B':
                    mordB[q]+=1
                    if t>=(invertir_en or T) and len(hB)<3: hB.append(round(float(hambre),2))
                if objs[pos] in (comida,'D'): R=1.0; E=min(E+.8,1.5); ateA.append(t)
        ...
        if learn:
            ...
            if mordio:                                             # sabor -> consecuencia, específico al patrón
                pred=(Wp-Wn)@kc; dlt=R-pred   # aprendizaje sin alpha: idéntico a 2G                                    # 2G: sorpresa específica del estímulo
                if dlt>0: Wp=np.clip(Wp+eta*dlt*kc,0,3.0)
                else:     Wn=np.clip(Wn+eta*aversion*(-dlt)*kc,0,3.0)
                Wb=Wp-Wn
        if invertir_en is not None and t>invertir_en and t%200==0:
            wa=Wb@kenyon(PA); wb=Wb@kenyon(PB)
        if t>=50000 and (t-50000)%5000==0: TL.append((t,round(float(Wb@kenyon(PA)),2),round(float(Wb@kenyon(PB)),2),round(float(Wb@kenyon(PD)),2)))
        if (t+1)%(T//4)==0: wbq.append(round(float(Wb@kenyon(PB)),2)); wA.append(round(float(Wb@kenyon(PA)),2)); wC.append(round(float(Wb@kenyon(PD)),2))
        if E<=0:
            deaths+=1; E=.6; pos=int(rng.integers(L))
            Wl=(1-olvido_muerte)*Wl+olvido_muerte*rng.uniform(.1,.4,(2,9)); Wp*=(1-olvido_muerte); Wn*=(1-olvido_muerte); Wb=Wp-Wn
    a=np.array(ateA); b=np.array(ateB); q=T//4
    f=lambda arr,i:((arr>=i*q)&(arr<(i+1)*q)).sum()
    return [(f(a,i),f(b,i)) for i in range(4)],deaths,np.array([Wb@kenyon(PA),Wb@kenyon(PB),Wp@kenyon(PB),Wn@kenyon(PB),Wp@kenyon(PA),Wn@kenyon(PA)]),pre,tA,tB,wbq,hB,mordB,mordA,mordC,wA,wC,KW,dict(WpA=Wp@kenyon(PA),WnA=Wn@kenyon(PA),WpB=Wp@kenyon(PB),WnB=Wn@kenyon(PB),WpD=Wp@kenyon(PD),WnD=Wn@kenyon(PD))
if False:
