...
def run(seed,T=100000,eta=.03,tau_e=.85,hambre_boca=2.0,aversion=1.0,olvido_muerte=.0,learn=True,nobj=4,costo=.002,invertir_en=None):
    ...
    return [(f(a,i),f(b,i)) for i in range(4)],deaths,np.array([Wb@kenyon(PA),Wb@kenyon(PB),Wp@kenyon(PB),Wn@kenyon(PB),Wp@kenyon(PA),Wn@kenyon(PA)]),pre,tA,tB,wbq,hB,mordB,mordA,mordC,wA,wC,KW
if False:
