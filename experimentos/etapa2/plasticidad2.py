...
def run(seed,T=100000,solap=2,plast=True,theta=0.6,ema=0.02,paso=0.5,eta=.03,alpha=1.2,costo=.002,nobj=4):
    ...
    Wp=np.zeros(NKMAX); Wn=np.zeros(NKMAX); err=np.zeros(NKMAX); mu=np.zeros((NKMAX,6)); el=np.zeros_like(Wl); tr=np.zeros(9)
    ...
    for t in range(T):
        ...
        if pos in objs:
            ...
            if rng.random()<pb:
                ...
                if plast:
                    idx=np.where(kc>0)[0]; err[idx]=(1-ema)*err[idx]+ema*abs(dlt); mu[idx]=(1-ema)*mu[idx]+ema*P   # qué suele ver cada celda
                    for c in idx:
                        if err[c]>theta and (~activa).any():
                            j=int(np.where(~activa)[0][0]); activa[j]=True
                            dist=P-mu[c]   # lo DISTINTIVO del patrón actual para esta celda
                            KW[j]=np.clip(KW[c]+paso*dist,0,5); KW[c]=np.clip(KW[c]-paso*dist,0,5); mu[j]=mu[c].copy()
