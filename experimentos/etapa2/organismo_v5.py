"""
Organismo v5 — baseline congelado (Etapa 2D).
Cambios respecto a v4: regla de la boca sin factor de sorpresa (2A), aversion=1 (2C),
codigos de Kenyon de A y B sin celdas compartidas (2D). Hambre lineal. Inversion del mundo opcional (invertir_en).
"""
"""
...
def run(seed,T=100000,eta=.03,tau_e=.85,hambre_boca=2.0,aversion=1.0,olvido_muerte=.0,learn=True,nobj=4,costo=.002,invertir_en=None):
    ...
    return [(f(a,i),f(b,i)) for i in range(4)],deaths,np.array([Wb@kenyon(PA),Wb@kenyon(PB)]),pre
if __name__=="__main__":
    for learn in (True,False):
        print("=== "+("APRENDE" if learn else "azar"))
        for s in (1,2,3,4,5,6):
            qs,d,Wb,_=run(s,learn=learn)
