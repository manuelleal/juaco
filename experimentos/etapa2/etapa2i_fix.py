...
import numpy as np
L=40
PA=np.array([1,1,0,1,0,0.]); PB=np.array([1,0,1,0,1,0.]); PC=np.array([0,1,1,0,0,1.])
VIS={'A':[0]*4,'B':[0]*4,'C':[0]*4}; HB=[]; TL=[]
def run(seed,T=100000,eta=.03,tau_e=.85,hambre_boca=2.0,aversion=1.0,olvido_muerte=.0,learn=True,nobj=4,costo=.002,invertir_en=None):
    ...
    while len(set(np.argsort(KW@PA)[-3:])&set(np.argsort(KW@PB)[-3:]))>0: KW=rng.uniform(0,1,(NK,6))   # 2D: sin solapamiento A/B
    ...
    def spawn():
        while len(objs)<nobj:
            ...
            if x not in objs:
                tipos=['A','B','C'] if hayC[0] else ['A','B']
    ...
    def see():
        ...
        d,k,left=best; pat={'A':PA,'B':PB,'C':PC}[k]
        return pat,d,left
    ...
    for t in range(T):
        ...
        if pos in objs:
            pat={'A':PA,'B':PB,'C':PC}[objs[pos]]   # FIX sincronía
        ...
        if t>=50000 and (t-50000)%5000==0: TL.append((t,round(float(Wb@kenyon(PA)),2),round(float(Wb@kenyon(PB)),2),round(float(Wb@kenyon(PC)),2)))
        if (t+1)%(T//4)==0: wbq.append(round(float(Wb@kenyon(PB)),2)); wA.append(round(float(Wb@kenyon(PA)),2)); wC.append(round(float(Wb@kenyon(PC)),2))
        if E<=0:
    ...
    return [(f(a,i),f(b,i)) for i in range(4)],deaths,np.array([Wb@kenyon(PA),Wb@kenyon(PB),Wp@kenyon(PB),Wn@kenyon(PB),Wp@kenyon(PA),Wn@kenyon(PA)]),pre,tA,tB,wbq,hB,mordB,mordA,mordC,wA,wC,KW,dict(WpA=Wp@kenyon(PA),WnA=Wn@kenyon(PA),WpB=Wp@kenyon(PB),WnB=Wn@kenyon(PB),WpC=Wp@kenyon(PC),WnC=Wn@kenyon(PC))
if False:
