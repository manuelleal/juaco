...
def run(seed,T=100000,eta=.03,tau_e=.85,hambre_boca=2.0,aversion=3.0,olvido_muerte=.0,learn=True,nobj=4,costo=.002):
    ...
    for t in range(T):
        ...
        if pos in objs:
            ...
            if mordio:
                if objs[pos]=='A': R=1.0; E=min(E+.8,1.5); ateA.append(t)
        ...
        if learn:
            ...
            if mordio:                                             # sabor -> consecuencia, específico al patrón
                g=eta*(aversion if R<0 else 1.0)*(mordio-pb)*delta
    ...
    return [(f(a,i),f(b,i)) for i in range(4)],deaths,np.array([Wb@kenyon(PA),Wb@kenyon(PB)])
