"""organismo_anclado.py -- GENERADO POR experimentos/mundo_anclado/construye_anclado.py DESDE
experimentos/nivel09_cuerpo_nuevo_b2/organismo_f9c.py (sha 9dd1fb91ecec35ae, SOLO SE LEYO). NO EDITAR A MANO.
...
def run(seed,T=100000,learn=True,invertir_en=None,nuevo=None,nuevo_en=50000,nuevo_val='veneno',solap_B=None,
    ...
    return dict(sobre=sobre,llegadas=llegadas,sin_objetivo=sin_objetivo,memoria_rechazo=memoria_rechazo,err_max=err_max,t_conflicto=t_conflicto,t_techo=t_techo,n_techo=n_techo,split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),puerta_pat=puerta_pat,pat_shuf=pat_shuf,pat_min=pat_min,n_cod=len(ncod),
                solap={'AB':len(code(PAT['A'])&code(PAT['B'])),'nB':len(code(PAT[nuevo])&code(PAT['B'])) if nuevo else None},W_lenta=W_lenta,Wps=[round(float(x),3) for x in Wps[_nm]],Wns=[round(float(x),3) for x in Wns[_nm]],**_ext,**({'anclado':dict(olv_mal=float(olv_mal),olv_ciego=int(olv_ciego),eventos=int(_anc_ev),rem=dict(_anc_rem),pres=dict(_anc_pres),pasos=int(_anc_pasos),nobj=int(nobj),sem_olv='890000+1000000*seed')} if (olv_mal or anc_mide) else {}))
