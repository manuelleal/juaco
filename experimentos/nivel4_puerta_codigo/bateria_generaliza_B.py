"""bateria_generaliza_B = organismo/bateria_generaliza.py (46772f5a582872c8) con DOS entradas mas en
...
if __name__ == '__main__':
    ...
    if _log['f']:
        ...
        json.dump(dict(meta=dict(fecha=time.strftime('%Y-%m-%dT%H:%M:%S'), modulo=modulo, semillas=seeds, veredictos=V,
                                 sha_bateria=h16(os.path.abspath(__file__)), sha_organismo=(h16(os.path.join(AQUI, modulo + '.py')) if os.path.exists(os.path.join(AQUI, modulo + '.py')) else h16(os.path.join(_B2, modulo.split('_n5')[0] + '.py'))),
