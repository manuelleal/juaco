"""bateria_v13B = organismo/bateria_v13.py (1a027bcb37eb536e, CONGELADO: solo se leyo) apuntando a
organismo_v13Bn5 en vez de organismo_v13. CRITERIOS Y UMBRALES INTACTOS (criterio v3').
Salida en datos/examen_v13B_<fecha>. Generado por construye_puerta_codigo.py. NO editar.
...
def tarea_id(args):
    cual, esc, seed = args
    import organismo_v13Bn5 as v13   # B-2: puerta por evidencia del codigo
    if cual == 'v11':
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv:
        _log['f'] = open(os.path.join(RAIZ, 'datos', f'examen_v13B_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    ...
    log(f"sha organismo_v13 {h16(os.path.join(_B2, 'organismo_v13Bn5.py'))}  bateria_v13 {h16(os.path.abspath(__file__))}"
    ...
    if _log['f']:
        dj = os.path.join(RAIZ, 'datos', f'examen_v13B_{stamp}.json')
        meta = dict(fecha=time.strftime('%Y-%m-%dT%H:%M:%S'), semillas=S, semilla_inicial=seeds[0], veredictos=V, congelar=bool(todo and S >= 20),
                    sha_organismo_v13=h16(os.path.join(_B2, 'organismo_v13Bn5.py')), sha_bateria_v13=h16(os.path.abspath(__file__)),
