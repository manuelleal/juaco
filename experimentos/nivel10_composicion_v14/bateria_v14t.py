"""bateria_v14t = organismo/bateria_v13.py (1a027bcb37eb536e, CONGELADO: solo se leyo) apuntando a
...
if __name__ == '__main__':
    ...
    if '--log' in sys.argv:
        _log['f'] = open(os.path.join(RAIZ, 'datos', f'examen_v14t_{stamp}.log'), 'w', encoding='utf-8', newline='\n')
    ...
    log(f"sha organismo_v13 {h16(os.path.join(_D14T, 'organismo_v14t_on.py'))}  bateria_v13 {h16(os.path.abspath(__file__))}"
    ...
    with mp.Pool(N_PARALELO) as pool:
        ...
        for c, nombre in (('v11', "v14t(eta_s=0,puerta=None,k_testE=0,eta_pred=0) == v11  [v3'' ERR-30]"), ('v10', "v14t(eta_s=0,puerta=None,div_signo=False,k_testE=0,eta_pred=0) == v10  [v3'' ERR-30]")):
    ...
    if _log['f']:
        dj = os.path.join(RAIZ, 'datos', f'examen_v14t_{stamp}.json')
        meta = dict(fecha=time.strftime('%Y-%m-%dT%H:%M:%S'), semillas=S, semilla_inicial=seeds[0], veredictos=V, congelar=bool(todo and S >= 20),
                    sha_organismo_v13=h16(os.path.join(_D14T, 'organismo_v14t_on.py')), sha_bateria_v13=h16(os.path.abspath(__file__)),
