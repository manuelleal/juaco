...
def run(seed,T=100000,learn=True,plan=None,pats=None,chk=None,
        ...
        plast=True,theta=0.6,ema=0.02,paso=0.5,lam=0.0,memoria_rechazo=20,mu_norm=False,div_signo=False,eta_s=0.0,clip_s=3.0,puerta=None,puerta_pat=5,pat_shuf=0,pat_min=1,mask_rel=2,del_s=0.25,del_c=0.25,ema_c=0.05):
    ...
    return dict(split_t=split_t,mord=mord,vis=vis,W=W,comp=comp,deaths=deaths,log=log,splits=splits,celdas=int(activa.sum()),puerta_pat=puerta_pat,pat_shuf=pat_shuf,pat_min=pat_min,n_cod=len(ncod),
                ...
                hist=hist,t_agot=t_agot,err_max=err_max,t_entra=t_entra,val=dict(val),t_techo=t_techo,techo_primero=techo_primero,n_techo=n_techo,mv_tot=mv_tot,mc_tot=mc_tot,nofam_fin=nofam_fin,nofam_est=nofam_est,ncod_est=ncod_est)
