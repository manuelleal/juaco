"""bateria_generaliza_v14t = organismo/bateria_generaliza.py (46772f5a582872c8) con DOS entradas mas en
...
if __name__ == '__main__':
    ...
    log(f"instrumento {inst} {h16(os.path.join(_dir, inst + '.py'))}"
        f"  organismo {h16(os.path.join(AQUI, 'organismo_v13.py') if modulo == 'organismo_v13_rapido' else (os.path.join(AQUI, modulo + '.py') if os.path.exists(os.path.join(AQUI, modulo + '.py')) else os.path.join(_D14T, modulo.split('_on')[0] + '.py')))}  esta bateria {h16(os.path.abspath(__file__))}")
    ...
    if _log['f']:
        ...
        json.dump(dict(meta=dict(fecha=time.strftime('%Y-%m-%dT%H:%M:%S'), modulo=modulo, semillas=seeds, veredictos=V,
                                 sha_bateria=h16(os.path.abspath(__file__)), sha_organismo=(h16(os.path.join(AQUI, modulo + '.py')) if os.path.exists(os.path.join(AQUI, modulo + '.py')) else h16(os.path.join(_D14T, modulo.split('_on')[0] + '.py'))),
